...
#define TEST_HIT_PRED(_hit, _pred)                                \
  ({                                                              \
    if (ret_val.hit != (_hit) || ret_val.prediction != (_pred)) { \
      rc = 1;                                                     \
      cout << "lpb failure: \n\t" << ret_val << endl;             \
    }                                                             \
  })
...
int main() {
  ...
  ret_val = lpb.lookup(p, p);
  // Here, we expect a misprediction, because we are moving to a shorter loop
  // length
  TEST_HIT_PRED(true, false);
  lpb.update(false, true, p, p);
  ...
  // Now, switch to the opposite direction, to test to see if that code
  // overrides the state properly
  ret_val = lpb.lookup(p, p);
  // This will be our first miss, but it's okay, it doesn't know about our
  // change in direction yet
  TEST_HIT_PRED(true, false);
  lpb.update(false, true, p, p);

  // Here it should have detected our change in direction and disabled the hit
  // signal
  ret_val = lpb.lookup(p, p);
  TEST_HIT(false);
  lpb.update(false, true, p, p);
