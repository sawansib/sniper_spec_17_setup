...
void update_environment(char* base_path) {
  ...
  pin_32_ld_library_path =
      append3(pin_runtime_libs32, ":", pin_32_ld_library_path);
  pin_64_ld_library_path =
      append3(pin_runtime_libs64, ":", pin_64_ld_library_path);
  ...
  /* This variable tells the injector to restore environment variables after pin
   * is injected. */
  ...
  /*
   * Backup the LD_LIBRARY_PATH, since pin uses a different one while launching.
   * It will be restored when the app is loaded to memory.
   */
  if (ld_library_path) {
  ...
  r = setenv("PIN_INJECTOR32_LD_LIBRARY_PATH", new_32_ld_library_path,
             overwrite);
  check_retval(r, "setenv PIN_INJECTOR32_LD_LIBRARY_PATH");
  r = setenv("PIN_INJECTOR64_LD_LIBRARY_PATH", new_64_ld_library_path,
             overwrite);
  check_retval(r, "setenv PIN_INJECTOR64_LD_LIBRARY_PATH_64");

  /*
   * If the LD_BIND_NOW, LD_ASSUME_KERNEL and LD_PRELOAD variables were defined
   * they should pass as is to the app. Since pin's injector doesn't need it, we
   * save them now and restore it after pin is injected into the process.
   */
...
char** build_child_argv(char* base_path, int argc, char** argv, int user_argc,
  ...
  /*
   * Since 64bit system can run 32bit executables, we run the 32bit pinbin. If
   * this is a 64bit machine, pinbin will switch to the 64bit version of itself
   * based on the -p64 parameter.
   */
