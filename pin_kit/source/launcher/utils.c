...
int get_gcc_version_string(int* major, int* minor) {
  ...
  if (!check_file_exists("/usr/bin/gcc")) return 0;

  /* Open the command for reading. */
  fp = popen(
      "/usr/bin/gcc --version | egrep -o \"[1-9]{1}\\.[1-9]{1}(\\.[0-9]*)?\" | "
      "head -1",
      "r");
  if (fp == NULL) {
    return 0;
...
/*!
 * @brief Checks for file properties and prints an error message is requirements
 * are not met.
 * @param fn The file path
 */
...
/*!
 * Searches for the given executable in the directories list at the PATH
 * environment variable, and returns the directory it was found in.
