 ...
 * build_user_argv. This function should return an array of the tool specific
 * arguments and their count. These arguments will be added to the pin command
 * line.
 *
 * An example of adding a tool name and its 64bit version is shown in the
 * comment inside build_user_argv.
 *
 */
...
void check_environment() {
  char* s;
  int i;
  const char* array[] = {"LD_LIBRARY_PATH",
...
int main(int orig_argc, char** orig_argv) {
  ...
  child_argv =
      build_child_argv(base_path, orig_argc, orig_argv, user_argc, user_argv);
