...
int main(int argc, char *argv[]) {
  ...
  if (argc == 1) {
    ...
    for (i = 0; i < argc - 1; i++) args[i] = atoi(argv[i + 1]);
    n = argc - 1;
  }
  ...
  for (i = 0; i < n; i += 2) {
    ...
    dispatch_source_set_event_handler(src, ^{
      /* Count down to zero */
      count--;
      dispatch_async(print_serial, ^{
        printf("%d second timer   count %u\n", args[i], count);
      });
      ...
      if (ntimers == 0) {
         ...
         */
        dispatch_sync(print_serial, ^{
          printf("All timers finished.  Goodbye\n");
        });
        exit(0);
      }
    });
