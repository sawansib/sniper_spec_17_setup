...
void routineEnter(THREADID threadIndex, IntPtr eip, IntPtr esp) {
  localStore[threadIndex].thread->getRoutineTracer()->routineEnter(
      eip, esp, localStore[threadIndex].lastCallSite);
...
void announceRoutine(RTN rtn) { announceRoutine(RTN_InsHeadOnly(rtn)); }
...
void addRtnTracer(RTN rtn) {
  if (Sim()->getRoutineTracer()) {
    ...
    RTN_InsertCall(rtn, IPOINT_BEFORE, AFUNPTR(routineEnter), IARG_THREAD_ID,
                   IARG_ADDRINT, RTN_Address(rtn), IARG_REG_VALUE,
...
void addRtnTracer(TRACE trace) {
  if (Sim()->getRoutineTracer()) {
    ...
    // to the function we think we're in. This will detect longjmps and tail
    // call elimination, and fix up the call stack appropriately.
    ...
    if (RTN_Valid(rtn)) {
      ...
      if (!Sim()->getRoutineTracer()->hasRoutine(0)) announceInvalidRoutine();
