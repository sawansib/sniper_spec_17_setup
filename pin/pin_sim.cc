...
VOID printInsInfo(CONTEXT *ctxt) {
  __attribute__((__unused__)) ADDRINT reg_inst_ptr =
      PIN_GetContextReg(ctxt, REG_INST_PTR);
  __attribute__((__unused__)) ADDRINT reg_stack_ptr =
...
void handleCheckScheduled(THREADID threadIndex) {
  ...
  if (core == NULL) {
    ...
    core->getPerformanceModel()->queuePseudoInstruction(
        new SpawnInstruction(time));
  }
}

VOID addCheckScheduled(TRACE trace, INS ins_head) {
  INS_InsertCall(ins_head, IPOINT_BEFORE, AFUNPTR(handleCheckScheduled),
                 IARG_THREAD_ID, IARG_END);
}

static int getInstMode() { return Sim()->getInstrumentationMode(); }
...
#ifdef PINPLAY_SUPPORTED
  // Make sure all version switches happen before any PinPlay instrumentation,
  // to avoid PinPlay seeing some instructions twice
  CALL_ORDER call_order =
      (CALL_ORDER)(pinplay_engine.PinPlayFirstBeforeCallOrder() - 1);
#else
  CALL_ORDER call_order = CALL_ORDER_DEFAULT;
#endif

  // Write the resulting mode to REG_INST_Gx for use by INS_InsertVersionCase
  INS_InsertCall(ins_head, IPOINT_BEFORE, (AFUNPTR)getInstMode,
                 IARG_RETURN_REGS, g_toolregs[TOOLREG_TEMP], IARG_CALL_ORDER,
                 call_order, IARG_END);

// Add version switch cases for all possible target versions (no test to switch
// to self)
#define SWITCH_VERSION(v)                                           \
  if (inst_mode != (v))                                             \
    INS_InsertVersionCase(ins_head, g_toolregs[TOOLREG_TEMP], v, v, \
                          IARG_CALL_ORDER, call_order, IARG_END);
  ...
  if (TRACE_Version(trace) == 0) return;
  ...
  // In fast-forward mode, this is the only instrumentation (except for some
  // rare stuff like magic, syscall),
  ...
  for (BBL bbl = bbl_head; BBL_Valid(bbl); bbl = BBL_Next(bbl)) {
    switch (instr_level) {
      ...
      case (InstrumentLevel::INSTR_WITH_BBVS):
        BBL_InsertCall(bbl, IPOINT_ANYWHERE,
                       (AFUNPTR)InstructionModeling::countInstructions,
    ...
    if (Sim()->getConfig()->getEnableICacheModeling()) {
      INSTRUMENT(INSTR_IF_CACHEONLY(inst_mode), trace, BBL_InsHead(bbl),
                 IPOINT_BEFORE,
                 AFUNPTR(InstructionModeling::accessInstructionCacheWarmup),
                 IARG_THREAD_ID, IARG_ADDRINT, BBL_Address(bbl), IARG_UINT32,
    ...
    for (INS ins = BBL_InsHead(bbl);; ins = INS_Next(ins)) {
      ...
      if (ins == BBL_InsTail(bbl) || icount >= 32 || INS_HasRealRep(ins)) {
        if (INS_HasFallThrough(ins))
          INSTRUMENT(INSTR_IF_DETAILED(inst_mode), trace, ins, IPOINT_AFTER,
                     AFUNPTR(InstructionModeling::handleBasicBlock),
                     IARG_THREAD_ID, IARG_END);
        if (INS_IsBranch(ins))
          INSTRUMENT(INSTR_IF_DETAILED(inst_mode), trace, ins,
                     IPOINT_TAKEN_BRANCH,
                     AFUNPTR(InstructionModeling::handleBasicBlock),
                     IARG_THREAD_ID, IARG_END);
        icount = 0;
...
VOID traceInvalidate(ADDRINT orig_pc, ADDRINT cache_pc, BOOL success) {
  if (!Sim()->getConfig()->getEnableSMCSupport()) {
    ...
    // (which means: stop using caches for basic blocks and decoded instruction
    // from now on). Just in case enabling this half-way through isn't reliable,
    // keep the warning for now.
    LOG_PRINT_WARNING_ONCE(
        "Trace invalidation orig_pc(%p) cache_pc(%p) success(%d)\n\n"
...
void ApplicationDetach(void *) { ApplicationExit(0, NULL); }

void PinDetach(void) {
  // PIN_Detach();
}

VOID threadStartCallback(THREADID threadIndex, CONTEXT *ctxt, INT32 flags,
                         VOID *v) {
  if (!done_app_initialization) {
    ...
    if (Sim()->getTraceManager()) Sim()->getTraceManager()->start();
  ...
  // Don't resize this vector at runtime as it's shared and we don't want it to
  // be reallocated while someone uses it
  LOG_ASSERT_ERROR(localStore.size() > threadIndex,
                   "Ran out of thread_id slots, increase MAX_PIN_THREADS");

  memset(&localStore[threadIndex], 0, sizeof(localStore[threadIndex]));
  localStore[threadIndex].thread =
      Sim()->getThreadManager()->getThreadFromID(thread_id);
  localStore[threadIndex].thread->m_os_info.tid = syscall(__NR_gettid);
  if (Sim()->getConfig()->getEnableSpinLoopDetection())
    localStore[threadIndex].sld.sld =
        new SpinLoopDetector(localStore[threadIndex].thread);
  for (unsigned int idx = 0; idx < ThreadLocalStorage::NUM_SCRATCHPADS; ++idx) {
    int status =
        posix_memalign((void **)&localStore[threadIndex].scratch[idx],
                       4 * sizeof(void *), ThreadLocalStorage::SCRATCHPAD_SIZE);
    assert(status == 0);
  }
}

VOID threadFiniCallback(THREADID threadIndex, const CONTEXT *ctxt, INT32 flags,
                        VOID *v) {
  if (forkedInChild) return;

  Sim()->getThreadManager()->onThreadExit(
      localStore[threadIndex].thread->getId());

  if (localStore[threadIndex].thread->getId() == 0)
    if (Sim()->getTraceManager()) Sim()->getTraceManager()->stop();
}

bool handleSigUsr1(THREADID threadIndex, INT32 signal, CONTEXT *ctx,
...
VOID forkAfterChild(THREADID threadid, const CONTEXT *ctxt, VOID *v) {
  // Application has fork()ed. We don't want to track child processes, but Pin
  // will attempt to do cleanups. Make sure that these don't trigger any
  // simulator code.
  forkedInChild = true;
}
...
int main(int argc, char *argv[]) {
  ...
  // To make sure output shows up immediately, make stdout and stderr line
  // buffered (if we're writing into a pipe to run-graphite, or redirected to a
  // file by the job runner, the default will be block buffered)
  setvbuf(stdout, NULL, _IOLBF, 0);
  setvbuf(stderr, NULL, _IOLBF, 0);

  const char *ld_orig = getenv("SNIPER_SCRIPT_LD_LIBRARY_PATH");
  if (ld_orig) setenv("LD_LIBRARY_PATH", ld_orig, 1);
  ...
  if (PIN_GetDebugStatus() != DEBUG_STATUS_DISABLED) {
    ...
    if (PIN_GetDebugConnectionInfo(&info) &&
      ...
      FILE *fp = fopen(
          Sim()->getConfig()->formatOutputFileName("appdebug_port.out").c_str(),
          "w");
      fprintf(fp, "%d %d", PIN_GetPid(), info._tcpServer._tcpPort);
      fclose(fp);
    }
    String scheme =
        Sim()->getCfg()->getString("clock_skew_minimization/scheme");
    if (!(scheme == "none" || scheme == "barrier")) {
      fprintf(stderr,
              "\n[WARNING] Application debugging is not compatible with %s "
              "synchronization.\n",
              scheme.c_str());
      fprintf(stderr,
              "          Consider adding -g "
              "--clock_skew_minimization/scheme={none|barrier}\n\n");
    }
  }
  ...
  if (cfg->getBool("log/pin_codecache_trace")) initCodeCacheTracing();
  ...
  if (syntax == "intel")
    ...
    // PIN_SpawnInternalThread doesn't schedule its threads until after
    // PIN_StartProgram
  ...
  if (Sim()->getConfig()->getEnablePinPlay()) {
    ...
    LOG_ASSERT_ERROR(
        Sim()->getConfig()->getEnableSyscallEmulation() == false,
        "System call emulation must be disabled when using PinPlay.");
  }
#else
  LOG_ASSERT_ERROR(Sim()->getConfig()->getEnablePinPlay() == false,
                   "PinPlay support not compiled in. Please use a compatible "
                   "pin kit when compiling.");
#endif

  // config::Config shouldn't be called outside of init/fini
  // With Sim()->hideCfg(), we let Simulator know to complain when someone does
  // call Sim()->getCfg()
  Sim()->hideCfg();
