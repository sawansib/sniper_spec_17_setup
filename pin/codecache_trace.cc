...
VOID newCacheBlock(USIZE new_block_size) { atomic_inc_int64(cc_counters[0]); }

VOID fullCache(USIZE trace_size, USIZE stub_size) {
  atomic_inc_int64(cc_counters[1]);
}

VOID cacheFlushed() { atomic_inc_int64(cc_counters[2]); }

VOID codeCacheEntered(ADDRINT cache_pc) { atomic_inc_int64(cc_counters[3]); }
...
VOID traceInserted(TRACE trace, VOID* v) { atomic_inc_int64(cc_counters[7]); }
...
void initCodeCacheTracing() {
  cctrace = fopen(
      Sim()->getConfig()->formatOutputFileName("cctrace.txt").c_str(), "w");
  LOG_ASSERT_ERROR(cctrace != NULL, "Expected to be able to create a file");

  ccstats = fopen(
      Sim()->getConfig()->formatOutputFileName("ccstats.txt").c_str(), "w");
  LOG_ASSERT_ERROR(cctrace != NULL, "Expected to be able to create a file");
  ...
  Sim()->getHooksManager()->registerHook(
      ...
      (HooksManager::HookCallbackFunc)codeCachePeriodicCallback, (UInt64)NULL);
