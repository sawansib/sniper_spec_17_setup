...
void InstructionModeling::handleInstruction(THREADID thread_id,
  ...
  localStore[thread_id].dynins =
      prfmdl->createDynamicInstruction(instruction, instruction->getAddress());
}
...
static void handleBranch(THREADID thread_id, ADDRINT eip, BOOL taken,
                         ADDRINT target) {
  assert(localStore[thread_id].dynins);
  localStore[thread_id].dynins->addBranch(taken, target);
}

static void handleBranchWarming(THREADID thread_id, ADDRINT eip, BOOL taken,
                                ADDRINT target) {
  ...
  if (mispredict) prfmdl->handleBranchMispredict();
}

static void handleCallDirect(THREADID thread_id, ADDRINT target) {
  assert(localStore[thread_id].dynins);
  localStore[thread_id].dynins->addCallDirect(target);
}

static void handleCallIndirect(THREADID thread_id, ADDRINT target) {
  assert(localStore[thread_id].dynins);
  localStore[thread_id].dynins->addCallIndirect(target);
}

static void handleRetIndirect(THREADID thread_id, ADDRINT target) {
  assert(localStore[thread_id].dynins);
  localStore[thread_id].dynins->addRetIndirect(target);
}

static void handleJumpDirect(THREADID thread_id, ADDRINT target) {
  assert(localStore[thread_id].dynins);
  localStore[thread_id].dynins->addJumpDirect(target);
}

static void handleJumpIndirect(THREADID thread_id, ADDRINT target) {
  assert(localStore[thread_id].dynins);
  localStore[thread_id].dynins->addJumpIndirect(target);
}

ADDRINT handleMagic(THREADID threadIndex, ADDRINT a, ADDRINT b, ADDRINT c) {
  return handleMagicInstruction(localStore[threadIndex].thread->getId(), a, b,
                                c);
}
...
static void handleRdtsc(THREADID thread_id, PIN_REGISTER *gax,
                        PIN_REGISTER *gdx) {
  Core *core = localStore[thread_id].thread->getCore();
  assert(core);
...
static void handleCpuid(THREADID thread_id, PIN_REGISTER *gax,
                        PIN_REGISTER *gbx, PIN_REGISTER *gcx,
                        PIN_REGISTER *gdx) {
  Core *core = localStore[thread_id].thread->getCore();
  assert(core);
...
static void fillOperandList(OperandList *list, INS ins) {
  ...
  for (unsigned int i = 0; i < max_read_regs; i++) {
    ...
    if (REG_valid(reg_i))
      list->push_back(Operand(Operand::REG, reg_i, Operand::READ,
                              REG_StringShort(reg_i).c_str(),
                              INS_RegRContain(ins, reg_i)));
  }

  for (unsigned int i = 0; i < max_write_regs; i++) {
    REG reg_i = INS_RegW(ins, i);
    if (REG_valid(reg_i))
      list->push_back(Operand(Operand::REG, reg_i, Operand::WRITE,
                              REG_StringShort(reg_i).c_str(),
                              INS_RegWContain(ins, reg_i)));
  }

  // immediate
  for (unsigned int i = 0; i < INS_OperandCount(ins); i++) {
    if (INS_OperandIsImmediate(ins, i)) {
      list->push_back(Operand(Operand::IMMEDIATE, INS_OperandImmediate(ins, i),
                              Operand::READ));
    }
  }
}

std::unordered_map<ADDRINT, const std::vector<const MicroOp *> *>
    instruction_cache;

VOID InstructionModeling::addInstructionModeling(
    TRACE trace, INS ins, InstMode::inst_mode_t inst_mode) {
  // Functional modeling
  ...
  if (INS_IsXchg(ins) && INS_OperandReg(ins, 0) == REG_BX &&
    ...
    if (Sim()->getConfig()->getEnableSyscallEmulation()) {
      INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)handleMagic,
                               IARG_THREAD_ID, IARG_REG_VALUE, REG_GAX,
#ifdef TARGET_IA32
                               IARG_REG_VALUE, REG_GDX,
#else
                               IARG_REG_VALUE, REG_GBX,
#endif
                               IARG_REG_VALUE, REG_GCX, IARG_RETURN_REGS,
                               REG_GAX, IARG_END);
      // Stop the trace after MAGIC (Redmine #118), which has potentially
      // changed the instrumentation mode, so execution can resume in the
  ...
  if (Sim()->getConfig()->getEnableSyscallEmulation()) {
    if (INS_IsRDTSC(ins))
      INS_InsertPredicatedCall(ins, IPOINT_AFTER, (AFUNPTR)handleRdtsc,
                               IARG_THREAD_ID, IARG_REG_REFERENCE, REG_GAX,
                               IARG_REG_REFERENCE, REG_GDX, IARG_END);

    if (INS_Opcode(ins) == XED_ICLASS_CPUID) {
      INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)handleCpuid,
                               IARG_THREAD_ID, IARG_REG_REFERENCE, REG_GAX,
                               IARG_REG_REFERENCE, REG_GBX, IARG_REG_REFERENCE,
  ...
  if (INS_IsSyscall(ins) && Sim()->getConfig()->getEnableSyscallEmulation()) {
    INS_InsertPredicatedCall(ins, IPOINT_BEFORE, AFUNPTR(lite::handleSyscall),
  ...
  if (INSTR_IF_DETAILED(inst_mode) && !INS_IsSyscall(ins)) {
    ...
    INSTRUMENT(INSTR_IF_DETAILED(inst_mode), trace, ins, IPOINT_BEFORE,
               AFUNPTR(InstructionModeling::handleInstruction), IARG_THREAD_ID,
               IARG_PTR, inst, IARG_END);
  }

  // Timing models, will add dynamic information to localStore[thread_id].dynins
  if (INS_IsBranchOrCall(ins)) {
    if (INS_HasFallThrough(ins)) {  // Conditional
      if (INS_IsCall(ins)) {
        assert(false);  // Conditional calls not found yet
      } else if (INS_IsRet(ins)) {
        assert(false);  // Conditional rets not found yet
      } else {
        assert(INS_IsBranch(ins));
        assert(INS_IsDirectBranchOrCall(ins));

        // In warming mode, warm up the branch predictors
        INSTRUMENT_PREDICATED(INSTR_IF_CACHEONLY(inst_mode), trace, ins,
                              IPOINT_TAKEN_BRANCH, (AFUNPTR)handleBranchWarming,
                              IARG_THREAD_ID, IARG_ADDRINT, INS_Address(ins),
                              IARG_BOOL, TRUE, IARG_BRANCH_TARGET_ADDR,
                              IARG_END);

        INSTRUMENT_PREDICATED(INSTR_IF_CACHEONLY(inst_mode), trace, ins,
                              IPOINT_AFTER, (AFUNPTR)handleBranchWarming,
                              IARG_THREAD_ID, IARG_ADDRINT, INS_Address(ins),
                              IARG_BOOL, FALSE, IARG_BRANCH_TARGET_ADDR,
                              IARG_END);

        // In detailed mode, push a DynamicInstructionInfo
        INSTRUMENT_PREDICATED(INSTR_IF_DETAILED(inst_mode), trace, ins,
                              IPOINT_TAKEN_BRANCH, (AFUNPTR)handleBranch,
                              IARG_THREAD_ID, IARG_ADDRINT, INS_Address(ins),
                              IARG_BOOL, TRUE, IARG_BRANCH_TARGET_ADDR,
                              IARG_END);

        INSTRUMENT_PREDICATED(INSTR_IF_DETAILED(inst_mode), trace, ins,
                              IPOINT_AFTER, (AFUNPTR)handleBranch,
                              IARG_THREAD_ID, IARG_ADDRINT, INS_Address(ins),
                              IARG_BOOL, FALSE, IARG_BRANCH_TARGET_ADDR,
                              IARG_END);
      }
      ...
      if (INS_IsCall(ins)) {
        if (INS_IsDirectBranchOrCall(ins)) {
          INSTRUMENT_PREDICATED(INSTR_IF_DETAILED(inst_mode), trace, ins,
                                IPOINT_TAKEN_BRANCH, (AFUNPTR)handleCallDirect,
                                IARG_THREAD_ID, IARG_BRANCH_TARGET_ADDR,
                                IARG_END);
        } else {
          INSTRUMENT_PREDICATED(INSTR_IF_DETAILED(inst_mode), trace, ins,
                                IPOINT_TAKEN_BRANCH,
                                (AFUNPTR)handleCallIndirect, IARG_THREAD_ID,
                                IARG_BRANCH_TARGET_ADDR, IARG_END);
        }
      } else if (INS_IsRet(ins)) {
        if (INS_IsDirectBranchOrCall(ins)) {
          assert(false);  // Direct rets not found yet
        } else {
          INSTRUMENT_PREDICATED(INSTR_IF_DETAILED(inst_mode), trace, ins,
                                IPOINT_TAKEN_BRANCH, (AFUNPTR)handleRetIndirect,
                                IARG_THREAD_ID, IARG_BRANCH_TARGET_ADDR,
                                IARG_END);
        }
      } else {  // Jmp
        if (INS_IsDirectBranchOrCall(ins)) {
          INSTRUMENT_PREDICATED(INSTR_IF_DETAILED(inst_mode), trace, ins,
                                IPOINT_TAKEN_BRANCH, (AFUNPTR)handleJumpDirect,
                                IARG_THREAD_ID, IARG_BRANCH_TARGET_ADDR,
                                IARG_END);
        } else {
          INSTRUMENT_PREDICATED(INSTR_IF_DETAILED(inst_mode), trace, ins,
                                IPOINT_TAKEN_BRANCH,
                                (AFUNPTR)handleJumpIndirect, IARG_THREAD_ID,
                                IARG_BRANCH_TARGET_ADDR, IARG_END);
        }
      }
    }
  ...
  if (INS_IsXchg(ins)) {
    ...
    if (INS_OperandReg(ins, 0) == INS_OperandReg(ins, 1) &&
      ...
      INSTRUMENT_PREDICATED(INSTR_IF_DETAILED(inst_mode), trace, ins,
                            IPOINT_BEFORE, (AFUNPTR)handleXchg,
...
Instruction *InstructionModeling::decodeInstruction(INS ins) {
  ...
  const std::vector<const MicroOp *> *uops =
      InstructionDecoder::decode(INS_Address(ins), INS_XedDec(ins), inst);
  inst->setMicroOps(uops);

  return inst;
}

VOID InstructionModeling::countInstructions(THREADID thread_id, ADDRINT address,
                                            INT32 count) {
  if (!Sim()->isRunning()) {
    // Main thread has exited, but we still seem to be running.
    // Don't touch any more simulator structure as they're being deallocated
    // right now. Just wait here until the whole application terminates us.
    while (1) sched_yield();
  }
  ...
  if (check_rescheduled) {
    // If countInstructions returns true, we may have been rescheduled (using
    // the fast-forward performance model)
    SubsecondTime time;
...
VOID InstructionModeling::accessInstructionCacheWarmup(THREADID threadid,
                                                       ADDRINT address,
                                                       UINT32 size) {
