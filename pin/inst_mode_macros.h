...
#define INSTR_IF_NOT_CACHEONLY(__inst_mode) \
  ((__inst_mode) != InstMode::CACHE_ONLY)
#define INSTR_IF_FASTFORWARD(__inst_mode) \
  ((__inst_mode) == InstMode::FAST_FORWARD)
#define INSTR_IF_NOT_FASTFORWARD(__inst_mode) \
  ((__inst_mode) != InstMode::FAST_FORWARD)

#define __INSTRUMENT(predicated, condition, trace, ins, point, func, ...) \
  if (condition) INS_Insert##predicated##Call(ins, point, func, __VA_ARGS__);
