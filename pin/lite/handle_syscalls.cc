...
void handleSyscall(THREADID threadIndex, CONTEXT* ctx) {
  // We shouldn't block on actual SYS_futex calls while inside the SyscallEntry
  // function Therefore, do all the work here, which is at
  // INS_InsertCall(syscall, IPOINT_BEFORE)

  IntPtr syscall_number = PIN_GetContextReg(ctx, REG_GAX);
  LOG_PRINT("Syscall Number(%d)", syscall_number);
  // printf("Entering syscall %s(%ld)\n", syscall_string(syscall_number),
  // syscall_number);
  ...
  if (syscall_number == SYS_clone) {
    thread_id_t new_thread_id = Sim()->getThreadManager()->spawnThread(
        localStore[threadIndex].thread->getId(), 0);
    Thread* new_thread =
        Sim()->getThreadManager()->getThreadFromID(new_thread_id);
...
#endif
    new_thread->m_os_info.clear_tid =
        args.arg0 & CLONE_CHILD_CLEARTID ? true : false;
  } else if (syscall_number == SYS_execve) {
    LOG_PRINT_WARNING(
        "execv system call not supported in Pin mode, use SIFT mode instead "
        "(--sift)");
  }
...
void syscallEnterRunModel(THREADID threadIndex, CONTEXT* ctx,
...
void syscallExitRunModel(THREADID threadIndex, CONTEXT* ctx,
                         SYSCALL_STANDARD syscall_standard, void* v) {
  Thread* thread = localStore[threadIndex].thread;
  LOG_ASSERT_ERROR(thread != NULL, "Thread(NULL)");
  ...
  if (thread->getSyscallMdl()->getCurrentSyscallNumber() == SYS_clone) {
    Thread* new_thread = Sim()->getThreadManager()->getThreadFromID(
        localStore[threadIndex].clone.new_thread_id);
...
bool interceptSignal(THREADID threadIndex, INT32 signal, CONTEXT* ctx,
  ...
  switch (PIN_GetDebugStatus()) {
    case DEBUG_STATUS_CONNECTED:
      ...
      break;
    // TODO: This is supposed to work in a future Pin version. Right now, a
    // fatal signal will kill Pin & the application
    case DEBUG_STATUS_UNCONNECTED:
      // The -appdebug_enable switch was used, but no debugger is currently
      // connected Instruct the user to do this now
    ...
    case DEBUG_STATUS_UNCONNECTABLE:
      // Application debugging is enabled, but it is too early to allow a
      // debugger to connect
      fprintf(stderr,
              "\nApplication debugging is enabled, but it is too early to "
    ...
    case DEBUG_STATUS_DISABLED:
      ...
      fprintf(stderr,
              ...
              "start Sniper with the --appdebug or --appdebug-wait command "
              "line switch.\n\n");
      exit(0);
  }
