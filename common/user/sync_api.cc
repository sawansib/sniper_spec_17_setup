...
// Defined with C linkage so we cannot return a pair<latency, success> as
// SyncClient::mutexTrylock does SubsecondTime::MaxTime() = not locked else =
// latency
SubsecondTime CarbonMutexTrylock(carbon_mutex_t *mux) {
  Thread *thread = Sim()->getThreadManager()->getCurrentThread();
  std::pair<SubsecondTime, bool> res =
      thread->getSyncClient()->mutexTrylock(mux);
  if (res.second)
    return res.first;
  else
