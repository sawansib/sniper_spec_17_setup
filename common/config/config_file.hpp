 ...
 * It accepts files very similar to the windows .reg file format, as in
 sub-sections
 ...
 * \verbatim
  ...
  --------------------- \endverbatim
 *
 * It uses boost::regex to perform the parsing. Note that comments are not
 preserved
 * across saves.
 */
...
string       = '"' >> *lex_escape_ch_p >> '"' | +(alnum | punct) >> *(alnum |
punct ) \endverbatim
 */
class ConfigFile : public config::Config {
 ...
 private:
  ...
  /*! loadConfig() Function which loads the Config tree with the entries
   * extracted from the file located at the m_path member variable.
   */
  void loadConfig();
  ...
  void createStringKey(Section &current, const String key_name,
                       const String &value);
  ...
  // The following code is commented out until full thread protection is
  // provided in the get/set functions as well as block writing, preferably
