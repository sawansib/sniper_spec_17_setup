...
ConfigFile::ConfigFile(bool case_sensitive) : Config(case_sensitive) {}
...
void ConfigFile::loadConfigFromString(const String& cfg) { parse(cfg, m_root); }

// Build the in-memory representation by line-by-line matching boost::regex
// expressions
void ConfigFile::parse(const String& source, Section& current) {
  // rules
  config_parser parser;
  parse_info_t info = ast_parse<config_parser::factory_t>(
      source.c_str(), source.c_str() + source.size(), parser,
      space_p | comment_p("#"));
...
void ConfigFile::showParseTree(tree_iter_t const& node, int depth) {
  String tabs = "";
  for (int i = 0; i < depth; i++) tabs = tabs.append("    ");
  ...
  // if(rule == sectionID || rule == keyID || rule == keyValueID || rule ==
  // keyNameID || rule == sectionNameID) if(rule > 0)
  std::cout << tabs << (int)(rule) << ".[" << value << "]" << std::endl;

  for (tree_iter_t chi = node->children.begin(); chi != node->children.end();
       chi++) {
...
void ConfigFile::unEscapeText(const String& source, String& dest) {
  ...
  if (source.size() >= 2 && source.c_str()[0] == '"' &&
      source.c_str()[source.size() - 1] == '"') {
    ++start;
    --end;
  }
...
String ConfigFile::getNodeValue(tree_iter_t const& node) {
  if (node->value.begin() == node->value.end()) return "";
...
// This function recursively decends the tree built by the parser and populates
// the config file with the appropriate entries
void ConfigFile::evalTree(Section& current, tree_iter_t const& node,
                          int depth) {
  String tabs = "";
  for (int i = 0; i < depth; i++) tabs = tabs.append("    ");
  ...
  if (rule == sectionNameID) {
    // Since this is a  flat file representation, whenever we get a section we
    // only add it to the root node, thus we break out if we are currently
    // nested.
    if (!current.isRoot()) return;

    // HACK: Strip off the '[' and ']', guaranteed to be matched.
    // This should be removed when we upgrade to Boost 1.35, as per
    // config_file_grammar.hpp note we should just be able to say section_name =
    // value
    String section_name;
    ...
    for (tree_iter_t chi_node = node->children.begin();
         chi_node != node->children.end(); chi_node++) {
    ...
    if (node->children.size() != 1 && node->children.size() != 2) {
      throw parserError(
          "Internal parser error: Expected one or two keyID children, not " +
          boost::lexical_cast<String>(node->children.size()));
    }

    if (getNodeID(node->children.begin()) != keyNameID) {
      throw parserError(
          "Internal parser error: Expected the first entry to be a keyNameID");
    }
    ...
    if (node->children.size() == 1) {
      ...
      switch (getNodeID(datanode)) {
        ...
        case keyValueArrayID:
          for (tree_iter_t chi = datanode->children.begin();
               chi != datanode->children.end(); chi++) {
            switch (getNodeID(chi)) {
              ...
              case keyValueID:
                ...
                if (key_value != "") current.addKey(key_name, key_value, index);
                break;
              default:
                throw parserError(
                    "Internal parser error: Expected separator or value");
            }
          }
          break;
      ...
      throw parserError(
          "Internal parser error: Unexpacted number of keyID children found: " +
          boost::lexical_cast<String>(node->children.size()));
    }
  } else {
    for (tree_iter_t chi = node->children.begin(); chi != node->children.end();
...
void ConfigFile::saveAs(const String& path) {
  ...
  output_file.exceptions(std::ofstream::eofbit | std::ofstream::failbit |
                         std::ofstream::badbit);
  ...
  if (res == -1) {
    int errsv = errno;
    String error_str("Moving " + tmp_path + " to " + path + ": error(" +
                     itostr(errsv) + ") " + strerror(errsv) + ".");
...
void ConfigFile::SaveTreeAs(std::ofstream& out, const Section& current) {
  ...
  for (KeyArrayList::const_iterator i = arr.begin(); i != arr.end(); ++i) {
    ...
    for (uint64_t index = 0; index < i->second.size(); ++index) {
      if (i->second[index]) {
        // Quote the value if it is a string
        if (i->second[index]->getFloatValid() ||
            i->second[index]->getIntValid())
  ...
  for (SectionList::const_iterator i = subsections.begin();
       i != subsections.end(); i++) {
