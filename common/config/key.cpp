...
unsigned short Key::DetermineType(String value) {
  ...
  if (icase_value == "true" || icase_value == "yes" || icase_value == "1") {
    ...
    m_value_b = true;
  } else if (icase_value == "false" || icase_value == "no" ||
             icase_value == "0") {
    valid |= TYPE_BOOL_VALID;
    m_value_b = false;
  }
...
void __attribute__((noreturn)) Key::throwInvalid(String type) const {
  if (m_value == "")
    std::cerr << "[SNIPER] "
              << "Required configuration value missing for key " << m_parentPath
              << "/" << m_name << std::endl;
  else
    std::cerr << "[SNIPER] "
              << "Invalid configuration value of type " << type << " for key "
              << m_parentPath << "/" << m_name << ": " << m_value << std::endl;
  throw std::bad_cast();
}
...
const String Key::getString() const { return m_value; }
...
void Key::getValue(bool &bool_val) const { bool_val = getBool(); }

void Key::getValue(SInt64 &int_val) const { int_val = getInt(); }

void Key::getValue(String &string_val) const { string_val = getString(); }

void Key::getValue(double &double_val) const { double_val = getFloat(); }
