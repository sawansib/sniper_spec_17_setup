...
/*!
 *\file config.hpp
 * This file contains most of the interface for the external->in memory
 *configuration management interface. There is a Config class which is the main
 *interface to the outside world for loading files, getting and setting values
 *as well as saving the given configuration. This base class is then derived
 *from for the different given backends. This Config class has a root 'Section'.
 *Each section has a list of subsections and a list of keys. Each key actually
 *contains the value.
 */
// Config Class
...
/*! \brief Config: A class for managing the interface to persistent
 * configuration entries defined at runtime. This class is used to manage a
 * configuration interface. It is the base class for which different back ends
 * will derive from.
 */
class Config {
 public:
  Config(bool case_sensitive = false)
      : m_case_sensitive(case_sensitive), m_root("", case_sensitive) {}
  Config(const Section& root, bool case_sensitive = false)
      : m_case_sensitive(case_sensitive), m_root(root, "", case_sensitive) {}
  virtual ~Config() {}
  ...
  /*! \brief A function to convert from external representation to in-memory
   * representation This function sets the member variable m_path and then calls
   * the loadConfig() function which must be implemented by the derived back-end
   * implementation. \param path - This is the path where we load the
   * configuration from. This may either be a registry path for the case of the
   * ConfigRegistry interface or a file path in the case of a ConfigFile
   * interface. \exception FileNotFound - This exception is thrown if the
   * ConfigFile interface is instructed to load a file which does not exist.
   * \exception parseError - This exception is thrown on a malformed file with
   * the ConfigFile interface.
   */
  void load(const String& path);

  /*! \brief A function which will save the in-memory representation to the
   * external representation that was specified with the load() function.
   */
  ...
  /*! \brief addSection() adds the specified path as a new section, creating
   * each entry in the path along the way.
   */
  const Section& addSection(const String& path);

  /*! \brief Look up the key at the given path, and return the value of that key
   * as a bool. \param path - Path for key to look up \exception KeyNotFound is
   * thrown if the specified path doesn't exist.
   */
  ...
  // This enables us to model optional components that may live at different
  // places (e.g. perf_model/*_cache), but relieve the user from disabling all
  // of them manually
  ...
  const String getString(const String& path) {
    return getStringArray(path, UINT64_MAX);
  }
  ...
  double getFloat(const String& path) {
    return getFloatArray(path, UINT64_MAX);
  }
  double getFloatArray(const String& path, UInt64 index);

  /*! \brief Returns a string representation of the tree starting at the
   * specified section \param current The root of the tree for which we are
  ...
  /*! addKey() Adds the specified path as a new key (with the given value),
   * creating each entry in the path along the way. \param path The path to the
   * new key \param new_key The value for the newly created key
   */
  const Key& addKey(const String& path, const String& new_key,
                    UInt64 index = UINT64_MAX) {
    return addKeyInternal(path, new_key, index);
  }
  /*! \overload addKey()
   */
 ...
 private:
  ...
  // This function splits the path up like the function above, but also allows
  // you to pass the path_elements vector in (for later traversal)
  static PathPair splitPathElements(const String& path,
                                    PathElementList& path_elements);

  // Utility function to determine if a given path is a leaf (i.e. it has no
  // '/'s in it)
