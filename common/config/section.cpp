...
Section &Section::addSubsection(const String &name_) {
  String iname(name_);
  if (!m_case_sensitive) boost::to_lower(iname);

  m_subSections.insert(std::pair<String, Section *>(
      iname, new Section(*this, name_, m_case_sensitive)));
  return *(m_subSections[iname]);
}

template <class V>
const Key &Section::addKeyInternal(const String &name_, const V &value,
                                   uint64_t index) {
  ...
  if (index == UINT64_MAX) {
    ...
    m_keys.insert(std::pair<String, Key *>(
        iname, new Key(this->getFullPath(), name_, value)));
    return *(m_keys[iname]);
  } else {
...
template const Key &Section::addKeyInternal(const String &, const String &,
                                            UInt64);
template const Key &Section::addKeyInternal(const String &, const SInt64 &,
                                            UInt64);
template const Key &Section::addKeyInternal(const String &, const double &,
                                            UInt64);
...
// Unsafe version of the getSection function, this should only be used
// internally
...
const String Section::getFullPath() const {
  ...
  if (getParent().isRoot()) return getName();
