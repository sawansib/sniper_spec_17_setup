...
Section &Config::getSection_unsafe(const String &path) {
  ...
  if (isLeaf(path)) {
    if (!m_root.hasSection(path)) m_root.addSubsection(path);
...
// Small wrapper which sets the m_path variable appropriatly and calls the
// virtual loadConfig()
...
void Config::clear() { m_root.clear(); }
  ...
  if (isLeaf(path)) {
    if (!m_root.hasKey(path, index)) {
      if (index == UINT64_MAX)
        ...
        config::Error("Configuration value %s[%i] not found.", path.c_str(),
                      index);
    } else {
...
std::pair<String, String> Config::splitPathElements(
    const String &path, PathElementList &path_elements) {
  // split up the path on "/", the last entry is the name of the key
  // Everything up to the last "/" is the 'base_path' (which will specify a
  // section)
...
template <class V>
const Key &Config::addKeyInternal(const String &path, const V &value,
                                  UInt64 index) {
  // Handle the base case
  if (isLeaf(path)) return m_root.addKey(path, value, index);
...
String Config::showTree(const Section &current, int depth) {
  ...
  for (KeyList::const_iterator i = keys.begin(); i != keys.end(); i++) {
    result += tabs + "Key: " + i->second->getName() + " - " +
              i->second->getString() + "\n";
  }
  return result;
}
