...
class Section {
 public:
  ...
  const Key& addKey(const String& name, const String& value,
    ...
    return addKeyInternal(name, value, index);
  }
   ...
   * Note: The isRoot() method can be used to determine if a section is the root
   * of the tree
   */
 ...
 private:
  template <class V>
  const Key& addKeyInternal(const String& name, const V& value,
                            UInt64 index = UINT64_MAX);
