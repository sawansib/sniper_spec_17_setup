...
// PIN requires special handling when floating point operations are used inside
// callbacks To avoid this, use integer arithmetic with values rescaled by 10k
...
class TFixedPoint {
 ...
 public:
  ...
  void set_int(SInt64 value) {
    assert((INT64_MAX / this->m_one) > llabs(value));
    this->m_value = value * this->m_one;
  }
  ...
  TFixedPoint operator*(const TFixedPoint& fp) const {
    return TFixedPoint::from_raw(this->m_value * fp.m_value / this->m_one);
  }
  TFixedPoint operator*(SInt64 i) const {
    return TFixedPoint::from_raw(this->m_value * i);
  }

  TFixedPoint operator/(const TFixedPoint& fp) const {
    return TFixedPoint::from_raw(/*__int128_t*/ (fp.m_one) * this->m_value /
                                 fp.m_value);
  }
  TFixedPoint operator/(SInt64 i) const {
    return TFixedPoint::from_raw(this->m_value / i);
...
template <SInt64 one>
inline TFixedPoint<one> operator/(SInt64 i, TFixedPoint<one>& fp) {
  return TFixedPoint<one>(i) / fp;
}
