...
class Log {
 public:
  ...
  void log(ErrorState err, const char *source_file, SInt32 source_line,
           const char *format, ...);
...
#define LOG_PRINT_ERROR(...) \
  (exit(-1))  // call exit() to inherit its __attribute__ ((__noreturn__))
...
#define __LOG_PRINT(err, file, line, ...)                                 \
  {                                                                       \
    if (unlikely(Log::getSingleton()->isLoggingEnabled()) ||              \
        err != Log::None) {                                               \
...
#define LOG_PRINT_WARNING_ONCE(...)                                   \
  {                                                                   \
    ...
    if (!already_printed) {                                           \
      ...
      _LOG_PRINT(Log::Warning,                                        \
                 "Future warnings of this type will be suppressed."); \
      already_printed = true;                                         \
    }                                                                 \
  }

// _LOG_PRINT(Log::Error) does not return, but the compiler doesn't know this
// which can result in
// ''control reaches end of non-void function'' warnings in places where the
// LOG_PRINT_ASSERT(false) idiom is used. We cannot easily add __attribute__
// ((__noreturn__)) to the LOG_PRINT_ERROR macro since that only works on
// functions, and we have the added complexity of passing the ... argument
// around. Adding exit() to the end of LOG_PRINT_ERROR effectively makes it
// inherit exit's __noreturn__ which suppresses the warning (even though the
// exit() itself isn't executed as it's never reached).
...
class FunctionTracer {
 public:
  FunctionTracer(const char *file, int line, const char *fn)
      : m_file(file), m_line(line), m_fn(fn) {
...
#define LOG_FUNC_TRACE() \
  FunctionTracer func_tracer(__FILE__, __LINE__, __PRETTY_FUNCTION__);
