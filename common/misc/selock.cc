...
void SELock::acquire_shared(void) {
  ...
  while (true) {
    ...
    // Now increment m_readers, but we need to be sure no writer comes in
    // inbetween Thus, make ourselves writer for a bit Note that we did not wait
    // for m_readers == 0, so there is no(t much) extra delay
...
void SELock::upgrade(void) {
  ...
  if (prev_writers) {
    // Someone is waiting to write. Release our read lock, and go the long way
    // (same as acquire_exclusive)
    __sync_sub_and_fetch(&m_readers, 1);
    ...
    // There were no previous waiting writers. We don't need to release the
    // shared lock we already have

    // Wait until the current writer and all readers have left
    WAIT_WHILE(m_write || m_readers);

    // No readers can come in, nor any other writer since we kept our readers
    // count
    m_write = 1;
...
void SELock::downgrade(void) {
  ...
  // First make ourselves a reader, so no other potential writer may come in yet
  // (until we fully release)
  __sync_add_and_fetch(&m_readers, 1);
