...
struct TimeConverter {
  static T PStoFS(T ps) { return ps * T(1000); }

  static inline T NStoPS(T ns) { return ns * T(1000); }

  static inline T NStoFS(T ns) { return PStoFS(NStoPS(ns)); }

  static inline T UStoNS(T us) { return us * T(1000); }
};
...
class SubsecondTime {
 public:
  ...
  uint64_t getInternalDataForced(void) const { return m_time; }

  // Not to be used normally
  void setInternalDataForced(uint64_t new_time) { m_time = new_time; }

  // Boolean operators
  friend inline bool operator==(const SubsecondTime &lhs,
                                const SubsecondTime &rhs);
  friend inline bool operator!=(const SubsecondTime &lhs,
                                const SubsecondTime &rhs);
  friend inline bool operator<(const SubsecondTime &lhs,
                               const SubsecondTime &rhs);
  ...
  friend inline bool operator>(const SubsecondTime &lhs,
                               const SubsecondTime &rhs);
  friend inline void atomic_add_subsecondtime(SubsecondTime &src_dest,
                                              const SubsecondTime &src);
  ...
  // Normally I would inline these as non-member functions,
  // (http://www.stackoverflow.com/questions/4421706), but as we would like to
  // enforce protection, we make them member functions
  ...
  static inline uint64_t divideRounded(const SubsecondTime &lhs,
                                       const SubsecondTime &rhs) {
...
inline void atomic_add_subsecondtime(SubsecondTime &src_dest,
                                     const SubsecondTime &src) {
  __sync_fetch_and_add(&src_dest.m_time, src.m_time);
}

// Base period (frequency) of a component.  This class is normally referenced as
// a pointer in other generating classes
...
class ComponentPeriod {
 public:
  ...
  //  to allow operations on SubsecondTime
  // From http://www.stackoverflow.com/questions/4421706
  operator SubsecondTime() const { return m_period; }
 ...
 private:
  ...
  ComponentPeriod(SubsecondTime &_time) : m_period(_time) {}
...
class SubsecondTimeCycleConverter {
 public:
  ...
  UInt64 subsecondTimeToCycles(SubsecondTime time) const {
    ...
    return (time / static_cast<const SubsecondTime>(*m_period))
...
class ComponentBandwidth {
 public:
  ComponentBandwidth(float bw_in_bits_per_ns)
      : m_bw_in_bits_per_us(
            bw_in_bits_per_ns *
            TimeConverter<float>::UStoNS(
                1))  // bits_per_ns * num_ns_per_us = bits_per_us
  {}
  ...
  friend inline std::ostream &operator<<(std::ostream &os,
                                         const ComponentBandwidth &time);

 private:
  uint64_t m_bw_in_bits_per_us;

  // Default constructor not supposed to be used. If we ever get rid of GCC 4.3,
  // make this '= delete' again.
  ComponentBandwidth() {}
};

inline std::ostream &operator<<(std::ostream &os,
                                const ComponentBandwidth &bandwidth) {
...
class ComponentBandwidthPerCycle {
 public:
  ...
  friend inline std::ostream &operator<<(
      std::ostream &os, const ComponentBandwidthPerCycle &time);
...
inline std::ostream &operator<<(std::ostream &os,
                                const ComponentBandwidthPerCycle &bandwidth) {
#ifdef SUBSECOND_TIME_SIMPLE_OSTREAM
  return (os << bandwidth.m_bw_in_bits_per_cycle);
#else
  return (os << "CBPC:" << bandwidth.m_bw_in_bits_per_cycle << "@"
             << bandwidth.m_period);
#endif
}

// This class is used in components that have fixed latencies that can be
// consumed by other components One needs to generate a delay that corresponds
// to the current frequency (period) of the connected
//  component
...
inline std::ostream &operator<<(std::ostream &os,
                                const ComponentLatency &latency) {
#ifdef SUBSECOND_TIME_SIMPLE_OSTREAM
  return (os << latency.m_fixed_cycle_latency);
#else
  return (os << "CL:" << latency.m_fixed_cycle_latency << "@"
             << latency.m_period);
#endif
}

// This class is used in components like the CPU and Shared Memory subsystems
// where both time and
//  numbers of cycles in latency need to be added to the core time
// Additionally, through getLatencyGenerator(), a temporary cost variable can be
// created that can
//  eventually be re-merged into this one.  This is helpful if you would like to
//  add cycle latencies but aren't sure if you'd like to commit them directly to
//  this component yet.
class ComponentTime {
 public:
  ...
  // This is for adding time that is not necessarily measured by the base
  // component frequency
  void addLatency(SubsecondTime time_to_add) { m_time += time_to_add; }
  void addLatency(const ComponentTime &time_to_add) {
    assert(time_to_add.m_period == this->m_period);
    m_time += time_to_add.m_time;
  }
  // We can skip the function names and use operators because this is the
  // default and expected case.  The time scale bases should be verified to be
  // the same From http://www.stackoverflow.com/questions/1751869
  ...
  void reset() { m_time = SubsecondTime(); }

  ComponentTime getLatencyGenerator(void) const {
    return ComponentTime(m_period);
  }
  SubsecondTime getElapsedTime(void) const { return m_time; }
  ...
  void setElapsedTime(SubsecondTime new_time) { m_time = new_time; }
