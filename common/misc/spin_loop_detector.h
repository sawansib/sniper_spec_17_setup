...
class SpinLoopDetector {
 private:
  ...
  struct SdtEntry {
    SdtEntry(uint64_t _eip, uint8_t _id, uint64_t _icount,
             SubsecondTime _tcount)
        : eip(_eip), id(_id), icount(_icount), tcount(_tcount) {}
    uint64_t eip;
    uint8_t id;
    uint64_t icount;
 ...
 public:
  SpinLoopDetector(Thread *thread)
      : m_thread(thread), m_rub(), m_sdt(), m_sdt_bitmask(0), m_sdt_nextid(0) {}
