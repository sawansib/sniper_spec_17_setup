...
void BottleGraphManager::threadStart(thread_id_t thread_id) {
  ...
  registerStatsMetric("thread", thread_id, "bottle_contrib_time",
                      &m_contrib[thread_id]);
  registerStatsMetric("thread", thread_id, "bottle_runtime_time",
                      &m_runtime[thread_id]);
}

void BottleGraphManager::update(SubsecondTime time, thread_id_t thread_id,
                                bool running) {
  if (time > m_time_last) {
    UInt64 n_running = 0;
    for (thread_id_t _thread_id = 0;
         _thread_id < (thread_id_t)Sim()->getThreadManager()->getNumThreads();
         ++_thread_id)
      if (m_running[_thread_id]) ++n_running;
  ...
  if (thread_id != INVALID_THREAD_ID) m_running[thread_id] = running;
}
