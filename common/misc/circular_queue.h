  ...
  class iterator : public std::iterator<std::forward_iterator_tag, T,
   ...
   public:
    ...
    bool operator==(iterator const& rhs) const {
      return &_queue == &rhs._queue && _idx == rhs._idx;
    }
...
CircularQueue<T>::CircularQueue(UInt32 size)
    // Since we use head == tail as the empty condition instead of an extra
    // empty flag, we can hold at most m_size-1 elements
    : m_size(size + 1), m_first(0), m_last(0), m_queue(new T[m_size]) {}

// Copy only the size parameter from the other CircularQueue
template <class T>
CircularQueue<T>::CircularQueue(const CircularQueue& queue)
    : m_size(queue.m_size), m_first(0), m_last(0), m_queue(new T[m_size]) {
  assert(queue.size() == 0);
}
...
template <class T>
void CircularQueue<T>::pushCircular(const T& t) {
  if (full()) pop();
  push(t);
}

template <class T>
T& CircularQueue<T>::next(void) {
  assert(!full());
  T& t = m_queue[m_first];
  m_first = (m_first + 1) % m_size;
  return t;
...
template <class T>
UInt32 CircularQueue<T>::size() const {
