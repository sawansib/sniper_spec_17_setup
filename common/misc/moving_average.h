...
class MovingAverage {
 public:
  ...
  static MovingAverage<T>* createAvgType(AvgType_t avg_type,
...
class MovingArithmeticMean : public MovingAverage<T> {
 ...
 public:
  MovingArithmeticMean(UInt32 max_window_size)
      : MovingAverage<T>(max_window_size), sum(SubsecondTime::Zero()) {}

  void update(T next_num) {
    UInt32 curr_window_size =
        (this->m_curr_window_back - this->m_curr_window_front).getValue();
  ...
  T compute() const {
    UInt32 curr_window_size =
        (this->m_curr_window_back - this->m_curr_window_front)
            .getValue();  // recompute
...
class MovingGeometricMean : public MovingAverage<T> {
 public:
  ...
  void update(T next_num) { this->addToWindow(next_num); }
  ...
  T compute() const {
    ...
    SubsecondTime ret;
    ret.setInternalDataForced(
        pow((double)geometric_mean.getInternalDataForced(),
            (double)1.0 / curr_window_size));
    return ret;
  }
};
...
class MovingMedian : public MovingAverage<T> {
 public:
  MovingMedian(UInt32 max_window_size) : MovingAverage<T>(max_window_size) {}
  ...
  T compute() const {
    UInt32 curr_window_size =
        (this->m_curr_window_back - this->m_curr_window_front).getValue();
    UInt32 median_index =
        (this->m_curr_window_front + (curr_window_size / 2)).getValue();
    return this->m_num_list[median_index];
  }
};

template <class T>
MovingAverage<T>* MovingAverage<T>::createAvgType(AvgType_t avg_type,
                                                  UInt32 max_window_size) {
  switch (avg_type) {
