...
// On simulation end or abort, the last 1M entries are formatted and written to
// sim.clog. This means the arguments must remain valid until simulation end,
// which is fine for integers and const char*. Pointers into dynamically
// allocated memory will be invalid by the time they are dereferenced and should
// thus not be used.
...
class CircularLog {
 public:
  ...
  void insert(const char* type, const char* msg, ...)
