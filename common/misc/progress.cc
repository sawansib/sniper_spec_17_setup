...
Progress::Progress()
    : m_enabled(false), m_t_last(0), m_manual(false), m_manual_value(0.f) {
...
void Progress::record(UInt64 simtime) {
  if (m_t_last + m_interval < time(NULL)) {
    ...
    // Always return global instruction count, so MIPS number as reported by job
    ...
    if (m_manual && m_manual_value > 0) {
      ...
      // Re-compute expected completion based on file-pointer based % to
      // completion, and the progress value (instruction count) we'll return
      if (_progress > 1)
    ...
    rewind(m_fp);
    fprintf(m_fp, "%u %" PRId64 " %" PRId64, unsigned(time(NULL)), progress,
            expect);
    fflush(m_fp);
  }
}
