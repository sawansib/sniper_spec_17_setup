...
/* Lock Implementation and Creator class (allows ::create to be overridden from
 * inside /pin) */
...
class TLock : public BaseLock {
 public:
  ...
  void release_read() { _lock->release_read(); }
...
class ScopedLock {
 ...
 public:
  ScopedLock(BaseLock& lock) : _lock(lock) { _lock.acquire(); }
...
class ScopedReadLock {
 ...
 public:
  ScopedReadLock(BaseLock& lock) : _lock(lock) { _lock.acquire_read(); }

  ~ScopedReadLock() { _lock.release_read(); }
};
