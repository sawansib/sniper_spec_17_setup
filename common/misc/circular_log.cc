...
void CircularLog::insert(const char* type, const char* msg, ...) {
  ...
  va_list args;
  va_start(args, msg);
  for (int i = 0; i < 6; ++i) m_buffer[position].args[i] = va_arg(args, UInt64);
  va_end(args);
}
...
void CircularLog::writeLog() {
  ...
  if (head != m_eventnum) {
    ...
    for (UInt64 idx = head; idx < BUFFER_SIZE; ++idx) writeEntry(fp, idx);
  }
  for (UInt64 idx = 0; idx < head; ++idx) writeEntry(fp, idx);
...
void CircularLog::writeEntry(FILE* fp, int idx) {
  ...
  fprintf(fp, e.msg, e.args[0], e.args[1], e.args[2], e.args[3], e.args[4],
          e.args[5]);
  fprintf(fp, "\n");
}
