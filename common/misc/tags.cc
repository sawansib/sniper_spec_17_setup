...
// We currently do not support assigning tag values via the configuration
// variables,
...
TagsManager::TagsManager(config::Config *config) {
  ...
  for (config::SectionList::const_iterator obj = objs.begin();
       obj != objs.end(); ++obj) {
    ...
    for (config::KeyArrayList::const_iterator tag_keys = tags_keys.begin();
         tag_keys != tags_keys.end(); ++tag_keys) {
      ...
      for (unsigned int id = 0; id < ids.size(); id++) {
        ...
        if (valid) addTag(objname, id, tag, valid);
      }
    }
  }
}
