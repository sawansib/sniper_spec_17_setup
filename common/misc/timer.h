...
class Timer {
 ...
 private:
  ...
  UInt64 r_start;
  UInt32 cpu_start;
  // Default FixedPoint uses a 16k multiplier, this overflows with long
  // simulation times. Now, we can simulate at least 3 months (0x100 * rdtsc() <
  // INT64_MAX) with sufficient precision
...
class TotalTimer {
 public:
  ...
  void add(UInt64 t, bool switched = false) {
    ...
    if (t > max) max = t;
    if (switched) __sync_fetch_and_add(&n_switched, 1);
  }
  void report(FILE* fp);
...
class ScopedTimer {
 ...
 public:
  ScopedTimer(TotalTimer& _total) : total(_total) {}
