...
class TypedAllocator : public Allocator {
 private:
  UInt64 m_items;
  FSBAllocator_ElemAllocator<sizeof(DataElement) + sizeof(T), MaxItems, T>
      m_alloc;

  // In ROB-SMT, DynamicMicroOps are allocated by their own thread but free'd in
  // simulate() which can be called by anyone
  Lock m_lock;

 public:
  TypedAllocator() : m_items(0) {}
