...
ClockSkewMinimizationObject::Scheme
    Config::m_knob_clock_skew_minimization_scheme;
...
// Don't call LOG_* inside Config::Config, as the logging infrastructure isn't
...
Config::Config(SimulationMode mode) {
  ...
  if (m_knob_num_host_cores == 0) {
    ...
    if (res == 0) {
      for (UInt32 cpu = 0; cpu < _SC_NPROCESSORS_ONLN; ++cpu)
        if (CPU_ISSET(cpu, &mask)) ++m_knob_num_host_cores;
    } else
      m_knob_num_host_cores = sysconf(_SC_NPROCESSORS_ONLN);
  }

  m_knob_enable_smc_support =
      Sim()->getCfg()->getBool("general/enable_smc_support");
  m_knob_enable_icache_modeling =
      Sim()->getCfg()->getBool("general/enable_icache_modeling");
  m_knob_issue_memops_at_functional =
      Sim()->getCfg()->getBool("general/issue_memops_at_functional");
  ...
  m_knob_enable_progress_trace =
      Sim()->getCfg()->getBool("progress_trace/enabled");
  m_knob_enable_sync =
      Sim()->getCfg()->getString("clock_skew_minimization/scheme") != "none";
  m_knob_enable_sync_report =
      Sim()->getCfg()->getBool("clock_skew_minimization/report");

  m_simulation_mode = mode;
  m_knob_bbvs = false;  // No config setting here, but enabled by code
                        // (BBVSamplingProvider, [py|lua]_bbv) that needs it
  m_knob_lightcache = Sim()->getCfg()->getInt("core/light_cache/num");

  // OS Emulation flags
  m_knob_osemu_pthread_replace =
      Sim()->getCfg()->getBool("osemu/pthread_replace");
  ...
  // HOOK_PERIODIC_INS
  m_knob_hpi_percore =
      Sim()->getCfg()->getInt("core/hook_periodic_ins/ins_per_core");
  m_knob_hpi_global =
      Sim()->getCfg()->getInt("core/hook_periodic_ins/ins_global");

  m_knob_enable_spinloopdetection =
      Sim()->getCfg()->getBool("core/spin_loop_detection");
  ...
  m_knob_enable_syscall_emulation =
      ...
      Sim()->getCfg()->getBool("general/enable_syscall_emulation");

  m_knob_clock_skew_minimization_scheme =
      ClockSkewMinimizationObject::parseScheme(
...
UInt32 Config::getTotalCores() { return m_total_cores; }

UInt32 Config::getApplicationCores() { return getTotalCores(); }
...
void Config::loadFromFile(char *filename) { return; }
...
void Config::loadFromCmdLine() { return; }

String Config::getOutputDirectory() const { return m_knob_output_directory; }
...
void Config::getNetworkModels(UInt32 *models) const {
  try {
    ...
    models[STATIC_NETWORK_MEMORY_1] = NetworkModel::parseNetworkType(
        cfg->getString("network/memory_model_1"));
    models[STATIC_NETWORK_SYSTEM] =
        NetworkModel::parseNetworkType(cfg->getString("network/system_model"));
  } catch (...) {
...
UInt32 Config::getNearestAcceptableCoreCount(UInt32 core_count) {
  ...
  try {
    config::Config *cfg = Sim()->getCfg();
    l_models[STATIC_NETWORK_MEMORY_1] = NetworkModel::parseNetworkType(
        cfg->getString("network/memory_model_1"));
    l_models[STATIC_NETWORK_SYSTEM] =
        NetworkModel::parseNetworkType(cfg->getString("network/system_model"));
  } catch (...) {
    config::Error("Exception while reading network model types.");
  }
  ...
  for (UInt32 i = 0; i < NUM_STATIC_NETWORKS; i++) {
    std::pair<bool, SInt32> core_count_constraints =
        NetworkModel::computeCoreCountConstraints(l_models[i],
                                                  (SInt32)core_count);
    if (core_count_constraints.first) {
      // Network Model has core count constraints
      if ((nearest_acceptable_core_count != 0) &&
          (core_count_constraints.second !=
           (SInt32)nearest_acceptable_core_count)) {
        config::Error(
            "Problem using the network models specified in the configuration "
            "file.");
      } else {
