...
class AllocItem {
 ...
 public:
  ...
  static unsigned int key() {
    ...
    unsigned int k = 0;
    for (int i = 2; i < n; ++i) k += (unsigned long)buffer[i] * (i + (1 << i));
    return k % NUM_ITEMS;
  }
  void init() {
    ScopedLock sl(lock);
  ...
  void free(size_t _size) { __sync_fetch_and_sub(&size, _size); }
  void report(FILE* fp) {
    for (unsigned int i = 3; i < BACKTRACE_SIZE; ++i)
      fprintf(fp, "%lu ",
              i < backtrace_n ? (unsigned long)backtrace_buffer[i] : 0);
...
void logmem_write_allocations() {
  printf("[LOGMEM] Total memory consumption increase: %.1f MiB\n",
         mem_total / (1024. * 1024));
  FILE* fp = fopen(
      Sim()->getConfig()->formatOutputFileName("allocations.out").c_str(), "w");
  fprintf(fp, "%lu\n", (unsigned long)rdtsc);
  for (int i = 0; i < NUM_ITEMS; ++i)
    if (allocated[i].size > 0) allocated[i].report(fp);
  fclose(fp);
}
...
void logmem_enable(bool enabled) {
  if (!logmem_enabled) {
    ...
    // if -fomit-frame-pointer was used, we should notice this here rather than
    // working with garbage keys
    ...
    for (unsigned int i = 1; i < backtrace_n; ++i) {
      LOG_ASSERT_ERROR(
          i < callstack_n && backtrace_buffer[i] == callstack_buffer[i],
          "Fast backtrace() not working");
    }

    Sim()->getHooksManager()->registerHook(HookType::HOOK_SIGUSR1,
                                           logmem_trigger, 0);
  }
