...
// Define to handle processor switches using cpuid and a clock_gettime()
// fallback when a switch is detected. By default, only the faster rdtsc is
// used, but this can give errors when a thread switches cores while timing.
...
/* Don't use a std::vector or anything fancy here, as this may be initialized
   *after* one of the timers, removing them from the list */
...
TotalTimer::TotalTimer(String _name, UInt32 _ignore)
    : name(_name),
      ...
      max(0),
      n_switched(0),
      backtrace_ignore(_ignore) {
  if (numtimers < MAX_TIMERS) alltimers[numtimers++] = this;
...
void TotalTimer::report(FILE* fp) {
  // printf("[TIMER] <%-20s> %6.1f s, %lu calls, avg %.0f us/call, max %.0f us,
  // switched = %.3f%% (%lu) ",
  //    name.c_str(), total / 1e9, n, total / (n ? n : 1) / 1e3, max / 1e3, 100.
  //    * n_switched / n, n_switched);
  fprintf(fp, "%" PRIu64 " %" PRIu64 " %" PRIu64 " %" PRIu64, total, n, max,
          n_switched);
  for (unsigned long i = backtrace_ignore; i < backtrace_ignore + 6; ++i)
    fprintf(fp, " %" PRIuPTR,
            i < backtrace_n ? (intptr_t)backtrace_buffer[i] : 0);
  fprintf(fp, " %s\n", name.c_str());
}

void TotalTimer::reports(void) {
  if (numtimers > 0) {
    char* timersfile = strdup(
        Sim()->getConfig()->formatOutputFileName("sim_timers.out").c_str());
    FILE* fp = fopen(timersfile, "w");
    fprintf(fp, "%" PRIuPTR "\n", (intptr_t)rdtsc);
    for (int i = 0; i < numtimers; ++i) alltimers[i]->report(fp);
    fclose(fp);
  }
}
...
static UInt64 getHashByStacktrace(void) {
  ...
  UInt64 hash = 1861;
  for (int i = 0; i < n; ++i)
    hash = ((hash * __UINT64_C(1994945959)) + (UInt64)buffer[i]) %
           __UINT64_C(49492920901);
  return hash;
}

/* Initialize manually, as some objects that are also static use us but may be
 * initialized earlier. */
static std::map<UInt64, TotalTimer*>* totaltimershash = NULL;

TotalTimer* TotalTimer::getTimerByStacktrace(String name) {
  if (!totaltimershash) totaltimershash = new std::map<UInt64, TotalTimer*>();
