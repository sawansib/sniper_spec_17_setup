...
class istreambuf : public std::streambuf {
 public:
  istreambuf(std::streambuf *_sbuf_p,
             std::streamsize _buff_size = default_buff_size,
    ...
    if (this->gptr() == this->egptr()) {
      ...
      do {
        ...
        if (auto_detect and not auto_detect_run) {
          ...
          is_text = not(in_buff_start + 2 <= in_buff_end and
                        ((b0 == 0x1F and b1 == 0x8B)     // gzip header
                         or (b0 == 0x78 and (b1 == 0x01  // zlib header
                                             or b1 == 0x9C or b1 == 0xDA))));
        }
        if (is_text) {
 ...
 public:
  ...
  int deflate_loop(int flush) {
    while (true) {
      ...
      if (ret != Z_OK and ret != Z_STREAM_END and ret != Z_BUF_ERROR)
        throw Exception(zstrm_p, ret);
