...
template <class T>
void UnstructuredBuffer::put(const T* data, int num) {
  assert(num >= 0);
  m_chars.append((const char*)data, num * sizeof(T));
}

template <class T>
bool UnstructuredBuffer::get(T* data, int num) {
  assert(num >= 0);
  if (m_chars.size() < (num * sizeof(T))) return false;
...
template <class T>
void UnstructuredBuffer::put(const T& data) {
  put<T>(&data, 1);
}

template <>
inline void UnstructuredBuffer::put(const SubsecondTime& _data) {
...
template <class T>
bool UnstructuredBuffer::get(T& data) {
  return get<T>(&data, 1);
}

template <>
inline bool UnstructuredBuffer::get(SubsecondTime& _data) {
  subsecond_time_t data;
...
template <>
inline UnstructuredBuffer& UnstructuredBuffer::operator<<(
    const SubsecondTime& _data) {
...
UnstructuredBuffer& UnstructuredBuffer::operator<<(std::pair<T*, I> buffer) {
  return (*this) << std::make_pair((const void*)buffer.first,
                                   (int)buffer.second);
}
