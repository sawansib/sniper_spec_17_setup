...
void BitVector::reset() {
  for (UInt32 i = 0; i < VECTOR_SIZE; i++) m_words[i] = 0;
...
SInt32 BitVector::find() {
  ...
  // if we get here, there is no set bit in bitVector (after the last_pos that
  // is)
  m_last_pos = -1;
  return -1;
}
...
void BitVector::set(const BitVector& vec2) {
  ...
  for (UInt32 i = 0; i < VECTOR_SIZE; i++) words[i] |= vec2.m_words[i];
}
...
bool BitVector::test(const BitVector& vec2) {
  ...
  for (UInt32 i = 0; i < VECTOR_SIZE; i++) {
    if (vec2.m_words[i] & m_words[i]) return true;
  }
...
void BitVector::debug() {
  ...
  vec2.set(66);
  cout << "test( (1<<66 | 1<<68), (1<<66) | (1<<44) ) -> " << test(vec2)
       << endl;

  clear(vec2);
  cout << "test( (1<<68), (1<<66) | (1<<44) ) -> " << test(vec2) << endl;
