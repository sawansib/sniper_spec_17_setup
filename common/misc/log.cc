...
// When debugging, it helps to be able to attach to the thread you would like to
// investigate directly, instead of running the program from the beginning in
// GDB.
...
Log::~Log() {
  ...
  for (core_id_t i = 0; i < _coreCount; i++) {
    if (_coreFiles[i]) fclose(_coreFiles[i]);
    if (_simFiles[i]) fclose(_simFiles[i]);
  }
  ...
  if (_systemFile) fclose(_systemFile);
...
bool Log::isEnabled(const char *module) {
  ...
  return _anyLoggingEnabled &&
         (_enabledModules.find(module) != _enabledModules.end() ||
          (_loggingEnabled &&
           _disabledModules.find(module) == _disabledModules.end()));
}
...
void Log::getFile(core_id_t core_id, bool sim_thread, FILE **file,
                  Lock **lock) {
...
String Log::getModule(const char *filename) {
  ...
  if (ptr != NULL) filename = ptr + 1;

  for (UInt32 i = 0; i < MODULE_LENGTH && filename[i] != '\0'; i++)
    mod.push_back(filename[i]);

  while (mod.length() < MODULE_LENGTH) mod.push_back(' ');
...
void Log::log(ErrorState err, const char *source_file, SInt32 source_line,
              const char *format, ...) {
  core_id_t core_id;
  bool sim_thread;
  ...
  // This is ugly, but it just prints the time stamp, core number, source
  // file/line
  if (core_id != INVALID_CORE_ID)  // valid core id
    p += sprintf(p, "%-10llu [%5d]  [%2i]%s[%s:%4d]  ",
                 (long long unsigned int)getTimestamp(), tid, core_id,
                 (sim_thread ? "* " : "  "), source_file, source_line);
  else  // who knows
    p += sprintf(p, "%-10llu [%5d]  [  ]  [%s:%4d]  ",
                 (long long unsigned int)getTimestamp(), tid, source_file,
                 source_line);
