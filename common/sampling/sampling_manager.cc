...
SamplingManager::SamplingManager(void)
    : m_sampling_enabled(Sim()->getCfg()->getBool("sampling/enabled")),
      ...
      m_time_total(Sim()->getConfig()->getApplicationCores(),
                   SubsecondTime::Zero()),
      m_time_nonidle(Sim()->getConfig()->getApplicationCores(),
                     SubsecondTime::Zero()) {
  if (!m_sampling_enabled) return;

  m_uncoordinated = Sim()->getCfg()->getBool("sampling/uncoordinated");

  LOG_ASSERT_ERROR(Sim()->getConfig()->getSimulationMode() == Config::PINTOOL,
                   "Sampling is only supported in Pin mode");

  Sim()->getHooksManager()->registerHook(
      HookType::HOOK_INSTR_COUNT,
      (HooksManager::HookCallbackFunc)SamplingManager::hook_instr_count,
      (UInt64)this);
  Sim()->getHooksManager()->registerHook(
      HookType::HOOK_PERIODIC,
      (HooksManager::HookCallbackFunc)SamplingManager::hook_periodic,
      (UInt64)this);
...
SamplingManager::~SamplingManager(void) {
  if (m_sampling_provider) delete m_sampling_provider;
  if (m_sampling_algorithm) delete m_sampling_algorithm;
}
...
void SamplingManager::disableFastForward() {
  ...
  for (unsigned int core_id = 0;
    ...
    if (Sim()->getThreadManager()->isThreadRunning(core_id)) {
      barrier_next =
          std::max(barrier_next, core->getPerformanceModel()->getElapsedTime());
      core_last =
          std::min(core_last, core->getPerformanceModel()->getElapsedTime());
    }
  }
  if (m_uncoordinated) {
    for (UInt32 core_id = 0;
         core_id < Sim()->getConfig()->getApplicationCores(); core_id++) {
      // In uncoordinated mode, some cores will be behind because they didn't
      // reach their target instruction count. Reset all their times to the same
      // value so they won't have to catch up in detailed mode.
      PerformanceModel *perf = Sim()
                                   ->getCoreManager()
                                   ->getCoreFromID(core_id)
                                   ->getPerformanceModel();
      perf->getFastforwardPerformanceModel()->incrementElapsedTime(
          barrier_next - perf->getElapsedTime());
    }
  }
  // Tell the barrier to go to detailed mode again, but make sure it skips over
  // the fastforwarded section of time.
  // FIXME:
  // We restart the barrier at the maximum time, and have all threads make
  // uncoordinated progress towards the fastest thread. There won't be any
  // HOOK_PERIODIC calls (which may not make much sense anyway, as during this
  // period at least one thread is way ahead and won't be running).
  // Alternatively, we could reset the barrier to the min time of all cores,
  // this way, all cores that are behind the fastest one, will start to make
  // detailed progress again in lockstep. We'll also get HOOK_PERIODIC calls
  // during this time.
  if (Sim()->getClockSkewMinimizationServer())
    Sim()->getClockSkewMinimizationServer()->setFastForward(false,
                                                            barrier_next);
  this->setInstrumentationMode(InstMode::DETAILED);
}

void SamplingManager::enableFastForward(SubsecondTime until, bool warmup,
                                        bool detailed_sync) {
  m_fastforward = true;
  m_warmup = warmup;
  ...
  for (unsigned int core_id = 0;
    ...
    PerformanceModel *perf =
        Sim()->getCoreManager()->getCoreFromID(core_id)->getPerformanceModel();
  ...
  // Set barrier to fastforward, and update next_barrier_time to the maximum of
  // all core times so we definitely release everyone
  if (Sim()->getClockSkewMinimizationServer())
...
SubsecondTime SamplingManager::getCoreHistoricCPI(
    Core *core, bool non_idle, SubsecondTime min_nonidle_time) const {
  UInt64 d_instrs = 0;
  SubsecondTime d_time = SubsecondTime::Zero();

  d_instrs = core->getPerformanceModel()->getInstructionCount() -
             m_instructions[core->getId()];
  if (non_idle)
    d_time = core->getPerformanceModel()->getNonIdleElapsedTime() -
             m_time_nonidle[core->getId()];
  else
    d_time = core->getPerformanceModel()->getElapsedTime() -
             m_time_total[core->getId()];
...
void SamplingManager::resetCoreHistoricCPIs() {
  for (UInt32 core_id = 0; core_id < Sim()->getConfig()->getApplicationCores();
       core_id++) {
    Core *core = Sim()->getCoreManager()->getCoreFromID(core_id);
    m_instructions[core->getId()] =
    ...
    m_time_nonidle[core->getId()] =
        core->getPerformanceModel()->getNonIdleElapsedTime();
  }
}
...
void SamplingManager::recalibrateInstructionsCallback(core_id_t core_id) {
  ...
  if (now > m_target_ffend) {
    ...
    SubsecondTime cpi = core->getPerformanceModel()
...
void SamplingManager::instr_count(core_id_t core_id) {
  if (m_fastforward && Sim()->getMagicServer()->inROI()) {
    if (m_uncoordinated) {
      for (UInt32 core_id = 0;
           core_id < Sim()->getConfig()->getApplicationCores(); core_id++) {
        // In uncoordinated mode, the first processor to reach his target
        // instruction count ends fast-forward mode for everyone
        Sim()
            ->getCoreManager()
            ->getCoreFromID(core_id)
            ->setInstructionsCallback(1);
      }
    }
    Sim()
        ->getCoreManager()
        ...
        ->synchronize(SubsecondTime::Zero(), true);
  }
}
