...
// - an algorithm receives a periodic callback, both while in detailed and in
// fastforward mode
// - during each callback the algorithm can examine system state, and decide to
// switch modes
//   by calling SamplingManager::{enable|disable}FastForward
// - the algorithm should perform the proper setup (configure each core's
// setCurrentCPI)
...
class SamplingAlgorithm {
 ...
 public:
  SamplingAlgorithm(SamplingManager *sampling_manager)
      : m_sampling_manager(sampling_manager) {}
