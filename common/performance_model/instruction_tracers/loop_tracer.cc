...
LoopTracer::LoopTracer(const Core *core)
    : m_core(core),
      m_address_base(
          strtol(Sim()
                     ->getCfg()
                     ->getStringArray("loop_tracer/base_address", core->getId())
                     .c_str(),
                 NULL, 16)),
      m_iter_start(Sim()->getCfg()->getIntArray("loop_tracer/iter_start",
                                                core->getId())),
      m_iter_count(Sim()->getCfg()->getIntArray("loop_tracer/iter_count",
                                                core->getId())),
      m_active(false),
      m_iter_current(-1),
      m_iter_instr(0),
...
LoopTracer::~LoopTracer() {
  uint64_t num_cycles =
      std::min(uint64_t(1000),
               m_cycle_max - m_cycle_min + 1);  // show at most 1000 cycles
  m_disas_max = std::max(
      uint64_t(20),
      m_disas_max);  // make disassembly column at least 20 characters wide
  ...
  printf("                   %s", String(m_disas_max, ' ').c_str());
  for (uint64_t i = 0; i < num_cycles; ++i) printf("%" PRIu64 " ", i % 10);
  printf("\n");
  ...
  for (Instructions::iterator it = m_instructions.begin();
    ...
    printf("[%8" PRIxPTR "] %-*s (%u)    ",
           it->second.instruction->getAddress(), int(m_disas_max),
           it->second.instruction->getDisassembly().c_str(),
           it->second.uop_num);
    std::vector<char> line(num_cycles, ' ');
    for (std::map<uint64_t, int64_t>::iterator jt = it->second.issued.begin();
         jt != it->second.issued.end(); ++jt)
      if (jt->second - m_cycle_min < num_cycles && jt->first < 36)
        line[jt->second - m_cycle_min] =
            jt->first < 10 ? '0' + jt->first : 'a' + jt->first - 10;
...
void LoopTracer::traceInstruction(const DynamicMicroOp *uop,
                                  uop_times_t *times) {
  // Ignore dynamic (fake) instructions
  if (!uop->getMicroOp()->getInstruction()) return;

  uint64_t cycle_issue = 0;
  if (times) {
    cycle_issue = SubsecondTime::divideRounded(
        times->issue, m_core->getDvfsDomain()->getPeriod());
  }
  ...
  if (address == m_address_base && uop->isFirst()) {
    ...
    if (m_iter_current >= m_iter_start &&
        m_iter_current < m_iter_start + m_iter_count)
      m_active = true;
    else
      m_active = false;
  ...
  if (m_active) {
    ...
    if (m_instructions.count(opid) == 0) {
      ...
      m_disas_max =
          std::max(m_disas_max, (uint64_t)inst->getDisassembly().length());
    }
    ...
    if (m_iter_instr > 100) m_active = false;
  }
}
