 ...
 public:
  ...
  Operand(const Operand& src)
      : m_type(src.m_type),
        ...
        m_value_name(src.m_value_name),
        m_mem_operand(src.m_mem_operand) {}

  Operand(Type type, Value value = 0, Direction direction = READ,
          const String& value_name = String(), bool mem_operand = false)
      : m_type(type),
        m_value(value),
        m_direction(direction),
        m_value_name(value_name),
        m_mem_operand(mem_operand) {}

  String toString(void) const {
    std::ostringstream o;
    o << "Operand: ";
    if (m_type == REG) o << "REG";
    if (m_type == MEMORY) o << "MEMORY";
    if (m_type == IMMEDIATE) o << "IMMEDIATE";

    o << " ";

    if (m_direction == READ) o << "READ";
    if (m_direction == WRITE) o << "WRITE";
