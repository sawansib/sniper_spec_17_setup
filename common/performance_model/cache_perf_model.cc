...
CachePerfModel::CachePerfModel(const ComponentLatency& cache_data_access_time,
...
CachePerfModel* CachePerfModel::create(
  ...
  switch (perf_model) {
    case (CACHE_PERF_MODEL_PARALLEL):
      return new CachePerfModelParallel(cache_data_access_time,
                                        cache_tags_access_time);

    case (CACHE_PERF_MODEL_SEQUENTIAL):
      return new CachePerfModelSequential(cache_data_access_time,
                                          cache_tags_access_time);

    default:
      LOG_ASSERT_ERROR(false, "Unsupported CachePerfModel type: %s",
                       cache_perf_model_type.c_str());
      return NULL;
  }
}

CachePerfModel::PerfModel_t CachePerfModel::parseModelType(String model_type) {
