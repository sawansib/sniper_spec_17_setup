...
QueueModel* QueueModel::create(String name, UInt32 id, String model_type,
                               SubsecondTime min_processing_time) {
  if (model_type == "basic") {
    bool moving_avg_enabled =
        Sim()->getCfg()->getBool("queue_model/basic/moving_avg_enabled");
    UInt32 moving_avg_window_size =
        Sim()->getCfg()->getInt("queue_model/basic/moving_avg_window_size");
    String moving_avg_type =
        Sim()->getCfg()->getString("queue_model/basic/moving_avg_type");
    return new QueueModelBasic(name, id, moving_avg_enabled,
                               moving_avg_window_size, moving_avg_type);
