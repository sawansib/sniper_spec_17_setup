...
class LoopBranchPredictor {
 public:
  ...
  bool predict(IntPtr ip, IntPtr target) {
    // A IP => predictor is undefined for the LPB because it provides both a hit
    // and prediction (prediction is only valid when there is a hit)
    return false;
  }
  BranchPredictorReturnValue lookup(IntPtr ip, IntPtr target) {
    UInt32 index, tag;
    BranchPredictorReturnValue ret = {
        0, 0, 0, BranchPredictorReturnValue::InvalidBranch};

    gen_index_tag(ip, index, tag);

    for (unsigned int w = 0; w < m_num_ways; ++w) {
      // When we are enabled, and we hit, we can use the value even if the count
      // isn't set to the limit
  ...
  void update(bool predicted, bool actual, IntPtr ip, IntPtr target) {
    ...
    for (UInt32 w = 0; w < m_num_ways; ++w) {
      if (m_ways[w].m_tags[index] == tag) {
        ...
        // First, determine the control logic for when we are moving in the same
        // direction
        //  When we see a change, update the limit and reset the counter
        //  If we are changing our prediction (000001 to 111110), this logic
        //  won't work,
        ...
        if (!match) {
          ...
          // If the new loop length is now shorter, update the limit with the
          // new counter (loop length) Reset the counter because we are starting
          // a new loop
          if (current_counter < current_limit) {
            debug_cout << "[SAME-DIRECTION] Mismatch: Shorter loop found"
                       << std::endl;

            same_direction_counter = 0;
            same_direction_limit = current_counter;
          }
          // Otherwise, the loop length is now longer, increment the counter and
          // the limit Do not reset the counter yet Since this loop is now
          // longer, we should match sometime in the future
          else {
            same_direction_counter = current_counter + 1;
            same_direction_limit = current_counter + 1;
            same_direction_enabled = 0;

            debug_cout << "[SAME-DIRECTION] Mismatch: Longer loop found [C:L] ["
                       << current_counter << ":" << current_limit << "] => ["
                       << same_direction_counter << ":" << same_direction_limit
                       << "]" << std::endl;
          }
        } else {
        ...
        // This happens when we have seen two branches both as either taken or
        // not-taken,
        //  and that this branch status was different than the type predicted.
        //  000001 -> predict() == 0; 111110 -> predict() == 1
        //  Example: For the 000001 case (0), we do not expect to see two 1's in
        //  a row.
        if (previous_actual == actual && actual == !current_prediction) {
          ...
          // We assume that the two predictions seen here are not part of the
          // same branch
          //  structure, but of two seperate branch structures, and therefore
          //  count it as one loop iteration instead of two
          next_counter = 1;
          next_limit = 1;

          // Update the predictor
          //  For the 000001 (0) case, and we've seen two 1's, set the predictor
          //  to (1), ie. 111110
          m_ways[w].m_predictors[index].update(actual);

          // Disable the entry since we have just started to look in another
          // direction
          next_enabled = false;
        } else {
          ...
          if (previous_actual != actual && actual == !current_prediction) {
            if (!current_enabled) {
              ...
              debug_cout << "[NO-CHANGE-DIR] Entry already enabled"
                         << std::endl;
            ...
            // We are continuing along the normal prediction path, simply update
            // with the next state

            debug_cout
                << "[NO-CHANGE-DIR] Updating entry with same direction [C:L] ["
                << current_counter << ":" << current_limit << "] => ["
                << same_direction_counter << ":" << same_direction_limit << "]"
                << std::endl;
          ...
          // Do not update the actual predictor, since we are moving in the same
          // loop direction
        }
  ...
  inline bool prediction_match(UInt32 way, UInt32 index, bool actual) {
    ...
    if (count == limit) {
      ...
      if (prediction != actual) {
        debug_cout << "[PRED-MATCH] Predicted the Loop taken/not-taken limit"
                   << std::endl;
        return true;
      } else
        return false;
