...
bool PentiumMBranchPredictor::predict(IntPtr ip, IntPtr target) {
  BranchPredictorReturnValue global_pred_out =
      m_global_predictor.lookup(ip, target, m_pir);
  ...
  // TODO FIXME: Failed matches against the target address should force a branch
  // or fetch miss

  return result;
}

void PentiumMBranchPredictor::update(bool predicted, bool actual, IntPtr ip,
                                     IntPtr target) {
  updateCounters(predicted, actual);
  m_btb.update(predicted, actual, ip, target);
  m_lpb.update(predicted, actual, ip, target);
  if (!m_last_gp_hit && !m_last_lpb_hit)  // Update bimodal predictor only when
                                          // global and loop predictors missed
    m_bimodal_table.update(predicted, actual, ip, target);
  bool lpb_or_bm_hit =
      m_last_lpb_hit ||
      m_last_bm_pred == actual;  // Global should only allocate when no loop
                                 // predictor hit and bimodal was wrong
  if (m_last_gp_hit) {
    if (predicted != actual &&
        lpb_or_bm_hit)  // Evict from global when mispredict and loop or bimodal
                        // hit
      ...
      m_global_predictor.update(predicted, actual, ip, target, m_pir);
             ...
             (m_last_gp_hit ||
              !lpb_or_bm_hit))  // Update on mispredict, but don't allocate when
                                // loop or bimodal hit
    m_global_predictor.update(predicted, actual, ip, target, m_pir);
  // TODO FIXME: Properly propagate the branch type information from the decoder
  // (IndirectBranch information)
  update_pir(actual, ip, target, BranchPredictorReturnValue::ConditionalBranch);
}

void PentiumMBranchPredictor::update_pir(
    bool actual, IntPtr ip, IntPtr target,
