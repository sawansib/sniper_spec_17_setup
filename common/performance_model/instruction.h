...
enum InstructionType {
  ...
  INST_JMP,
  INST_BRANCH,
  INST_PSEUDO_MISC,  // All instructions above and including this one are
                     // dynamic
  INST_RECV,
  INST_SYNC,
  INST_SPAWN,
  INST_TLB_MISS,
  INST_MEM_ACCESS,  // Not a regular memory access.  These are added latencies
                    // and do not correspond to a particular instruction.
                    // Dynamic Instruction
  INST_DELAY,
  INST_UNKNOWN,
...
__attribute__((unused)) static const char *INSTRUCTION_NAMES[] = {
    ...
    "fadd",     "fsub",         "fmul",  "fdiv",   "jmp",
    "branch",   "dynamic_misc", "recv",  "sync",   "spawn",
    "tlb_miss", "mem_access",   "delay", "unknown"};
...
class Instruction {
 public:
  ...
  bool isPseudo() const { return getType() >= INST_PSEUDO_MISC; }
  bool isIdle() const {
    return getType() == INST_SYNC || getType() == INST_DELAY ||
           getType() == INST_RECV;
  }

  static void initializeStaticInstructionModel();

  const OperandList &getOperands() const { return m_operands; }
  ...
  void setMicroOps(const std::vector<const MicroOp *> *uops) { m_uops = uops; }

  const std::vector<const MicroOp *> *getMicroOps(void) const { return m_uops; }
...
class JmpInstruction : public Instruction {
 public:
  JmpInstruction(OperandList &dest) : Instruction(INST_JMP, dest) {}
};
...
class PseudoInstruction : public Instruction {
 public:
  PseudoInstruction(SubsecondTime cost,
...
class RecvInstruction : public PseudoInstruction {
 public:
  RecvInstruction(SubsecondTime cost) : PseudoInstruction(cost, INST_RECV) {}
};
...
class MemAccessInstruction : public PseudoInstruction {
 ...
 public:
  MemAccessInstruction(SubsecondTime cost, IntPtr address, UInt32 data_size,
                       bool is_fence)
