 ...
 public:
  DramDirectoryPerfModelBase() {
    access_dir_cache_delay =
        Sim()->getCfg()->getInt("perf_model/dram_dir/access_time");
    ...
    // Config::getSingleton()->getShMemSendMessageDelay();
    // shmem_receive_message_delay =
    // Config::getSingleton()->getShMemReceiveMessageDelay();
  }
  ...
  virtual UInt32 getLatency(DramDirActions_t action) { return 0; }
};
