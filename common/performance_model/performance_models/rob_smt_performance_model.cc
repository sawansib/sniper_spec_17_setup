...
std::unordered_map<core_id_t, RobSmtTimer *>
    RobSmtPerformanceModel::s_rob_timers;

RobSmtTimer *RobSmtPerformanceModel::getRobTimer(Core *core,
  ...
  if (s_rob_timers.count(core_id_master) == 0) {
    s_rob_timers[core_id_master] = new RobSmtTimer(
        smt_threads, core, perf, core_model,
        Sim()->getCfg()->getIntArray(
            "perf_model/branch_predictor/mispredict_penalty", core->getId()),
        Sim()->getCfg()->getIntArray(
            "perf_model/core/interval_timer/dispatch_width", core->getId()),
        Sim()->getCfg()->getIntArray(
            "perf_model/core/interval_timer/window_size", core->getId()));
  } else {
    // Only master core_id is used to synchronize, siblings are synchronized by
    // us
    Sim()->getClockSkewMinimizationServer()->setGroup(core->getId(),
                                                      core_id_master);
  }
...
RobSmtPerformanceModel::RobSmtPerformanceModel(Core *core)
    : MicroOpPerformanceModel(
          core, !Sim()->getCfg()->getBoolArray(
                    "perf_model/core/rob_timer/issue_memops_at_issue",
                    core->getId())),
...
boost::tuple<uint64_t, uint64_t> RobSmtPerformanceModel::simulate(
  ...
  return boost::tuple<uint64_t, uint64_t>(
      ins, SubsecondTime::divideRounded(latency, m_elapsed_time.getPeriod()));
}
