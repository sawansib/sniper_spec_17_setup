...
class IntervalTimer {
 public:
  IntervalTimer(Core *core, PerformanceModel *perf, const CoreModel *core_model,
  ...
  boost::tuple<uint64_t, uint64_t> simulate(
      const std::vector<DynamicMicroOp *> &insts);

  // Update internal time after syncronization event
  // Since interval_timer currently has no notion of outside time, no need to do
  // anything for now NOTE: These events are supposed to be long-latency, so we
  // may want to flush the windows here as well
 ...
 protected:
  ...
  uint64_t dispatchInstruction(Windows::WindowEntry &instruction,
                               StopDispatchReason &continueDispatching);
 ...
 private:
  ...
  UInt64 m_lll_dep_mask;  // Memory access dependency granularity for
                          // long-latency loads

  // State of previous cycle
  FixedPoint m_remaining_dispatch_bandwidth;

  // For LOCKed instructions, we need to determine the latency of all loads and
  // stores (even independent ones), as strong ordering requires that we
  // complete them before executing the instruction itself.
