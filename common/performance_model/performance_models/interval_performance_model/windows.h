...
class Windows {
 public:
  struct WindowEntry {
    ...
    /** The cycle in which the instruction was fetched. This clock is not
     * synchronized with the simulator clock. */
    uint64_t fetchTime;
    ...
    /** The latency of the microInstruction can be overlapped by a long latency
     * load. The flag states what is overlapped: a icache miss, a branch
     * mispredict or a dcache miss. */
    uint32_t overlapFlags;

    enum { NO_DEP = 0, DATA_DEP = 1, INDEP_MISS = 2 };
    /** Used during the block window algorithm, shouldn't be here -> has to be
     * int[doubleWindowSize] in IntervalTimer or Windows. */
    uint32_t dependent;
    ...
    const MicroOp* getMicroOp() const {
      return this->getDynMicroOp()->getMicroOp();
    }
    uint64_t getSequenceNumber() const {
      return this->getDynMicroOp()->getSequenceNumber();
    }
  ...
  Windows(int windowSize, bool doFunctionalUnitContention, Core* core,
  ...
  uint64_t getCpContrFraction(CpContrType type,
 ...
 private:
  ...
  uint32_t* const
      m_exec_time_map;  // Used to store the execution time of the producers
                        // when calculating the branch resolution time.
