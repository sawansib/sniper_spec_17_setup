...
IntervalTimer::IntervalTimer(Core* core, PerformanceModel* _perf,
                             const CoreModel* core_model,
                             int misprediction_penalty, int dispatch_width,
                             int window_size,
    ...
    : m_core(core),
      ...
      m_loadstore_contention(
          "interval_timer.loadstore_contention", core->getId(),
          Sim()->getCfg()->getIntArray(
              "perf_model/core/interval_timer/num_outstanding_loadstores",
              core->getId())),
      m_windows(new Windows(window_size, do_functional_unit_contention, core,
                            core_model)),
      m_perf_model(_perf),
      m_frequency_domain(core->getDvfsDomain()) {
  // Granularity of memory dependencies, in bytes
  UInt64 mem_gran = Sim()->getCfg()->getIntArray(
      "perf_model/core/interval_timer/memory_dependency_granularity",
      core->getId());
  LOG_ASSERT_ERROR(
      isPower2(mem_gran),
      "memory_dependency_granularity needs to be a power of 2. [%u]", mem_gran);
  m_mem_dep_mask = ~(mem_gran - 1);

  // Granularity of memory dependencies on long-latency loads, in bytes
  UInt64 lll_gran = Sim()->getCfg()->getIntArray(
      "perf_model/core/interval_timer/lll_dependency_granularity",
      core->getId());
  LOG_ASSERT_ERROR(isPower2(lll_gran),
                   "lll_dependency_granularity needs to be a power of 2. [%u]",
                   lll_gran);
  ...
  registerStatsMetric("interval_timer", core->getId(), "uops_total",
                      &m_uops_total);
  registerStatsMetric("interval_timer", core->getId(), "uops_x87", &m_uops_x87);
  registerStatsMetric("interval_timer", core->getId(), "uops_pause",
                      &m_uops_pause);
  ...
  registerStatsMetric("interval_timer", core->getId(), "numICacheOverlapped",
                      &m_numICacheOverlapped);
  registerStatsMetric("interval_timer", core->getId(), "numBPredOverlapped",
                      &m_numBPredOverlapped);
  registerStatsMetric("interval_timer", core->getId(), "numDCacheOverlapped",
                      &m_numDCacheOverlapped);

  m_numLongLatencyLoads = 0;
  m_numTotalLongLatencyLoadLatency = 0;

  registerStatsMetric("interval_timer", core->getId(), "numLongLatencyLoads",
                      &m_numLongLatencyLoads);
  registerStatsMetric("interval_timer", core->getId(),
                      "numTotalLongLatencyLoadLatency",
                      &m_numTotalLongLatencyLoadLatency);

  m_numSerializationInsns = 0;
  m_totalSerializationLatency = 0;

  registerStatsMetric("interval_timer", core->getId(), "numSerializationInsns",
                      &m_numSerializationInsns);
  registerStatsMetric("interval_timer", core->getId(),
                      "totalSerializationLatency",
                      &m_totalSerializationLatency);

  m_totalHiddenDCacheLatency = 0;
  registerStatsMetric("interval_timer", core->getId(),
                      "totalHiddenDCacheLatency", &m_totalHiddenDCacheLatency);

  m_totalHiddenLongerDCacheLatency = 0;
  m_numHiddenLongerDCacheLatency = 0;
  registerStatsMetric("interval_timer", core->getId(),
                      "totalHiddenLongerDCacheLatency",
                      &m_totalHiddenLongerDCacheLatency);
  registerStatsMetric("interval_timer", core->getId(),
                      "numHiddenLongerDCacheLatency",
  ...
  registerStatsMetric("interval_timer", core->getId(), "numMfenceInsns",
                      &m_numMfenceInsns);
  registerStatsMetric("interval_timer", core->getId(), "totalMfenceLatency",
  ...
  registerStatsMetric("interval_timer", core->getId(), "cpiBranchPredictor",
                      &m_cpiBranchPredictor);
  registerStatsMetric("interval_timer", core->getId(), "cpiSerialization",
                      &m_cpiSerialization);
  registerStatsMetric("interval_timer", core->getId(), "cpiLongLatency",
                      &m_cpiLongLatency);
  ...
  for (int h = HitWhere::WHERE_FIRST; h < HitWhere::NUM_HITWHERES; h++) {
    if (HitWhereIsValid((HitWhere::where_t)h)) {
      ...
      registerStatsMetric("interval_timer", core->getId(), name,
  ...
  for (int i = 0; i < STOP_DISPATCH_SIZE; i++) {
    ...
    String name =
        "detailed-cpiBase-" + StopDispatchReasonString((StopDispatchReason)i);
    registerStatsMetric("interval_timer", core->getId(), name,
                        &(m_cpiBaseStopDispatch[i]));
  }
...
IntervalTimer::~IntervalTimer() { free(); }
...
// Simulate a collection of micro-ops and report the number of instructions
// executed and the latency
boost::tuple<uint64_t, uint64_t> IntervalTimer::simulate(
  ...
  for (std::vector<DynamicMicroOp*>::const_iterator i = insts.begin();
       i != insts.end(); ++i) {
    if ((*i)->isSquashed()) {
      delete *i;
      continue;
    ...
    if (m_uops_total > 10000 && m_uops_x87 > m_uops_total / 20) {
      LOG_PRINT_WARNING_ONCE(
          ...
          "be low. Compile without -mno-sse2 -mno-sse3 to avoid these "
          "instructions.");
    }

    // Only dispatch instructions from the window when it is full
    // It needs to be full so that we can walk the window to find independent
    // misses
  ...
  return boost::tuple<uint64_t, uint64_t>(total_instructions_executed,
                                          total_latency);
}
...
boost::tuple<uint64_t, uint64_t> IntervalTimer::dispatchWindow() {
  ...
  SubsecondTime micro_op_period =
      m_frequency_domain->getPeriod();  // Approximate the current cycle count
                                        // if we don't find a MicroOp

  // Instruction dispatch
  // micro_ops_executed must be < dispatch_rate.  micro_ops_executed <=
  // dispatch_rate will cause significant speedups (see
  // calculateCurrentDispatchRate())
  while ((!m_windows->wIsEmpty()) &&
         (instructions_executed < m_dispatch_width) &&
         (micro_ops_executed < dispatch_rate) &&
         (continue_dispatching == STOP_DISPATCH_NO_REASON)) {
    Windows::WindowEntry& micro_op = m_windows->getInstructionToDispatch();

    uint64_t instruction_latency =
        dispatchInstruction(micro_op, continue_dispatching);
    ...
    m_core->getPerformanceModel()->traceInstruction(micro_op.getDynMicroOp(),
                                                    NULL);
    ...
    if (latency > 16) {
      ...
      const char* opcode_name =
          micro_op.getMicroOp()->getInstructionOpcodeName().c_str();
#else
      const char* opcode_name = "Unknown";
#endif
      fprintf(m_insn_log, "[%ld,%ld] %s latency=%d\n", cycle_count, insn_count,
              opcode_name, instruction_latency);
    }
#endif
  ...
  // Detect the microarchitectural limiting factors for this cycle for cpi
  // accounting
  if (latency == 0) {
    if (m_windows->wIsEmpty()) {
      continue_dispatching = ADD_STOP_DISPATCH_REASON(
          STOP_DISPATCH_WINDOW_EMPTY, continue_dispatching);
    }
    if (instructions_executed >= m_dispatch_width) {
      continue_dispatching = ADD_STOP_DISPATCH_REASON(
          STOP_DISPATCH_DISPATCH_WIDTH, continue_dispatching);
    }
    if (micro_ops_executed >= dispatch_rate) {
      continue_dispatching = ADD_STOP_DISPATCH_REASON(
          STOP_DISPATCH_DISPATCH_RATE, continue_dispatching);
      ...
      //   If the critical path got extended due to issue contention, add that
      //   separately
      int critical_path_length = m_windows->getCriticalPathLength();
      int effective_cp_length =
          m_windows->getEffectiveCriticalPathLength(critical_path_length, true);
      for (unsigned int i = 0; i < (unsigned int)CPCONTR_TYPE_SIZE; ++i)
        m_cpContrByType[i] +=
            m_windows->getCpContrFraction((CpContrType)i, effective_cp_length);
#if 0
         printf("cpContr: ");
         for(unsigned int i = 0; i < (unsigned int)CPCONTR_TYPE_SIZE; ++i)
...
uint32_t IntervalTimer::calculateCurrentDispatchRate() {
  ...
  if (critical_path_length > 0) {
    FixedPoint ipc = FixedPoint(m_windows->getOldWindowLength()) /
                         m_windows->getEffectiveCriticalPathLength(
                             critical_path_length, false) +
                     m_remaining_dispatch_bandwidth;
    dispatch_rate = FixedPoint::floor(ipc);
    m_remaining_dispatch_bandwidth =
        (dispatch_rate < m_dispatch_width) ? (ipc - dispatch_rate) : 0;
  } else {
...
void IntervalTimer::issueMemOp(Windows::WindowEntry& micro_op) {
  ...
  // - not at all if all memory operations are issued at fetch
  // (perf_model/core/interval_timer/issue_memops_at_dispatch == false)
  ...
  if ((micro_op.getMicroOp()->isLoad() || micro_op.getMicroOp()->isStore()) &&
    ...
    MemoryResult res = m_core->accessMemory(
        Core::NONE, micro_op.getMicroOp()->isLoad() ? Core::READ : Core::WRITE,
        micro_op.getDynMicroOp()->getAddress().address, NULL,
        micro_op.getMicroOp()->getMemoryAccessSize(), Core::MEM_MODELED_RETURN,
        micro_op.getMicroOp()->getInstruction()
            ? micro_op.getMicroOp()->getInstruction()->getAddress()
            : static_cast<uint64_t>(NULL));
    uint64_t latency = SubsecondTime::divideRounded(
        res.latency, m_core->getDvfsDomain()->getPeriod());
    micro_op.getDynMicroOp()->setExecLatency(
        micro_op.getDynMicroOp()->getExecLatency() +
...
uint64_t IntervalTimer::dispatchInstruction(
    Windows::WindowEntry& micro_op, StopDispatchReason& continue_dispatching) {
  ...
  bool icache_miss =
      (micro_op.getDynMicroOp()->getICacheHitWhere() != HitWhere::L1I) &
      (!micro_op.hasOverlapFlag(Windows::WindowEntry::ICACHE_OVERLAP));
  ...
  if (icache_miss) {
    ...
    m_cpiInstructionCache[micro_op.getDynMicroOp()->getICacheHitWhere()] +=
        icache_latency * micro_op.getDynMicroOp()->getPeriod();
  }

  if (micro_op.getMicroOp()->isBranch()) {
    if (micro_op.getDynMicroOp()->isBranchMispredicted() &&
        !micro_op.hasOverlapFlag(Windows::WindowEntry::BPRED_OVERLAP)) {
      uint64_t bpred_latency = m_branch_misprediction_penalty +
                               m_windows->calculateBranchResolutionLatency();
      ...
      m_cpiBranchPredictor +=
          bpred_latency * micro_op.getDynMicroOp()->getPeriod();
    }
  }

  if (micro_op.getMicroOp()->isSerializing() ||
      micro_op.getMicroOp()->isInterrupt()) {
    uint64_t flushLatency =
        std::max(m_windows->getCriticalPathLength(),
                 m_windows->getMinimalFlushLatency(m_dispatch_width));
    uint64_t serialize_latency =
        flushLatency + micro_op.getDynMicroOp()->getExecLatency();
    latency += serialize_latency;

    m_numSerializationInsns++;
    m_totalSerializationLatency += serialize_latency;
    m_cpiSerialization +=
        serialize_latency * micro_op.getDynMicroOp()->getPeriod();

    micro_op.setExecTime(m_windows->getCriticalPathTail());
    m_windows->clearOldWindow(micro_op.getExecTime());
  } else if (micro_op.getMicroOp()->isExecute() &&
             micro_op.getMicroOp()->isMemBarrier()) {
    // Handle the M/L/SFENCE operations (when supplied as a single execute
    // operation) now as a strict MFENCE
    // uint64_t current_cycle = m_windows->getCriticalPathTail();
    uint64_t cycle_to_wait_until = std::max(
        max_producer_exec_time,
        std::max(m_max_store_completion_time, m_max_load_completion_time));
    // If we have loads or stores that will complete in the future, wait for
    // them all to complete Otherwise, our critical path is longer, and we
    // become a nop This code below doesn't work.  CriticalPath is just the time
    // that we put the last instruction in. We need to use the maxProd, and we
    // can't just add latencies, because we don't have a concept of latency
    //  in the old-window (only execution ciompletion time)
    // We just need to make sure that the memory dependencies are properly taken
    // care of.
    // if (cycle_to_wait_until > current_cycle) {
    //   uint64_t mfence_latency = cycle_to_wait_until - current_cycle;
    //   // Keep track of the memory fence instructions and their contributed
    //   latencies m_totalMfenceLatency += mfence_latency;
    //}
    m_numMfenceInsns++;
    micro_op.setExecTime(cycle_to_wait_until +
                         micro_op.getDynMicroOp()->getExecLatency());
    ...
    if (micro_op.getMicroOp()->isLoad()) {
      if (micro_op.hasOverlapFlag(Windows::WindowEntry::DCACHE_OVERLAP)) {
        // do nothing for overlapped loads, as they don't add to the critical
        // path and happen in parallel to other loads (MLP > 1)
        ...
        uint64_t dispatch_cycle =
            std::max(max_producer_exec_time, m_windows->getCriticalPathHead());
        ...
        if (micro_op.getMicroOp()->isMemBarrier())
          contention_exec_cycle =
              m_loadstore_contention.getBarrierCompletionTime(sched_cycle,
                                                              dcache_latency);
        else
          contention_exec_cycle = m_loadstore_contention.getCompletionTime(
              sched_cycle, dcache_latency);
        ...
        uint64_t long_latency_load_latency =
            reswin_extra_latency +
            contention_extra_latency;  // dcache_latency is already taken into
                                       // account in contention_extra_latency
        ...
        const char* opcode_name = "Unknown";
#endif
        fprintf(m_insn_log, "[%ld,%ld] %s(", cycle_count, insn_count,
                opcode_name);
        if (micro_op.isLoad()) fprintf(m_insn_log, "L");
        if (micro_op.isExecute()) fprintf(m_insn_log, "X");
        if (micro_op.isStore()) fprintf(m_insn_log, "S");
        fprintf(m_insn_log, ") latency=%ld (%ld+%ld(%ld))\n", latency,
                reswin_extra_latency, contention_extra_latency, dcache_latency);
#endif
        ...
        m_cpiDataCache[micro_op.getDynMicroOp()->getDCacheHitWhere()] +=
            long_latency_load_latency * micro_op.getDynMicroOp()->getPeriod();
      } else {
        ...
        // This logic only works for LFENCEs, when marked on load uops
        if (micro_op.getMicroOp()->isMemBarrier())
          contentionExecTime = m_loadstore_contention.getBarrierCompletionTime(
              sched_cycle, dcache_latency);
        else
          contentionExecTime = m_loadstore_contention.getCompletionTime(
              sched_cycle, dcache_latency);
        micro_op.setExecTime(contentionExecTime);
        /* doesn't block window but adds to the critical path (unless
         * long-latency). */
        updateCriticalPath(micro_op, latency);
      }

      m_max_load_completion_time =
          std::max(m_max_load_completion_time, micro_op.getExecTime());
      ...
      if (micro_op.getDynMicroOp()->isLongLatencyLoad()) {
        ...
        // Ins will be outstanding for until it is done. By accounting
        // beforehand I don't need to worry about fast-forwarding simulations
      ...
      uint64_t dispatch_cycle =
          std::max(max_producer_exec_time, m_windows->getCriticalPathHead());
      uint64_t sched_cycle = dispatch_cycle;
      uint64_t bypass_latency =
          m_core_model->getBypassLatency(micro_op.getDynMicroOp());
      uint64_t data_ready_cycle =
          sched_cycle + bypass_latency +
          1;  // This store result will be ready to use one cycle later
      micro_op.setExecTime(
          data_ready_cycle);  // Time the critical path calculation will use
      updateCriticalPath(micro_op, latency);

      // FIXME See Redmine issue #89: CPU forwarding logic vs. RFO store
      // completion delaying the CPU
      uint64_t exec_time_cycle =
          sched_cycle + store_latency;  // For future instructions that depend
                                        // on this instruction's result
      m_max_store_completion_time =
          std::max(m_max_store_completion_time, exec_time_cycle);

      // FIXME: Don't update micro_op.setExecTime (else it will eventually
      // affect the critical path head, once this instruction goes through
      // Windows::dispatchInstruction). If we need this time, figure out another
      // place to put it.
 ...
 */
void IntervalTimer::updateCriticalPath(Windows::WindowEntry& micro_op,
                                       uint64_t& latency) {
...
/**
 * An microOperation blocks the window: mark the micro-ops that overlap with
 * this micro-op. Start the loads that overlap with this micro-op.
 */
void IntervalTimer::blockWindow() {
  ...
  Windows::WindowEntry& head =
      window_iterator.next();  // Returns the current head: disregard it.
  head.setIndependentMiss();

  IntPtr head_address = head.getMicroOp()->isLoad()
                            ? head.getDynMicroOp()->getLoadAccess().phys
                            : 0;
  ...
  while (window_iterator.hasNext()) {
    ...
    if (micro_op.getMicroOp()->isSerializing()) break;

    if (micro_op.getMicroOp()->isMemBarrier()) mem_barrier_pending = true;

    // Generate dependencies using dependencies
    for (uint32_t i = 0; i < micro_op.getDynMicroOp()->getDependenciesLength();
         i++) {
      if (m_windows->windowContains(
              micro_op.getDynMicroOp()->getDependency(i))) {
        Windows::WindowEntry& dependee = m_windows->getInstruction(
            micro_op.getDynMicroOp()->getDependency(i));
        if (dependee.isDependent()) {
          // Dependee depends on the long-latency load blocking the window: do
          // not issue this uop now
          micro_op.setDataDependent();
          break;
        } else if (dependee.isIndependent() &&
                   dependee.getDynMicroOp()->isLongLatencyLoad()) {
          // Our dependee is independent of the long-latency load blocking the
          // window, but it is a long-latency event by itself: do not issue this
          // uop now Since the window head is independent and long-latency, this
          // code path will also start the chain of dependent loads.
          micro_op.setDataDependent();
          break;
        }
        // else: our dependee is independent of the long-latency load blocking
        // the window, and is not a long-latency load in itself, which means it
        // will complete under the original LLL. Therefore, we can also be
        // hidden under the long-latency load which makes us not dependent.
      }
    }
    ...
    if (micro_op.getMicroOp()->isLoad()) {
      ...
      if ((address & m_lll_dep_mask) == head_address) {
        // This load accesses the same cache line as the long-latency load
        // that's blocking the ROB We should be an overlapped miss, but due to
        // the way the cache model works (instant completion), we will see an L1
        // hit. Still, we can't complete until the miss is resolved. Model this
        // by making us dependent on the long-latency load.
        micro_op.setDataDependent();
      }
    }
    ...
    if (micro_op.getMicroOp()->isBranch() && !micro_op.isDependent()) {
       ...
       * if short latency -> check if delayed hit and also mark as independent
       * (secondary) miss
       */
      if (!mem_barrier_pending) {
        if (!micro_op.hasOverlapFlag(Windows::WindowEntry::DCACHE_OVERLAP)) {
          uint64_t overlappedLatency =
              micro_op.getDynMicroOp()->getExecLatency();
...
uint64_t IntervalTimer::getMaxProducerExecTime(Windows::WindowEntry& micro_op) {
  int64_t oldestStartTime =
      m_windows->getOldestInstruction().getExecTime() -
      m_windows->getOldestInstruction().getDynMicroOp()->getExecLatency();
  ...
  for (uint32_t i = 0; i < micro_op.getDynMicroOp()->getDependenciesLength();
       i++) {
    if (m_windows->oldWindowContains(
            micro_op.getDynMicroOp()->getDependency(i))) {
      Windows::WindowEntry& producer =
          m_windows->getInstruction(micro_op.getDynMicroOp()->getDependency(i));
      int64_t producerExecTime = producer.getExecTime();
      int64_t producerStartTime =
          producerExecTime - producer.getDynMicroOp()->getExecLatency();
      ...
      max_producer_exec_time =
          std::max(max_producer_exec_time, producerExecTime);
    }
  }
