...
Windows::Windows(int window_size, bool doFunctionalUnitContention, Core* core,
...
Windows::~Windows() {
  for (int i = 0; i < m_double_window_size; i++)
    if (m_double_window[i].uop) delete m_double_window[i].uop;
...
bool Windows::wIsFull() const { return m_window_length == m_window_size; }

bool Windows::wIsEmpty() const { return m_window_length == 0; }
...
int Windows::windowIndex(const int index) const {
  return ((index % m_double_window_size) + m_double_window_size) %
...
void Windows::add(DynamicMicroOp* micro_op) {
  ...
  uint64_t lowestValidSequenceNumber =
      getInstructionByIndex(m_old_window_head).getSequenceNumber();
  m_register_dependencies->setDependencies(*micro_op,
...
Windows::WindowEntry& Windows::getInstructionByIndex(int index) const {
  LOG_ASSERT_ERROR(index >= 0 && index < m_double_window_size,
                   "Index is out of bounds");
  return m_double_window[index];
}

Windows::WindowEntry& Windows::getInstruction(uint64_t sequenceNumber) const {
  Windows::WindowEntry& windowHead =
      getInstructionByIndex(m_window_head__old_window_tail);
  ...
  if (ret.getSequenceNumber() == sequenceNumber) {
    ...
    std::cout << "Getting an instruction that is not in the window !!! That's "
                 "it, I'm out of here !"
              << std::endl;
    exit(0);
  }
...
uint64_t Windows::getEffectiveCriticalPathLength(uint64_t critical_path_length,
  ...
  if (!m_do_functional_unit_contention) {
    ...
    return m_interval_contention->getEffectiveCriticalPathLength(
        critical_path_length, update_reason);
  }
}
...
void Windows::dispatchInstruction() {
  if (m_old_window_length == m_window_size) {
    ...
    m_critical_path_head =
        std::max(m_critical_path_head,
                 getInstructionByIndex(m_old_window_head).getExecTime());
  ...
  m_window_head__old_window_tail =
      incrementIndex(m_window_head__old_window_tail);
  m_window_length--;
  m_old_window_length++;
}
...
bool Windows::windowContains(uint64_t sequenceNumber) const {
  uint64_t lowestValid =
      getInstructionByIndex(m_window_head__old_window_tail).getSequenceNumber();
  uint64_t highestValid =
      getInstructionByIndex(decrementIndex(m_window_tail)).getSequenceNumber();
...
bool Windows::oldWindowContains(uint64_t sequenceNumber) const {
  uint64_t lowestValid =
      getInstructionByIndex(m_old_window_head).getSequenceNumber();
  uint64_t highestValid =
      getInstructionByIndex(decrementIndex(m_window_head__old_window_tail))
          .getSequenceNumber();
  return sequenceNumber >= lowestValid && sequenceNumber <= highestValid;
}

int Windows::getOldWindowLength() const { return m_old_window_length; }

uint64_t Windows::getCriticalPathHead() const { return m_critical_path_head; }

uint64_t Windows::getCriticalPathTail() const { return m_critical_path_tail; }

int Windows::getCriticalPathLength() const {
  if (!(m_critical_path_head <= m_critical_path_tail)) {
    LOG_PRINT_WARNING_ONCE(
        "Warning: Windows::getCriticalPathLength() - head:%lu > tail:%lu",
        m_critical_path_head, m_critical_path_tail);
    return 0;
  }
...
uint64_t Windows::longLatencyOperationLatency(WindowEntry& uop) {
  if (uop.getExecTime() < m_critical_path_tail) {
    ...
    return 0;
  } else if (uop.getExecTime() - m_critical_path_tail <
             m_core_model->getLongLatencyCutoff()) {
...
uint64_t Windows::updateCriticalPathTail(WindowEntry& uop) {
  ...
  if (execTime > m_critical_path_tail) {
    if (execTime - m_critical_path_tail >
        m_core_model->getLongLatencyCutoff()) {
      // Extending the critical path by huge amounts isn't healthy (see Redmine
      // #113). Instead, the caller should add the long latency directly (and
      // attribute it) and call clearOldWindow()
      LOG_PRINT_WARNING_ONCE(
          "Warning: Updating the critical path by %lu > %u cycles",
          execTime - m_critical_path_tail,
          m_core_model->getLongLatencyCutoff());
    }
...
uint64_t Windows::getCpContrFraction(CpContrType type,
...
bool Windows::Iterator::hasNext() { return index != stop; }
  ...
  for (uint32_t i = 0; i < micro_op.getDynMicroOp()->getDependenciesLength();
    ...
    if (oldWindowContains(micro_op.getDynMicroOp()->getDependency(i))) {
      Windows::WindowEntry& producer =
          getInstruction(micro_op.getDynMicroOp()->getDependency(i));
      m_exec_time_map[producer.getWindowIndex()] =
          producer.getDynMicroOp()->getExecLatency();
    }
  }

  // Find/mark producers of producers
  for (int i = windowIndex(m_window_head__old_window_tail - 1), j = 0;
       j < m_old_window_length; i = windowIndex(i - 1), j++) {
    if (m_exec_time_map[i]) {
      ...
      for (uint32_t k = 0; k < op.getDynMicroOp()->getDependenciesLength();
           k++) {
        if (oldWindowContains(op.getDynMicroOp()->getDependency(k))) {
          Windows::WindowEntry& producer =
              getInstruction(op.getDynMicroOp()->getDependency(k));
          m_exec_time_map[producer.getWindowIndex()] = std::max(
              (producer.getDynMicroOp()->getExecLatency() + m_exec_time_map[i]),
              m_exec_time_map[producer.getWindowIndex()]);
        }
      }

      br_resolution_latency =
          std::max(br_resolution_latency, m_exec_time_map[i]);
...
String Windows::toString() {
  ...
  out << "|";
  for (int i = 0; i < m_double_window_size; i++) {
    if (i == m_old_window_head) out << "OH";
    if (i == m_window_head__old_window_tail) out << "WH";
    if (i == m_window_tail) out << "WT";
    out << std::setfill(' ') << std::setw(5)
        << getInstructionByIndex(i).getWindowIndex() << "|";
  }
  out << std::endl << "|";
  for (int i = 0; i < m_double_window_size; i++)
    out << std::setfill(' ') << std::setw(7)
        << getInstructionByIndex(i).getSequenceNumber() << "|";
  out << std::endl;
