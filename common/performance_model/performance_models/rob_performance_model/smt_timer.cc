...
SmtTimer::SmtTimer(uint64_t num_threads)
    : m_num_threads(num_threads),
      ...
      enabled(true),
      execute_thread(0) {
  Sim()->getHooksManager()->registerHook(HookType::HOOK_ROI_BEGIN,
                                         SmtTimer::hookRoiBegin, (UInt64)this);
  Sim()->getHooksManager()->registerHook(
      HookType::HOOK_THREAD_START, SmtTimer::hookThreadStart, (UInt64)this);
  Sim()->getHooksManager()->registerHook(
      HookType::HOOK_THREAD_EXIT, SmtTimer::hookThreadExit, (UInt64)this);
  Sim()->getHooksManager()->registerHook(
      HookType::HOOK_THREAD_STALL, SmtTimer::hookThreadStall, (UInt64)this);
  Sim()->getHooksManager()->registerHook(
      HookType::HOOK_THREAD_RESUME, SmtTimer::hookThreadResume, (UInt64)this);
  Sim()->getHooksManager()->registerHook(
      HookType::HOOK_THREAD_MIGRATE, SmtTimer::hookThreadMigrate, (UInt64)this);
}

SmtTimer::~SmtTimer() {
  for (std::vector<SmtThread *>::iterator it = m_threads.begin();
       it != m_threads.end(); ++it)
    delete *it;
}
...
SmtTimer::smtthread_id_t SmtTimer::findSmtThreadFromThread(
  ...
  if (thread_id == INVALID_THREAD_ID) return INVALID_SMTTHREAD_ID;
  for (smtthread_id_t smtthread_id = 0; smtthread_id < m_threads.size();
       ++smtthread_id)
    if (m_threads[smtthread_id]->thread &&
        (m_threads[smtthread_id]->thread->getId() == thread_id))
      return smtthread_id;
  return INVALID_SMTTHREAD_ID;
}

SmtTimer::smtthread_id_t SmtTimer::findSmtThreadFromCore(core_id_t core_id) {
  if (core_id == INVALID_CORE_ID) return INVALID_SMTTHREAD_ID;
  for (smtthread_id_t smtthread_id = 0; smtthread_id < m_threads.size();
       ++smtthread_id)
    if (m_threads[smtthread_id]->core->getId() == core_id) return smtthread_id;
  return INVALID_SMTTHREAD_ID;
}
...
bool SmtTimer::barrierRelease(bool release_all, smtthread_id_t thread_id) {
  ...
  while (num_running == 0) {
    // All threads have enough instructions. We still need to release someone to
    // make forward progress. Find the thread with the (approximately) lowest
    // number of surplus instructions and release it

    for (smtthread_id_t thread_num = 0; thread_num < m_threads.size();
         ++thread_num) {
      SmtThread *thread = m_threads[thread_num];

      if (thread->in_barrier && thread->running &&
          threadNumSurplusInstructions(thread_num) < limit) {
        ++num_running;
...
bool SmtTimer::barrier(smtthread_id_t thread_id) {
  ...
  LOG_ASSERT_ERROR(
      thread != NULL,
      "How can SMT barrier be called when there is not thread running?");

  smtthread->in_barrier = true;
  CLOG("smtbarrier", "Entry core %d thread %d", smtthread->core->getId(),
       thread->getId());
  ...
  CLOG("smtbarrier", "Exit core %d thread %d (execute = %s)",
       smtthread->core->getId(), thread->getId(),
...
void SmtTimer::signalBarrier() {
  if (isBarrierReached()) barrierRelease(false, INVALID_SMTTHREAD_ID);
}
...
void SmtTimer::threadExit(HooksManager::ThreadTime *argument) {
  ...
  // Re-evaluate whether the stalled thread can run (maybe it was waiting for
  // us?) Note: HOOK_THREAD_EXIT happens right at the end, after possible
  // rescheduling. So always call signalBarrier() even though the thread is not
  // currently here, since it may have been before
  signalBarrier();
}
...
void SmtTimer::threadStall(HooksManager::ThreadStall *argument) {
  ...
  // Re-evaluate whether the stalled thread can run (maybe it was waiting for
  // us?) Note: HOOK_THREAD_STALL happens right at the end, after possible
  // rescheduling. So always call signalBarrier() even though the thread is not
  // currently here, since it may have been before
  signalBarrier();
}
...
void SmtTimer::threadResume(HooksManager::ThreadResume *argument) {
  ...
  // When a thread wakes up, it will usually call simulate() before it processes
  // the SYNC_INSTRUCTION that updates their thread->now to the wakeup time. In
  // this intervening period, we should not return any latency from simulate()
  // since that will be huge (this->now has advanced, because the other threads
  // have ran, leaving this thread who was asleep behind; but thread->now is
  // still set to the time the thread went to sleep). Instead, we listen for
  // HookType::HOOK_THREAD_RESUME (called from the MCP, before the thread gets
  // its wakeup message and way before the offending simulate() call) and set
  // thread->in_wakeup, which signals simulate() to not return any latency until
  // RobSmtTimer::synchronize() was called which updates thread->now to the
  // correct wakeup time.
...
void SmtTimer::threadMigrate(HooksManager::ThreadMigrate *argument) {
  ...
  // Same as in wakeup case: thread will send a SpawnInstruction, but we don't
  // want to send huge latencies to simulate() calls that precede it
  ...
  if (smtthread_id != INVALID_SMTTHREAD_ID) {
    ...
    std::cout << "** [" << argument->thread_id << "] threadMigrate"
              << std::endl;
#endif
    m_threads[smtthread_id]->in_wakeup = true;
    m_threads[smtthread_id]->running = true;
    m_threads[smtthread_id]->thread =
        Sim()->getThreadManager()->getThreadFromID(argument->thread_id);
    notifyNumActiveThreadsChange();
  }
}
...
void SmtTimer::simulate(smtthread_id_t thread_id) {
  ...
  if (threadNumSurplusInstructions(thread_id) > 128) {
    if (barrier(thread_id)) {
      ...
      // We executed this cycle, make sure no-one else (potentially released
      // through sync->periodic->reschedule->migrate) executes again.
      execute_thread = INVALID_THREAD_ID;

      ClockSkewMinimizationClient *client =
          thread->core->getClockSkewMinimizationClient();
      if (client) {
        in_sync = true;
        m_lock.release();
        client->synchronize();
...
void SmtTimer::enable() { enabled = true; }
