...
class RobTimer {
 private:
  class RobEntry {
   ...
   public:
    ...
    void addAddressProducer(UInt64 sequenceNumber) {
      addressProducers.push_back(sequenceNumber);
    }
    UInt64 getNumAddressProducers() const { return addressProducers.size(); }
    UInt64 getAddressProducer(size_t idx) const {
      return addressProducers.at(idx);
    }

    DynamicMicroOp* uop;
    SubsecondTime dispatched;
    SubsecondTime ready;  // Once all dependencies are resolved, cycle number
                          // that this uop becomes ready for issue
    SubsecondTime
        readyMax;  // While some but not all dependencies are resolved, keep the
                   // time of the latest known resolving dependency
  ...
  std::deque<unsigned int> deptrace_src_regs;
  std::deque<unsigned int> deptrace_dst_regs;

  bool deptrace_is_call = false;
  bool deptrace_is_ret = false;
  bool deptrace_is_jump = false;
  bool deptrace_is_direct = false;
  ...
  std::unordered_map<uint64_t, uint64_t>
      deptrace_thread_released;  // threadid -> mutex released
  ...
  static SInt64 __deptrace_roi_begin(UInt64 user, UInt64 arg) {
    reinterpret_cast<RobTimer*>(user)->deptrace_roi_begin();
    return 0;
  }
  static SInt64 __deptrace_roi_end(UInt64 user, UInt64 arg) {
    reinterpret_cast<RobTimer*>(user)->deptrace_roi_end();
    return 0;
  }

  static SInt64 __deptrace_thread_create(UInt64 user, UInt64 arg) {
    reinterpret_cast<RobTimer*>(user)->deptrace_thread_create(
        reinterpret_cast<HooksManager::ThreadCreate*>(arg));
    return 0;
  }
 ...
 public:
  RobTimer(Core* core, PerformanceModel* perf, const CoreModel* core_model,
           int misprediction_penalty, int dispatch_width, int window_size);
  ~RobTimer();

  boost::tuple<uint64_t, SubsecondTime> simulate(
