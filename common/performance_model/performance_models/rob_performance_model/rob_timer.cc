...
boost::tuple<uint64_t, SubsecondTime> RobTimer::simulate(const std::vector<DynamicMicroOp*>& insts)
{
    ...
    for (std::vector<DynamicMicroOp*>::const_iterator it = insts.begin(); it != insts.end(); it++) {
         ...
         *  B=conditional branch (direct), C=call direct, c=call indirect, J=jump direct, j=jump indirect, r=return (indirect)
         *  A=fp_addsub, M=fp_mul, D=fp_div, Q=fp_sqrt, []=generic
         *  d=reg dependence, m=mem dependence, a=addr dependence, t=target address, *=taken (only for conditional)
        ...
        if (deptraceIsActive(thread_id)) {
            ...
            for (unsigned int i = 0; i < (*it)->getMicroOp()->getSourceRegistersLength(); i++) {
                deptrace_src_regs.push_back((*it)->getMicroOp()->getSourceRegister(i));
            }
            for (unsigned int i = 0; i < (*it)->getMicroOp()->getDestinationRegistersLength(); i++) {
                deptrace_dst_regs.push_back((*it)->getMicroOp()->getDestinationRegister(i));
            }
            ...
            if ((*it)->isLast() || deptrace_microops) {
                ...
                if (deptrace_is_branch || deptrace_is_call || deptrace_is_ret || deptrace_is_jump) {
                    if (deptrace_last_was_newline)
                        deptrace_last_was_newline = false;
                    else
                        deptrace_f << " ";
                ...
                if (!deptrace_is_branch && !deptrace_is_call && !deptrace_is_ret && !deptrace_is_jump && !deptrace_is_load && !deptrace_is_store) {
                    if (deptrace_last_was_newline)
                        deptrace_last_was_newline = false;
                    else
                        deptrace_f << " ";
