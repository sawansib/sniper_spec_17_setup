...
RobTimer::RobTimer(Core* core, PerformanceModel* _perf,
    ...
    : dispatchWidth(dispatch_width),
      ...
      deptrace(Sim()->getCfg()->getBoolArray(
          "perf_model/core/rob_timer/deptrace", core->getId())),
      deptrace_roi(Sim()->getCfg()->getBoolArray(
          "perf_model/core/rob_timer/deptrace_roi", core->getId())),
      deptrace_microops(Sim()->getCfg()->getBoolArray(
          "perf_model/core/rob_timer/deptrace_microops", core->getId())),
      m_core(core),
      rob(window_size + 255),
      m_num_in_rob(0),
      m_rs_entries_used(0),
  ...
  m_uops_x87 = 0;
  m_uops_pause = 0;
  registerStatsMetric("rob_timer", core->getId(), "insns_total",
                      &m_insns_total);
  ...
  if (m_mlp_histogram) {
    ...
    for (unsigned int i = 0; i < MAX_OUTSTANDING; ++i) {
      ...
      registerStatsMetric("rob_timer", core->getId(), name,
                          &(m_outstandingLoadsAll[i]));
    }
  }

  if (deptrace) {
    _deptrace_f = std::unique_ptr<std::ostream>(new zstr::ofstream(
        (Sim()->getConfig()->formatOutputFileName(
             String("sim.deptrace.") + itostr(core->getId()) + ".gz"))
            .c_str()));
    _deptrace_insn_f = std::unique_ptr<std::ostream>(new zstr::ofstream(
        (Sim()->getConfig()->formatOutputFileName(
             String("sim.deptrace_insn.") + itostr(core->getId()) + ".gz"))
            .c_str()));
    ...
    if (deptrace_roi) {
      ...
      Sim()->getHooksManager()->registerHook(
          HookType::HOOK_ROI_BEGIN, __deptrace_roi_begin, (UInt64)this);
      Sim()->getHooksManager()->registerHook(HookType::HOOK_ROI_END,
                                             __deptrace_roi_end, (UInt64)this);
    } else {
      if (m_core->getId() == 0) {
        ScopedLock sl(m_print_lock);
    ...
    Sim()->getHooksManager()->registerHook(
        HookType::HOOK_THREAD_CREATE, __deptrace_thread_create, (UInt64)this);
  } else {
...
void RobTimer::deptraceRMSSetActive(thread_id_t thread_id, bool is_active) {
  {
    ScopedLock sl(m_print_lock);
    if (is_active) {
      std::cout << "[DEPTRACE:" << m_core->getId() << ":" << thread_id
                << "] RMS: Enabling trace generation after " << m_insns_total
                << " instructions / " << m_uops_total << " micro-ops\n";
    } else {
      std::cout << "[DEPTRACE:" << m_core->getId() << ":" << thread_id
                << "] RMS: Disabling trace generation after " << m_insns_total
                << " instructions / " << m_uops_total << " micro-ops\n";
    }
  }
...
// We need to be both in an ROI-like state (RMS active) and the thread needs to
// be active to write data to the trace file
bool RobTimer::deptraceIsActive(thread_id_t thread_id) {
  return deptrace_rms_active[thread_id] && deptrace_active[thread_id];
}

// Allow the changing of the active state (determined by the trace commands),
// even if we aren't in RMS-active. This is independent of RMS active.
...
void RobTimer::deptrace_thread_create(HooksManager::ThreadCreate* args) {
  ...
  // this active variable is only to disable trace generation during
  // spinloops/syscalls, etc. Default to enabled
...
boost::tuple<uint64_t, SubsecondTime> RobTimer::simulate(
  ...
  for (std::vector<DynamicMicroOp*>::const_iterator it = insts.begin();
    ...
    if (false /*deptrace_active*/) {
      (*_deptrace_insn_f)
          << (*it)->getSequenceNumber() << " " << (*it)->getInstructionNumber()
          << " " << deptrace_insn_count << " " << (*it)->getMicroOp()->isLoad()
          << (*it)->getMicroOp()->isStore() << (*it)->getMicroOp()->isBranch()
          << " " << (*it)->getMicroOp()->getInstruction()->getDisassembly()
          << "\n";
    ...
    uint64_t lowestValidSequenceNumber =
        this->rob.size() > 0 ? this->rob.front().uop->getSequenceNumber() : 0;
    if (entry->uop->getMicroOp()->isStore()) {
      for (unsigned int i = 0;
        ...
        uint64_t addressProducer = this->registerDependencies->peekProducer(
    ...
    this->registerDependencies->setDependencies(*entry->uop,
    ...
    if (deptraceIsActive(thread_id)) {
      ...
      for (size_t i = 0; i < entry->uop->getDependenciesLength(); ++i) {
        uint64_t d = entry->uop->getDependency(i);
        uint64_t in = deptrace_microops ? entry->uop->getSequenceNumber()
                                        : entry->uop->getInstructionNumber();
        ...
        if (in != din) {
          if (i < num_reg_dep) {
            // Only add the register dependency if we don't find the addr-gen
            // dep
    ...
    this->memoryDependencies->setDependencies(*entry->uop,
                                              lowestValidSequenceNumber);

    /* Trace format:
     *  [L|LA]<PC_diff_in_decimal>(d<dep>)(m<mem_dep>) <hex_addr> <size>
     *  [S|SA]<PC_diff_in_decimal>(d<dep>)(m<mem_dep>)(a<addr_dep>) <hex_addr>
     <size>
     *  [B|C|c|J|j|r]<PC_diff_in_decimal>(d<dep>)(m<dep>)(t<target_addr>)(*)?
     *  [A|M|D|Q]<PC_diff_in_decimal>(d<dep>)(m<dep>)
     *
     * Legend:
     *  L=load, LA=load of atomic, S=store, SA=store of atomic,
     *  B=conditional branch (direct), C=call direct, c=call indirect, J=jump
     direct, j=jump indirect, r=return (indirect)
     *  A=fp_addsub, M=fp_mul, D=fp_div, Q=fp_sqrt, []=generic
     *  d=reg dependence, m=mem dependence, a=addr dependence, t=target address,
     *=taken (only for conditional)
     *  X=src regs Y=dst regs

     * Example:
     *  2 A0 3d1 B2d2t-120* L5d1 fff0 4
     */

    // deptrace_src_regs.clear();
    // deptrace_dst_regs.clear();
    ...
    if (deptraceIsActive(thread_id)) {
      ...
      for (unsigned int i = 0;
           i < (*it)->getMicroOp()->getSourceRegistersLength(); i++) {
        deptrace_src_regs.push_back((*it)->getMicroOp()->getSourceRegister(i));
      }
      for (unsigned int i = 0;
           i < (*it)->getMicroOp()->getDestinationRegistersLength(); i++) {
        deptrace_dst_regs.push_back(
            (*it)->getMicroOp()->getDestinationRegister(i));
      }
      ...
      if ((*it)->isCall()) {
        deptrace_is_call = true;
      }
      if ((*it)->isRet()) {
        deptrace_is_ret = true;
      }
      if ((*it)->isJump()) {
        deptrace_is_jump = true;
      }
      if ((*it)->isDirect()) {
        deptrace_is_direct = true;
      }
      ...
      // If we are looking for instructions (!deptrace_microops), then we only
      // do this on the last instruction If we are looking for microops
      // (deptrace_microops), then we handle this for each microop
      if ((*it)->isLast() || deptrace_microops) {
        ...
        if (deptrace_first_line) {
          ...
          instr_to_insert_clear_stats =
              Sim()->getCfg()->getInt("rob_timer/insert-clear-stats-by-icount");
        }
        ...
        if (deptrace_is_load) {
          ...
          if (dmo.getMicroOp()->getInstruction()->isAtomic())
            deptrace_f << "LA"
                       << getPCDiffAndUpdateLast();  // Load uop (part of an
                                                     // atomic instruction)
          else
...
#if DEBUG_DEPTRACE >= 1
          deptrace_f << "(" << std::hex
                     << dmo.getMicroOp()->getInstruction()->getAddress()
                     << std::dec << ")";
#endif
          ...
          for (auto src : deptrace_src_regs) {
            deptrace_f << "x" << src;
          }
          for (auto dst : deptrace_dst_regs) {
            deptrace_f << "y" << dst;
          }
        ...
        if (deptrace_is_store) {
          ...
          if (dmo.getMicroOp()->getInstruction()->isAtomic())
            deptrace_f << "SA"
                       << getPCDiffAndUpdateLast();  // Store uop (part of an
                                                     // atomic instruction)
          else
        ...
        if (deptrace_is_branch || deptrace_is_call || deptrace_is_ret ||
            deptrace_is_jump) {
          if (deptrace_last_was_newline)
            deptrace_last_was_newline = false;
          else
          ...
          if (deptrace_is_branch) {
            assert(deptrace_is_direct);
            deptrace_f << "B";
          }
          if (deptrace_is_call) {
            if (deptrace_is_direct)
              deptrace_f << "C";
            else
              deptrace_f << "c";
          }
          ...
          deptrace_f << "t"
                     << getPCDiff(dmo.getBranchTarget(), deptrace_last_pc);
          if (deptrace_is_branch && dmo.isBranchTaken()) {
            deptrace_f << "*";
          }
        }
        ...
        if (!deptrace_is_branch && !deptrace_is_call && !deptrace_is_ret &&
            !deptrace_is_jump && !deptrace_is_load && !deptrace_is_store) {
          if (deptrace_last_was_newline)
            deptrace_last_was_newline = false;
          else
            deptrace_f << " ";
          ...
          } else if (dmo.getMicroOp()->getSubtype() ==
                     MicroOp::UOP_SUBTYPE_FP_ADDSUB) {
            deptrace_f << "A";
          } else if (dmo.getMicroOp()->getSubtype() ==
                     MicroOp::UOP_SUBTYPE_FP_MUL) {
            deptrace_f << "M";
          } else if (dmo.getMicroOp()->getSubtype() ==
                     MicroOp::UOP_SUBTYPE_FP_DIV) {
            deptrace_f << "D";
          } else if (dmo.getMicroOp()->getSubtype() ==
                     MicroOp::UOP_SUBTYPE_FP_SQRT) {
            deptrace_f << "Q";
          }  // else deptrace_f << "i";
          deptrace_f << getPCDiffAndUpdateLast();
      ...
      deptrace_is_call = false;
      deptrace_is_ret = false;
      deptrace_is_jump = false;
      deptrace_is_direct = false;
    ...
    Thread* thread =
        Sim()->getThreadManager()->getThreadOnCore(m_core->getId());
    thread_id_t thread_id = thread ? thread->getId() : 0xffffffff;
      ...
      switch (current_cmd) {
        ...
        case 1:
          if (deptraceRMSIsActive(thread_id)) {
...

            {
              ScopedLock sl(m_print_lock);
              std::cout << "[DEPTRACE:" << m_core->getId() << ":" << thread_id
                        << "] RMS: Clear stats command seen at "
                        << m_insns_total << " instructions / " << m_uops_total
                        << " micro-ops\n";
            }
          }
          break;
        ...
        case 3:
          if (deptraceRMSIsActive(thread_id)) {
            ...
            deptrace_saved_data
                << "ACQ " << std::hex << (*it)->getMicroOp()->trace_data[1]
                << " " << (*it)->getMicroOp()->trace_data[2] << " " << thread_id
                << std::dec;
        ...
        case 4:
          ...
          // We need to test this again because we might not be in the RMS/ROI
          // region
        ...
        case 5:
          if (deptraceRMSIsActive(thread_id)) {
            ...
            deptrace_f << "REL " << std::hex
                       << (*it)->getMicroOp()->trace_data[1] << " "
                       << (*it)->getMicroOp()->trace_data[2] << " " << thread_id
                       << std::dec << "\n";
            deptrace_last_was_newline = true;
          }
        ...
        case 7:
          if (deptraceRMSIsActive(thread_id)) {
            deptrace_saved_data.str("");
            deptrace_saved_data
                << "BAR " << std::hex << (*it)->getMicroOp()->trace_data[1]
                << " " << (*it)->getMicroOp()->trace_data[2] << " "
                << (*it)->getMicroOp()->trace_data[3] << " " << thread_id
                << std::dec;
            sync_instr_pending = true;
            deptraceSetActive(thread_id, false);
          }
        ...
        case 13:
          if (deptraceRMSIsActive(thread_id)) {
            // Always handle thread state data
            deptrace_saved_data.str("");
            deptrace_saved_data << "CV_SIGNAL " << std::hex
                                << (*it)->getMicroOp()->trace_data[1] << " "
                                << thread_id << std::dec;
            sync_instr_pending = true;
            deptraceSetActive(thread_id, false);
        ...
        case 15:
          if (deptraceRMSIsActive(thread_id)) {
            // Always handle thread state data
            deptrace_saved_data.str("");
            deptrace_saved_data << "CV_BCAST " << std::hex
                                << (*it)->getMicroOp()->trace_data[1] << " "
                                << thread_id << std::dec;
            sync_instr_pending = true;
            deptraceSetActive(thread_id, false);
        ...
        case 17:
          if (deptraceRMSIsActive(thread_id)) {
            ...
            deptrace_saved_data.str("");
            deptrace_saved_data
                << "CV_WAIT " << std::hex << (*it)->getMicroOp()->trace_data[1]
                << " " << (*it)->getMicroOp()->trace_data[2] << " " << thread_id
                << std::dec;
            sync_instr_pending = true;
            deptraceSetActive(thread_id, false);
        ...
        case 24:
          ...
          // init sem signal but not tested will be commented in the main
          // routinereplace.cc file
        case 27:
          if (deptraceRMSIsActive(thread_id)) {
            // Always handle thread state data
            deptrace_saved_data.str("");
            deptrace_saved_data << "SIGNAL " << std::hex
                                << (*it)->getMicroOp()->trace_data[1] << " "
                                << thread_id << std::dec;
            sync_instr_pending = true;
            deptraceSetActive(thread_id, false);
        ...
        case 29:
          if (deptraceRMSIsActive(thread_id)) {
            // Always handle thread state data
            deptrace_saved_data.str("");
            deptrace_saved_data << "WAIT " << std::hex
                                << (*it)->getMicroOp()->trace_data[1] << " "
                                << thread_id << std::dec;
            sync_instr_pending = true;
            deptraceSetActive(thread_id, false);
    ...
    if (m_store_to_load_forwarding && entry->uop->getMicroOp()->isLoad()) {
      for (unsigned int i = 0; i < entry->uop->getDependenciesLength(); ++i) {
        RobEntry* prodEntry =
            this->findEntryBySequenceNumber(entry->uop->getDependency(i));
        // If we depend on a store
        if (prodEntry->uop->getMicroOp()->isStore()) {
          // Remove dependency on the store (which won't execute until it
          // reaches the front of the ROB)
          entry->uop->removeDependency(entry->uop->getDependency(i));

          // Add dependencies to the producers of the value being stored instead
          // Remark: one of these may be producing the store address, but
          // because the store has to be
    ...
    for (unsigned int i = 0; i < entry->getNumAddressProducers(); ++i) {
      if (rob.size() &&
          entry->getAddressProducer(i) >= rob[0].uop->getSequenceNumber()) {
        RobEntry* prodEntry =
            this->findEntryBySequenceNumber(entry->getAddressProducer(i));
        bool found = false;
...
#ifdef DEBUG_PERCYCLE
    std::cout << "** simulate: "
              << entry->uop->getMicroOp()->toShortString(true) << std::endl;
#endif
  ...
  while (true) {
    ...
    totalLat += latency;
    if (latency == SubsecondTime::Zero()) break;
  }
...
SubsecondTime RobTimer::doDispatch(SubsecondTime** cpiComponent) {
  ...
  if (frontend_stalled_until <= now) {
    ...
    while (m_num_in_rob < windowSize) {
      ...
      // This is actually in the decode stage, there's a buffer between decode
      // and dispatch so we shouldn't do this here.
...
#ifdef DEBUG_PERCYCLE
      std::cout << "DISPATCH " << entry->uop->getMicroOp()->toShortString()
                << std::endl;
#endif
  ...
  if (cpiFrontEnd) {
    ...
    if (cpiRobHead) {
      // Have memory/serialization components take precendence over front-end
      // stalls
  ...
  if (m_num_in_rob == windowSize)
    return next_event;  // front-end is effectively stalled so wait for another
                        // event
  else
...
#ifdef DEBUG_PERCYCLE
  std::cout << "ISSUE    " << entry->uop->getMicroOp()->toShortString()
  ...
  for (size_t idx = 0; idx < entry->getNumDependants(); ++idx) {
    ...
    if (depEntry->uop->getMicroOp()->isStore() &&
      ...
      for (unsigned int i = 0; i < depEntry->getNumAddressProducers(); ++i) {
        ...
        RobEntry* prodEntry =
            addressProducer >= this->rob.front().uop->getSequenceNumber()
                ? this->findEntryBySequenceNumber(addressProducer)
                : NULL;
...
#ifdef ASSERT_SKIP
        LOG_ASSERT_ERROR(
            m_outstandingLongLatencyInsns >= m_outstandingLongLatencyCycles,
...
SubsecondTime RobTimer::doCommit(uint64_t& instructionsExecuted) {
  ...
  while (rob.size() && (rob.front().done <= now)) {
...
#ifdef DEBUG_PERCYCLE
    std::cout << "COMMIT   " << entry->uop->getMicroOp()->toShortString()
              << std::endl;
#endif
    ...
    if (entry->uop->isLast()) instructionsExecuted++;
...
void RobTimer::execute(uint64_t& instructionsExecuted, SubsecondTime& latency) {
  ...
  if (m_mlp_histogram) countOutstandingMemop(skip);

  LOG_ASSERT_ERROR(
      cpiComponent != NULL,
      "We expected cpiComponent to be set by doDispatch, but it wasn't");
...
void RobTimer::countOutstandingMemop(SubsecondTime time) {
  ...
  for (unsigned int h = 0; h < HitWhere::NUM_HITWHERES; ++h)
    if (counts[h] > 0)
      m_outstandingLoads[h][counts[h] >= MAX_OUTSTANDING ? MAX_OUTSTANDING - 1
                                                         : counts[h]] += time;
  if (total > 0)
    m_outstandingLoadsAll[total >= MAX_OUTSTANDING ? MAX_OUTSTANDING - 1
                                                   : total] += time;
}

void RobTimer::printRob() {
  std::cout << "** ROB state @ " << now << "  size(" << m_num_in_rob
  ...
  if (frontend_stalled_until > now) {
    ...
    if (in_icache_miss) std::cout << ", in I-cache miss";
    std::cout << std::endl;
  }
