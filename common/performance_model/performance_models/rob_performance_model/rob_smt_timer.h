...
class RobSmtTimer : public SmtTimer {
 private:
  class RobEntry {
   ...
   public:
    ...
    void addAddressProducer(uint64_t sequenceNumber) {
      LOG_ASSERT_ERROR(
          numAddressProducers < MAX_ADDRESS_PRODUCERS,
          "Too many address producers, increase MAX_ADDRESS_PRODUCERS(%d)",
  ...
  void setStoreAddressProducers(smtthread_id_t thread_id, RobEntry *entry,
                                uint64_t lowestValidSequenceNumber);
  RobEntry *findEntryBySequenceNumber(smtthread_id_t thread_num,
  ...
  bool tryIssue(smtthread_id_t thread_num, SubsecondTime &next_event,
                bool would_have_skipped = false);
  void issueInstruction(smtthread_id_t thread_num, uint64_t idx,
                        SubsecondTime &next_event);

  void computeCurrentWindowsize();

 public:
  RobSmtTimer(int num_threads, Core *core, PerformanceModel *perf,
  ...
  virtual void pushInstructions(smtthread_id_t thread_id,
                                const std::vector<DynamicMicroOp *> &insts);
  virtual boost::tuple<uint64_t, SubsecondTime> returnLatency(
