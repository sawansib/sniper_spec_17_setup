 ...
 protected:
  class SmtThread {
   public:
    ...
    ConditionVariable cond;  // thread waits here when it has too many
                             // outstanding pre-ROB instructions
...
#define INVALID_SMTTHREAD_ID ((smtthread_id_t)-1)
  ...
  virtual void pushInstructions(smtthread_id_t thread_id,
                                const std::vector<DynamicMicroOp *> &insts) = 0;
  virtual boost::tuple<uint64_t, SubsecondTime> returnLatency(
      smtthread_id_t thread_id) = 0;
 ...
 private:
  ...
  static SInt64 hookThreadMigrate(UInt64 object, UInt64 argument) {
    ((SmtTimer *)object)
        ->threadMigrate((HooksManager::ThreadMigrate *)argument);
    return 0;
  }
  void roiBegin();
