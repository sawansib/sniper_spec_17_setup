...
const char *DynamicMicroOpNehalem::getType() const { return "nehalem"; }

const char *DynamicMicroOpNehalem::PortTypeString(
    DynamicMicroOpNehalem::uop_port_t port) {
  switch (port) {
    case UOP_PORT0:
      return "port0";
    case UOP_PORT1:
    ...
    case MicroOp::UOP_SUBTYPE_GENERIC:
      switch (uop->getInstructionOpcode()) {
        case XED_ICLASS_IMUL:
          return uop->getOperandSize() == 64 ? DynamicMicroOpNehalem::UOP_PORT0
                                             : DynamicMicroOpNehalem::UOP_PORT1;
        case XED_ICLASS_LEA:
          return DynamicMicroOpNehalem::UOP_PORT1;  // 0 on Core2, 1 on Nehalem,
                                                    // 0 (simple) or 1 (complex)
                                                    // on Sandy Bridge
...
DynamicMicroOpNehalem::uop_bypass_t DynamicMicroOpNehalem::getBypassType(
  ...
  switch (uop->getSubtype()) {
    case MicroOp::UOP_SUBTYPE_LOAD:
      if (uop->isFpLoadStore()) return UOP_BYPASS_LOAD_FP;
      break;
    case MicroOp::UOP_SUBTYPE_STORE:
      if (uop->isFpLoadStore()) return UOP_BYPASS_FP_STORE;
      break;
    default:
      break;
  }
...
DynamicMicroOpNehalem::DynamicMicroOpNehalem(const MicroOp *uop,
