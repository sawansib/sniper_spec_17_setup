...
CoreModelNehalem::CoreModelNehalem() {
  ...
  instructionLatencies[XED_ICLASS_DIVSS] =
      11;  // 7-14 (data dependent) according to agner.org
  ...
  instructionLatencies[XED_ICLASS_DIVSD] =
      17;  // 7-22 (data dependent) according to agner.org
  ...
  instructionLatencies[XED_ICLASS_SQRTSS] =
      18;  // 7-18 (data dependent) according to agner.org
  ...
  instructionLatencies[XED_ICLASS_SQRTSD] =
      32;  // 7-32 (data dependent) according to agner.org
  ...
  instructionLatencies[XED_ICLASS_CVTDQ2PD] =
      6;  // 4 + 2 bypass (should be 2 uops)
  instructionLatencies[XED_ICLASS_CVTPD2DQ] =
      6;  // 4 + 2 bypass (should be 2 uops)
  instructionLatencies[XED_ICLASS_CVTTPD2DQ] =
      6;  // 4 + 2 bypass (should be 2 uops)
  ...
  instructionLatencies[XED_ICLASS_CVTSI2SD] =
      6;  // 4 + 2 bypass (should be 2 uops)
  ...
  m_lll_cutoff =
      Sim()->getCfg()->getInt("perf_model/core/interval_timer/lll_cutoff");
}
...
unsigned int CoreModelNehalem::getInstructionLatency(const MicroOp *uop) const {
  ...
  LOG_ASSERT_ERROR(instruction_type >= 0 && instruction_type < XED_ICLASS_LAST,
                   "Invalid instruction type %d", instruction_type);
...
unsigned int CoreModelNehalem::getBypassLatency(
    const DynamicMicroOp *uop) const {
  const DynamicMicroOpNehalem *info =
      uop->getCoreSpecificInfo<DynamicMicroOpNehalem>();
  DynamicMicroOpNehalem::uop_bypass_t bypass_type = info->getBypassType();
  LOG_ASSERT_ERROR(
      bypass_type >= 0 && bypass_type < DynamicMicroOpNehalem::UOP_BYPASS_SIZE,
      "Invalid bypass type %d", bypass_type);
  return bypassLatencies[bypass_type];
}

unsigned int CoreModelNehalem::getLongestLatency() const { return 60; }
...
DynamicMicroOp *CoreModelNehalem::createDynamicMicroOp(
    Allocator *alloc, const MicroOp *uop, ComponentPeriod period) const {
  DynamicMicroOpNehalem *info =
      DynamicMicroOp::alloc<DynamicMicroOpNehalem>(alloc, uop, this, period);
