...
OneIPCPerformanceModel::OneIPCPerformanceModel(Core *core)
  ...
  /* Maximum latency which is assumed to be completely overlapped. L1-D hit
   * latency should be a good value. */
  m_latency_cutoff = Sim()->getCfg()->getIntArray(
      "perf_model/core/oneipc/latency_cutoff", core->getId());

  registerStatsMetric("oneipc_timer", core->getId(), "cpiBase", &m_cpiBase);
  registerStatsMetric("oneipc_timer", core->getId(), "cpiBranchPredictor",
  ...
  for (int h = HitWhere::WHERE_FIRST; h < HitWhere::NUM_HITWHERES; h++) {
    if (HitWhereIsValid((HitWhere::where_t)h)) {
      ...
      registerStatsMetric("oneipc_timer", core->getId(), name,
                          &(m_cpiDataCache[h]));
    }
  }
}
...
void OneIPCPerformanceModel::handleInstruction(DynamicInstruction *dynins) {
  ...
  for (unsigned int i = 0; i < ops.size(); i++) {
    ...
    if (o.m_type == Operand::MEMORY) {
      ...
      if (o.m_direction == Operand::READ) {
        if (info.latency >
            ComponentLatency(getCore()->getDvfsDomain(), m_latency_cutoff)
                .getLatency())
      ...
      if (cpiComponent == NULL) cpiComponent = &m_cpiDataCache[info.hit_where];
    }
  }
  ...
  if (isModeled(dynins->instruction))
    ...
    cost.addLatency(
        ComponentLatency(getCore()->getDvfsDomain(), 1).getLatency());

  LOG_ASSERT_ERROR((dynins->instruction->getType() != INST_SYNC &&
...
bool OneIPCPerformanceModel::isModeled(Instruction const *instruction) const {
  ...
  // Mispredicted branches: penalty as defined by
  // BranchPredictor::getMispredictPenalty()
