...
MicroOpPerformanceModel::MicroOpPerformanceModel(Core* core, bool issue_memops)
    : PerformanceModel(core),
      m_core_model(CoreModel::getCoreModel(Sim()->getCfg()->getStringArray(
          "perf_model/core/core_model", core->getId()))),
      m_allocator(m_core_model->createDMOAllocator()),
      m_issue_memops(issue_memops),
      m_dyninsn_count(0),
      m_dyninsn_cost(0),
      m_dyninsn_zero_count(0) {
  registerStatsMetric("performance_model", core->getId(), "dyninsn_count",
                      &m_dyninsn_count);
  registerStatsMetric("performance_model", core->getId(), "dyninsn_cost",
                      &m_dyninsn_cost);
  registerStatsMetric("performance_model", core->getId(), "dyninsn_zero_count",
  ...
  registerStatsMetric("performance_model", core->getId(), "cpiITLBMiss",
                      &m_cpiITLBMiss);
  registerStatsMetric("performance_model", core->getId(), "cpiDTLBMiss",
                      &m_cpiDTLBMiss);

  m_cpiUnknown = SubsecondTime::Zero();
  registerStatsMetric("performance_model", core->getId(), "cpiUnknown",
                      &m_cpiUnknown);

  m_cpiMemAccess = SubsecondTime::Zero();
  registerStatsMetric("performance_model", core->getId(), "cpiSyncMemAccess",
                      &m_cpiMemAccess);
  ...
  if (!m_memaccess_uop) {
    ...
    m_memaccess_uop->makeLoad(0  // uop offset of 0 (first uop)
                              ,
...
void MicroOpPerformanceModel::doSquashing(
    std::vector<DynamicMicroOp*>& current_uops, uint32_t first_squashed) {
  ...
  for (i = first_squashed; i < size; i++) {
    ...
    if (!uop->isSquashed()) {
      ...
      if (intraDeps != 0) {
        ...
        LOG_ASSERT_ERROR(iBase >= intraDeps,
                         "intraInstructionDependancies (%d) should be <= (%d)",
                         intraDeps, iBase);
        uop->setIntraInstrDependenciesLength(
            intraDeps - (current_uops[iBase]->getSquashedCount() -
...
void MicroOpPerformanceModel::handleInstruction(DynamicInstruction* dynins) {
  ComponentPeriod insn_period = *(const_cast<ComponentPeriod*>(
      static_cast<const ComponentPeriod*>(m_elapsed_time)));
  std::vector<DynamicMicroOp*> current_uops;

  // These vectors are local to handleInstruction, but keeping them around as
  // member variables saves a lot on allocation/deallocation time.
  ...
  if (dynins->instruction->getMicroOps()) {
    for (std::vector<const MicroOp*>::const_iterator it =
         ...
         it != dynins->instruction->getMicroOps()->end(); it++) {
      m_current_uops.push_back(
          m_core_model->createDynamicMicroOp(m_allocator, *it, insn_period));
    }
  }
  ...
  for (size_t m = 0; m < m_current_uops.size(); m++) {
    ...
    if (m_current_uops[m]->getMicroOp()->isStore()) {
      ++num_stores;
      if (store_base_index == SIZE_MAX) store_base_index = m;
    }
    if (m_current_uops[m]->getMicroOp()->isLoad()) {
      ++num_loads;
      if (load_base_index == SIZE_MAX) load_base_index = m;
    }
  }

  // Compute the iCache cost, and add to our cycle time
  if (Sim()->getConfig()->getEnableICacheModeling()) {
    // Sometimes, these aren't real instructions (INST_SPAWN, etc), and
    // therefore, we need to skip these
    if (dynins->instruction->getAddress() && !dynins->instruction->isPseudo() &&
        m_current_uops.size() > 0) {
      MemoryResult memres = getCore()->readInstructionMemory(
          dynins->eip, dynins->instruction->getSize());

      // For the interval model, for now, use integers for the cycle latencies
      UInt64 memory_cycle_latency =
          SubsecondTime::divideRounded(memres.latency, insn_period);

      // Set the hit_where information for the icache
      // The interval model will only add icache latencies if there hasn't been
      // a hit.
  ...
  if (m_issue_memops) dynins->accessMemory(getCore());

  // If we haven't gotten all of our read or write data yet, iterate over the
  // operands
  for (size_t i = 0; i < ops.size(); ++i) {
    const Operand& o = ops[i];

    if (o.m_type == Operand::MEMORY) {
      LOG_ASSERT_ERROR(dynins->num_memory > memidx,
                       "Did not get enough memory_info objects");
      DynamicInstruction::MemoryInfo& info = dynins->memory_info[memidx++];
      LOG_ASSERT_ERROR(info.dir == o.m_direction,
                       "Expected memory %d info, got: %d.", o.m_direction,
                       info.dir);

      // Because the interval model is currently in cycles, convert the data to
      // cycles here before using it Force the latencies into cycles for use in
      // the original interval model
      // FIXME Update the Interval Timer to use SubsecondTime
      UInt64 memory_cycle_latency =
          SubsecondTime::divideRounded(info.latency, insn_period);

      // Optimize multiple accesses to the same cache line by one instruction
      // (vscatter/vgather)
      //   For simplicity, vgather/vscatter have 16 load/store microops, one for
      //   each address. Here, we squash microops that touch a given cache line
      //   a second time
      //   FIXME: although the microop is squashed and its latency ignored, the
      //   cache still sees the access
      ...
      if (o.m_direction == Operand::READ) {
        ...
        if (load_base_index != SIZE_MAX) {
          ...
          LOG_ASSERT_ERROR(
              load_index < m_current_uops.size(),
              "Expected load_index(%x) to be less than uops.size()(%d).",
              load_index, m_current_uops.size());
          LOG_ASSERT_ERROR(m_current_uops[load_index]->getMicroOp()->isLoad(),
                           "Expected uop %d to be a load.", load_index);

          if (std::find(m_cache_lines_read.begin(), m_cache_lines_read.end(),
                        cache_line) != m_cache_lines_read.end()) {
          ...
          UInt64 bypass_latency =
              m_core_model->getBypassLatency(m_current_uops[load_index]);
        ...
        if (store_base_index != SIZE_MAX) {
          ...
          LOG_ASSERT_ERROR(
              store_index < m_current_uops.size(),
              "Expected store_index(%d) to be less than uops.size()(%d).",
              store_index, m_current_uops.size());
          LOG_ASSERT_ERROR(
              m_current_uops[store_index]->getMicroOp()->isStore(),
              "Expected uop %d to be a store. [%d|%s]", store_index,
              m_current_uops[store_index]->getMicroOp()->getType(),
              m_current_uops[store_index]->getMicroOp()->toString().c_str());

          if (std::find(m_cache_lines_written.begin(),
                        m_cache_lines_written.end(),
                        cache_line) != m_cache_lines_written.end()) {
          ...
          UInt64 bypass_latency =
              m_core_model->getBypassLatency(m_current_uops[store_index]);
  ...
  if (do_squashing > 0) doSquashing(m_current_uops);

  // Make sure there was an Operand/MemoryInfo for each MicroOp
  // This should detect mismatches between decoding as done by
  // fillOperandListMemOps and InstructionDecoder
  ...
  if (dynins->instruction->getType() == INST_BRANCH) {
    assert(dynins->branch_info.is_branch);
    bool is_mispredict;
    dynins->getBranchCost(getCore(), &is_mispredict);

    // Set whether the branch was mispredicted or not
    LOG_ASSERT_ERROR(m_current_uops[exec_base_index]->getMicroOp()->isBranch(),
                     "Expected to find a branch here.");
    m_current_uops[exec_base_index]->setBranchMispredicted(is_mispredict);
    m_current_uops[exec_base_index]->setBranch(dynins->branch_info.is_branch);
    m_current_uops[exec_base_index]->setDirect(dynins->branch_info.is_direct);
    m_current_uops[exec_base_index]->setBranchTaken(dynins->branch_info.taken);
    m_current_uops[exec_base_index]->setBranchTarget(
        dynins->branch_info.target);
    ...
    if (dynins->branch_info.is_call) {
      m_current_uops[exec_base_index]->setCall(dynins->branch_info.is_call);
      m_current_uops[exec_base_index]->setDirect(dynins->branch_info.is_direct);
      m_current_uops[exec_base_index]->setBranchTaken(
          dynins->branch_info.taken);
      m_current_uops[exec_base_index]->setBranchTarget(
          dynins->branch_info.target);
    }
    if (dynins->branch_info.is_ret) {
      m_current_uops[exec_base_index]->setRet(dynins->branch_info.is_ret);
      m_current_uops[exec_base_index]->setDirect(dynins->branch_info.is_direct);
      m_current_uops[exec_base_index]->setBranchTaken(
          dynins->branch_info.taken);
      m_current_uops[exec_base_index]->setBranchTarget(
    ...
    if (dynins->branch_info.is_jump) {
      m_current_uops[exec_base_index]->setJump(dynins->branch_info.is_jump);
      m_current_uops[exec_base_index]->setDirect(dynins->branch_info.is_direct);
      m_current_uops[exec_base_index]->setBranchTaken(
          dynins->branch_info.taken);
      m_current_uops[exec_base_index]->setBranchTarget(
    ...
    if (insn_cost > 17) {
      ...
      if (exec_base_index != SIZE_MAX) {
        ...
        fprintf(m_insn_log, "-%s:%s",
                dynins->instruction->getDisassembly().c_str(),
                dynins->instruction->getTypeName().c_str());
        fflush(m_insn_log);
#endif
      }
  ...
  // Insert an instruction into the interval model to indicate that time has
  // passed
  uint32_t new_num_insns = 0;
  ComponentTime new_latency(
      m_elapsed_time
          .getLatencyGenerator());  // Get a new, empty holder for latency
  bool latency_out_of_band =
      false;  // If new_latency contains any values not returned from
              // IntervalTimer/RobTimer, we'll need to call synchronize()
  if (m_current_uops.size() > 0) {
    ...
    if (exec_base_index != SIZE_MAX) {
      ...
      fprintf(
          m_insn_log, "-%s",
          m_current_uops[exec_base_index].getInstructionOpcodeName().c_str());
#endif
    }
    fprintf(m_insn_log, "approx cost = %llu\n",
            (long long unsigned int)insn_cost);
             ...
             dynins->instruction->getType() != INST_MEM_ACCESS) {
    ...
    // Because of timing issues (large synchronization deltas), take these
    // latencies into account immediately
    //  Nevertheless, do not add the instruction cost into the interval model
    //  because the latency has already been taken into account here.  The
    //  interval model will serialize, flushing the old window

    std::vector<DynamicMicroOp*> uops;
    uops.push_back(m_core_model->createDynamicMicroOp(
        m_allocator, m_serialize_uop, insn_period));
    ...
    if (dynins->instruction->getType() == INST_TLB_MISS) {
      TLBMissInstruction const* tlb_miss_insn =
          dynamic_cast<TLBMissInstruction const*>(dynins->instruction);
      LOG_ASSERT_ERROR(tlb_miss_insn != NULL,
                       "Expected a TLBMissInstruction, but did not get one.");
    ...
    } else if ((dynins->instruction->getType() == INST_SYNC) ||
               (dynins->instruction->getType() == INST_RECV)) {
      LOG_PRINT_ERROR(
          "The performance model expects non-idle instructions, not INST_SYNC "
          "or INST_RECV");
    } else {
      LOG_PRINT_ERROR(
          "Unexpectedly received something other than a TLBMiss Instruction");
    }

#if DEBUG_DYN_INSN_LOG
    fprintf(m_dyninsn_log, "[%llu] %s: cost = %llu\n",
            (long long unsigned int)m_elapsed_time,
            dynins->instruction->getTypeName().c_str(),
            (long long unsigned int)insn_cost);
#endif
  } else if (insn_cost > SubsecondTime::Zero()) {
    ...
    // MemAccess instructions are memory overheads, and should be handled the
    // same way that Long Latency Loads are Currently, the simulator requires
    // that latencies that return from the memory hierarchy go into effect
    // immediately.  If one tries to place these latencies as normal
    // instructions into the interval model, Graphite will still see a huge
    // delta with this cpu's time, and then generate another, equally large
    // MemAccess instruction.  Therefore, as a work around, add the latencies to
    // the

    MemAccessInstruction const* mem_dyn_insn =
        dynamic_cast<MemAccessInstruction const*>(dynins->instruction);
    LOG_ASSERT_ERROR(mem_dyn_insn != NULL,
                     "Expected a MemAccessInstruction, but did not get one.");

    // Update uop with the necessary information for the MemAccess
    // DynamicInstruction
    std::vector<DynamicMicroOp*> uops;
    DynamicMicroOp* uop = m_core_model->createDynamicMicroOp(
        m_allocator, m_memaccess_uop, insn_period);
    ...
    if ((cutoff > 0) && (insn_cost > cutoff * insn_period.getPeriod())) {
      ...
      // Force this instruction as a LLL, even though we will be taking into
      // account the latencies right away This will flush the old window and
      // also check for 2nd order effects (nested LLLs)
      // FIXME For possible next steps, be sure to pass real LLL value in when
      // we want to compare LLL event lengths for higher accuracy
      force_lll = true;
    } else {
      // Normal load
      cost_add_latency_now = SubsecondTime::Zero();
      cost_add_latency_interval =
          SubsecondTime::divideRounded(insn_cost, insn_period.getPeriod());
    }
    ...
    // Right now, serialization instructions are assumed to be executes, and the
    // load latencies are skipped
    ...
    if (mem_dyn_insn->isFence()) {
      ...
      // CMPXCHG instructions are called in mutex handlers, and their
      // performance is about the same as MFENCEs
      uops.push_back(m_core_model->createDynamicMicroOp(
          m_allocator, m_mfence_uop, insn_period));
      ...
      //  and then we go into the OS only to get the lock and return without
      //  much wait.  In that case, the serialization effects could hit
      //  performance more than the memory barrier ones.
    } else {
      uops.push_back(uop);
    }
  ...
  if (latency_out_of_band) notifyElapsedTimeUpdate();

#if DEBUG_CYCLE_COUNT_LOG
  fprintf(m_cycle_log, "[%s] latency=%d\n", itostr(m_elapsed_time).c_str(),
          itostr(new_latency.getElapsedTime()).c_str());
#endif
}
