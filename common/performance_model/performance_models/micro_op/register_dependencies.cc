...
RegisterDependencies::RegisterDependencies() { clear(); }

void RegisterDependencies::setDependencies(DynamicMicroOp& microOp,
                                           uint64_t lowestValidSequenceNumber) {
  // Create the dependencies for the microOp
  for (uint32_t i = 0; i < microOp.getMicroOp()->getSourceRegistersLength();
       i++) {
    xed_reg_enum_t sourceRegister = microOp.getMicroOp()->getSourceRegister(i);
    uint64_t producerSequenceNumber;
    LOG_ASSERT_ERROR(sourceRegister < XED_REG_LAST,
                     "Source register src[%u]=%u is invalid", i,
                     sourceRegister);
  ...
  for (uint32_t i = 0;
       i < microOp.getMicroOp()->getDestinationRegistersLength(); i++) {
    uint32_t destinationRegister =
        microOp.getMicroOp()->getDestinationRegister(i);
    LOG_ASSERT_ERROR(destinationRegister < XED_REG_LAST,
                     "Destination register dst[%u] = %u is invalid", i,
                     destinationRegister);
...
uint64_t RegisterDependencies::peekProducer(
    xed_reg_enum_t reg, uint64_t lowestValidSequenceNumber) {
  if (reg == XED_REG_INVALID) return INVALID_SEQNR;

  uint64_t producerSequenceNumber = producers[reg];
  if (producerSequenceNumber == INVALID_SEQNR ||
      producerSequenceNumber < lowestValidSequenceNumber)
