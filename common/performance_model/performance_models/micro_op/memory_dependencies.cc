...
void MemoryDependencies::setDependencies(DynamicMicroOp &microOp,
...
uint64_t MemoryDependencies::find(uint64_t address) {
  // There may be multiple entries with the same address, we want the latest one
  // so traverse list in reverse order
  for (int i = producers.size() - 1; i >= 0; --i)
    if (producers.at(i).address == address) return producers.at(i).seqnr;
  return INVALID_SEQNR;
}

void MemoryDependencies::clean(uint64_t lowestValidSequenceNumber) {
  while (!producers.empty() &&
         producers.front().seqnr < lowestValidSequenceNumber)
    producers.pop();
}

void MemoryDependencies::clear() {
  while (!producers.empty()) producers.pop();
