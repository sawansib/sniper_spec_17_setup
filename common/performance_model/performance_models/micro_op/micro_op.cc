...
// Enabling verification can help if there is memory corruption that is
// overwriting the MicroOp datastructure and you would like to detect when it is
// happening
...
void MicroOp::makeLoad(uint32_t offset, xed_iclass_enum_t instructionOpcode,
...
void MicroOp::makeExecute(uint32_t offset, uint32_t num_loads,
...
void MicroOp::makeStore(uint32_t offset, uint32_t num_execute,
                        xed_iclass_enum_t instructionOpcode,
...
void MicroOp::makeDynamic(const String& instructionOpcodeName,
...
MicroOp::uop_subtype_t MicroOp::getSubtype_Exec(const MicroOp& uop) {
  ...
  //  used in determining the data type for load/store when calculating bypass
  //  delays)
...
MicroOp::uop_subtype_t MicroOp::getSubtype(const MicroOp& uop) {
  ...
  // Count all of the GENERIC insns, and if we have too many (3x-per-cycle),
  // break
  if (uop.isLoad())
...
void MicroOp::verify() const {
  LOG_ASSERT_ERROR(uop_subtype == MicroOp::getSubtype(*this),
                   "uop_subtype %u != %u", uop_subtype,
                   MicroOp::getSubtype(*this));
  LOG_ASSERT_ERROR(sourceRegistersLength < MAXIMUM_NUMBER_OF_SOURCE_REGISTERS,
                   "sourceRegistersLength(%d) > MAX(%u)", sourceRegistersLength,
                   MAXIMUM_NUMBER_OF_SOURCE_REGISTERS);
  LOG_ASSERT_ERROR(
      destinationRegistersLength < MAXIMUM_NUMBER_OF_DESTINATION_REGISTERS,
  ...
  for (uint32_t i = 0; i < sourceRegistersLength; i++)
    LOG_ASSERT_ERROR(sourceRegisters[i] < XED_REG_LAST,
                     "sourceRegisters[%u] >= XED_REG_LAST", i);
  for (uint32_t i = 0; i < destinationRegistersLength; i++)
    LOG_ASSERT_ERROR(destinationRegisters[i] < XED_REG_LAST,
                     "destinationRegisters[%u] >= XED_REG_LAST", i);
}
...
void MicroOp::addSourceRegister(xed_reg_enum_t registerId,
...
void MicroOp::addAddressRegister(xed_reg_enum_t registerId,
                                 const String& registerName) {
  VERIFY_MICROOP();
...
void MicroOp::addDestinationRegister(xed_reg_enum_t registerId,
                                     const String& registerName) {
  VERIFY_MICROOP();
...
String MicroOp::toString() const {
  ...
  out << "===============================" << std::endl;
  if (this->isFirst()) out << "FIRST ";
  if (this->isLast()) out << "LAST ";
...
String MicroOp::toShortString(bool withDisassembly) const {
  ...
  if (withDisassembly)
    out << "  --  "
        << (this->getInstruction() ? this->getInstruction()->getDisassembly()
                                   : "(dynamic)");
