...
class DynamicMicroOp {
 private:
  ...
  /** The address is valid for UOP_LOAD and UOP_STORE, it contains the load or
   * store address. */
  ...
  /** Used in doSquashing. Contains number of squashed preceding uops of the
   * instruction*/
  uint32_t squashedCount;
  /** This field contains the length of the dependencies array. */
  uint32_t dependenciesLength;
  /** This array contains the dependencies. The uint64_t stored in the array is
   * the sequenceNumber of the dependency. */
  ...
  /** These first/last flags are needed in case of squashing, as long as
     squashed uop doesn't go to rob and we can't use first/last flags of m_uop
     in such a cases.*/
 ...
 public:
  DynamicMicroOp(const MicroOp *uop, const CoreModel *core_model,
                 ComponentPeriod period);
  virtual ~DynamicMicroOp();

  template <typename T>
  static T *alloc(Allocator *alloc, const MicroOp *uop,
                  const CoreModel *core_model, ComponentPeriod period) {
  ...
  const T *getCoreSpecificInfo() const {
    ...
    LOG_ASSERT_ERROR(ptr != NULL,
                     "DynamicMicroOp of the wrong type: expected %s, got %s",
                     /*T::getType()*/ "???", this->getType());
    return ptr;
  }

  // Mark this micro-op as squashed so it will be ignored in further pipeline
  // stages
  void squash(std::vector<DynamicMicroOp *> *array = NULL);
  bool isSquashed() { return squashed; }

  uint32_t getDependenciesLength() const {
    return this->intraInstructionDependencies + this->dependenciesLength;
  }
  ...
  uint32_t getIntraInstrDependenciesLength() const {
    return this->intraInstructionDependencies;
  }
  void setIntraInstrDependenciesLength(uint32_t deps) {
    intraInstructionDependencies = deps;
  }
  ...
  bool isCall() const { return m_is_call; }
  bool isRet() const { return m_is_ret; }
  bool isJump() const { return m_is_jump; }
  bool isDirect() const { return m_is_direct; }

  void setBranch(bool is_branch) { m_is_branch = is_branch; }
  void setCall(bool is_call) { m_is_call = is_call; }
  void setRet(bool is_ret) { m_is_ret = is_ret; }
  void setJump(bool is_jump) { m_is_jump = is_jump; }
  void setDirect(bool is_direct) { m_is_direct = is_direct; }

  bool isBranchTaken() const { return this->branchTaken; }
  void setBranchTaken(bool _branch_taken) { branchTaken = _branch_taken; }
  bool isBranchMispredicted() const {
    LOG_ASSERT_ERROR(m_uop->isBranch(), "Expected a branch instruction.");
    return this->branchMispredicted;
  }
  void setBranchMispredicted(bool mispredicted) {
    this->branchMispredicted = mispredicted;
  }
  IntPtr getBranchTarget() const { return this->branchTargetAddress; }
  ...
  void setInstructionNumber(uint64_t number) {
  ...
  void setForceLongLatencyLoad(bool forceLLL) {
    m_forceLongLatencyLoad = forceLLL;
  }

  SubsecondTime getPeriod() const {
    LOG_ASSERT_ERROR(m_period != SubsecondTime::Zero(),
                     "MicroOp Period is == SubsecondTime::Zero()");
    return m_period;
  }
  ...
  // More dynamic, architecture-dependent information to be defined by derived
  // classes
