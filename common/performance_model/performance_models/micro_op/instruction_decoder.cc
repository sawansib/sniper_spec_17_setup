...
void InstructionDecoder::addSrcs(std::set<xed_reg_enum_t> regs,
                                 MicroOp *currentMicroOp) {
  for (std::set<xed_reg_enum_t>::iterator it = regs.begin(); it != regs.end();
       ++it)
...
void InstructionDecoder::addAddrs(std::set<xed_reg_enum_t> regs,
                                  MicroOp *currentMicroOp) {
  for (std::set<xed_reg_enum_t>::iterator it = regs.begin(); it != regs.end();
       ++it)
    if (*it != XED_REG_INVALID) {
...
void InstructionDecoder::addDsts(std::set<xed_reg_enum_t> regs,
                                 MicroOp *currentMicroOp) {
  for (std::set<xed_reg_enum_t>::iterator it = regs.begin(); it != regs.end();
       ++it)
    if (*it != XED_REG_INVALID) {
      ...
      if (reg == XED_REG_EIP || reg == XED_REG_RIP)
        continue;  // eip/rip is known at decode time, shouldn't be a dependency
      currentMicroOp->addDestinationRegister(reg,
                                             String(xed_reg_enum_t2str(reg)));
    }
}

unsigned int InstructionDecoder::getNumExecs(const xed_decoded_inst_t *ins,
  ...
  if (xed_decoded_inst_get_category(ins) == XED_CATEGORY_DATAXFER ||
    ...
    // Move instructions with additional microops to process the load and store
    // information
...
const std::vector<const MicroOp *> *InstructionDecoder::decode(
    IntPtr address, const xed_decoded_inst_t *ins, Instruction *ins_ptr) {
  ...
  if (!xed_decoded_inst_get_attribute(ins, XED_ATTRIBUTE_NOP)) {
    for (uint32_t mem_idx = 0;
         mem_idx < xed_decoded_inst_number_of_memory_operands(ins); ++mem_idx) {
      ...
      if (xed_decoded_inst_mem_read(ins, mem_idx)) {
        ...
        memop_load_size.push_back(
            xed_decoded_inst_get_memory_operand_length(ins, mem_idx));
        numLoads++;
      }

      if (xed_decoded_inst_mem_written(ins, mem_idx)) {
        regs_stores.push_back(regs);
        memop_store_size.push_back(
            xed_decoded_inst_get_memory_operand_length(ins, mem_idx));
        numStores++;
      }
  ...
  if (xed_operand_values_get_atomic(ops)) is_atomic = true;
  ...
  for (uint32_t idx = 0; idx < xed_inst_noperands(inst); ++idx) {
    ...
    if (name == XED_OPERAND_AGEN) {
      ...
      if (xed_operand_read(op) && regs_mem.count(reg) == 0)
        regs_src.insert(reg);
      if (xed_operand_written(op)) regs_dst.insert(reg);
    }
  }

// Not sure if this is needed, the hardware is doing a lot of optimizations on
// stack operations
#if 0
  ...
  if (operand_size == 0) operand_size = 64;

  bool is_serializing = false;
  switch (xed_decoded_inst_get_iclass(ins)) {
    // TODO: There may be more (newer) instructions, but they are all kernel
    // only
  ...
  std::vector<const MicroOp *> *uops =
      new std::vector<const MicroOp *>();  //< Return value
  ...
  for (int index = 0; index < totalMicroOps; ++index) {
    ...
    // be dependent on register values.  Therefore, fill it in at simulation
    // time.
    ...
    if (index < numLoads) /* LOAD */
    {
      ...
      currentMicroOp->makeLoad(
          loadIndex, xed_decoded_inst_get_iclass(ins),
    ...
    } else if (index < numLoads + numExecs) /* EXEC */
    {
      ...
      currentMicroOp->makeExecute(
          execIndex, numLoads, xed_decoded_inst_get_iclass(ins),
          xed_iclass_enum_t2str(xed_decoded_inst_get_iclass(ins)),
    ...
    } else /* STORE */
    {
      ...
      currentMicroOp->makeStore(
          storeIndex, numExecs, xed_decoded_inst_get_iclass(ins),
          xed_iclass_enum_t2str(xed_decoded_inst_get_iclass(ins)),
          memop_store_size[storeIndex]);
      if (is_atomic) currentMicroOp->setMemBarrier(true);
    }

    // Fill in the destination registers for both loads and executes, also on
    // stores if there are no loads or executes
    ...
    else if (index < numLoads + numExecs) /* EXEC */
    {
      ...
      if ((xed_decoded_inst_get_iclass(ins) == XED_ICLASS_MOVHPD) ||
          ...
          (xed_decoded_inst_get_iclass(ins) ==
           XED_ICLASS_MOVSD_XMM)  // EXEC exists only for reg-to-reg moves
          || (xed_decoded_inst_get_iclass(ins) == XED_ICLASS_MOVSS)) {
        // In this case, we have a memory to XMM load, where the result merges
    ...
    if (index == 0) {
      ...
      // Use of x87 FPU?
      if (toupper(xed_iclass_enum_t2str(xed_decoded_inst_get_iclass(ins))[0]) ==
          'F')
        currentMicroOp->setIsX87(true);
    }

    // If we are a load, and there is also a store, make sure that we mark this
    // as Last as well to increment the instruction count artificially (for
    // deptrace)
    if (index < numLoads) /* LOAD */
    {
      if (numStores) {
...
// Since we're user-level, there are no interrupts
      ...
      if (is_serializing) currentMicroOp->setSerializing(true);
    }
