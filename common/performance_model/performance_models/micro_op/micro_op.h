...
/**
 * An instruction will be decoded in MicroOperations. There are 3 MicroOperation
 * types: LOAD, EXECUTE AND STORE.
 *
 ...
 * S1 and S2 depend on E1: intraInstructionDependencies = 1 (for both S1 and
 * S2).
 *
 * The typeOffset for L1 = 0, L2 = 1, E1 = 0, S1 = 0, S2 = 1.
 *
 * Getting the sequenceNumber of the MicroOperation for intra instruction
 * dependencies: this->sequenceNumber - typeOffset (= sequenceNumber of first
 * microOperation with the current type) - intraInstructionDependencies
 *
 */
...
struct MicroOp {
  ...
  /** The typeOffset field contains the offset of the microOp starting from the
   * first microOp with that type. */
  uint32_t microOpTypeOffset;
  /** The intraInstructionDependencies variable gives the number of preceding
   * microOp on which this micrOp depends. This number is counted from the first
   * microOp with that type. */
  ...
  /** This array contains the registers read by this MicroOperation, the integer
   * is an id given by libdisasm64. Only valid for UOP_LOAD and UOP_EXECUTE. */
  ...
  /** This array contains the registers written by this MicroOperation, the
   * integer is an id given by libdisasm64. Only valid for UOP_EXECUTE. */
  ...
  void makeLoad(uint32_t offset, xed_iclass_enum_t instructionOpcode,
                const String& instructionOpcodeName, uint16_t mem_size);
  void makeExecute(uint32_t offset, uint32_t num_loads,
                   xed_iclass_enum_t instructionOpcode,
                   const String& instructionOpcodeName, bool isBranch);
  void makeStore(uint32_t offset, uint32_t num_execute,
                 xed_iclass_enum_t instructionOpcode,
                 const String& instructionOpcodeName, uint16_t mem_size);
  ...
  void addAddressRegister(xed_reg_enum_t registerId,
  ...
  void addDestinationRegister(xed_reg_enum_t registerId,
                              const String& registerName);
  ...
  bool isCacheFlush() const {
    // FIXME: Old decoder only listed these three instructions, but there may be
    // more (INVEPT, INVLPGA, INVVPID)
  ...
  bool isDiv() const {
    return this->instructionOpcode == XED_ICLASS_DIV ||
           this->instructionOpcode == XED_ICLASS_IDIV;
  }
