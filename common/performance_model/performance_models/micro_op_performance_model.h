...
class MicroOpPerformanceModel : public PerformanceModel {
 ...
 protected:
  ...
  void doSquashing(std::vector<DynamicMicroOp *> &current_uops,
                   uint32_t first_squashed = 0);
 ...
 private:
  ...
  // An std::set would sound like a better choice for these, but since the
  // number of elements is usually small (one or two, except for some rare
  // vector instructions) a linear search is fast enough; while std::vector does
  // *much* fewer memory allocations/deallocations
