...
FastforwardPerformanceModel::FastforwardPerformanceModel(Core *core,
                                                         PerformanceModel *perf)
    : m_core(core),
      m_perf(perf),
      m_include_memory_latency(Sim()->getCfg()->getBool(
          "perf_model/fast_forward/oneipc/include_memory_latency")),
      m_include_branch_mispredict(Sim()->getCfg()->getBool(
          "perf_model/fast_forward/oneipc/include_branch_misprediction")),
  ...
  registerStatsMetric("fastforward_performance_model", core->getId(),
                      "fastforwarded_time", &m_fastforwarded_time);
  registerStatsMetric("performance_model", core->getId(), "cpiFastforwardTime",
                      &m_fastforwarded_time);

  registerStatsMetric("fastforward_timer", core->getId(), "cpiBase",
                      &m_cpiBase);
  registerStatsMetric("fastforward_timer", core->getId(), "cpiBranchPredictor",
  ...
  for (int h = HitWhere::WHERE_FIRST; h < HitWhere::NUM_HITWHERES; h++) {
    if (HitWhereIsValid((HitWhere::where_t)h)) {
      ...
      registerStatsMetric("fastforward_timer", core->getId(), name,
                          &(m_cpiDataCache[h]));
    }
  }
}

void FastforwardPerformanceModel::incrementElapsedTime(SubsecondTime latency) {
  incrementElapsedTime(latency, m_cpiBase);
}

void FastforwardPerformanceModel::incrementElapsedTime(
    SubsecondTime latency, SubsecondTime &cpiComponent) {
...
void FastforwardPerformanceModel::notifyElapsedTimeUpdate() {
  ...
  if (Sim()->getFastForwardPerformanceManager())
    Sim()->getFastForwardPerformanceManager()->recalibrateInstructionsCallback(
        m_core->getId());
}

void FastforwardPerformanceModel::countInstructions(IntPtr address,
                                                    UInt32 count) {
  incrementElapsedTime(count * m_cpi, m_cpiBase);
}

void FastforwardPerformanceModel::handleMemoryLatency(
    SubsecondTime latency, HitWhere::where_t hit_where) {
...
void FastforwardPerformanceModel::queuePseudoInstruction(
  ...
  LOG_ASSERT_ERROR(!instr->isIdle(), "Expected non-idle instruction, got %s",
                   INSTRUCTION_NAMES[instr->getType()]);
