...
ContentionModel::ContentionModel(String name, core_id_t core_id,
  ...
  if (m_num_outstanding > 0) {
    ...
    registerStatsMetric(name, core_id, "requests-out-of-order",
                        &m_n_outoforder);
    registerStatsMetric(name, core_id, "requests-simultaneous",
                        &m_n_simultaneous);
    registerStatsMetric(name, core_id, "no-free-slots", &m_n_hasfreefail);
    registerStatsMetric(name, core_id, "total-delay", &m_total_delay);
    registerStatsMetric(name, core_id, "total-barrier-delay",
...
UInt32 ContentionModel::getNumUsed(uint64_t t_start) {
  SubsecondTimeCycleConverter conv(m_proc_period);
  return getNumUsed(conv.cyclesToSubsecondTime(t_start));
}

UInt32 ContentionModel::getNumUsed(SubsecondTime t_start) {
  UInt32 num_used = 0;
  for (UInt32 i = 0; i < m_num_outstanding; ++i) {
    if (m_time[i].first > t_start) ++num_used;
  }
  return num_used;
}

SubsecondTime ContentionModel::getTagCompletionTime(UInt64 tag) {
  for (UInt32 i = 0; i < m_num_outstanding; ++i) {
    if (m_time[i].second == tag) return m_time[i].first;
  }
  return SubsecondTime::MaxTime();
}

bool ContentionModel::hasFreeSlot(uint64_t t_start, UInt64 tag) {
  SubsecondTimeCycleConverter conv(m_proc_period);
  return hasFreeSlot(conv.cyclesToSubsecondTime(t_start), tag);
}

bool ContentionModel::hasFreeSlot(SubsecondTime t_start, UInt64 tag) {
  for (UInt32 i = 0; i < m_num_outstanding; ++i) {
    if (m_time[i].first <= t_start) return true;

    // When using tags: an identical tag that's already in process is also
    // acceptable
    if (m_time[i].second == tag) return true;
  }
  ++m_n_hasfreefail;
  return false;
}

bool ContentionModel::hasTag(UInt64 tag) {
  for (UInt32 i = 0; i < m_num_outstanding; ++i) {
    if (m_time[i].second == tag) return true;
  }
  return false;
...
uint64_t ContentionModel::getBarrierCompletionTime(uint64_t t_start,
                                                   uint64_t t_delay,
                                                   UInt64 tag) {
  SubsecondTimeCycleConverter conv(m_proc_period);
  SubsecondTime result =
      getBarrierCompletionTime(conv.cyclesToSubsecondTime(t_start),
                               conv.cyclesToSubsecondTime(t_delay), tag);
  return conv.subsecondTimeToCycles(result);
}

SubsecondTime ContentionModel::getBarrierCompletionTime(SubsecondTime t_start,
                                                        SubsecondTime t_delay,
                                                        UInt64 tag) {
  if (m_num_outstanding == 0) return t_start + t_delay;

  SubsecondTime max_time = t_start;
  for (UInt32 i = 0; i < m_num_outstanding; ++i) {
    if (m_time[i].first > max_time) max_time = m_time[i].first;
  }
...
uint64_t ContentionModel::getCompletionTime(uint64_t t_start, uint64_t t_delay,
                                            UInt64 tag) {
  SubsecondTimeCycleConverter conv(m_proc_period);
  SubsecondTime result =
      getCompletionTime(conv.cyclesToSubsecondTime(t_start),
                        conv.cyclesToSubsecondTime(t_delay), tag);
  return conv.subsecondTimeToCycles(result);
}
 ...
 */
SubsecondTime ContentionModel::getCompletionTime(SubsecondTime t_start,
                                                 SubsecondTime t_delay,
                                                 UInt64 tag) {
  if (m_num_outstanding == 0) return t_start + t_delay;
  ...
  if (t_start == SubsecondTime::Zero())
    ...
    if (t_start == m_t_last) m_n_simultaneous++;
...
uint64_t ContentionModel::getStartTime(uint64_t t_start) {
  SubsecondTimeCycleConverter conv(m_proc_period);
  SubsecondTime result = getStartTime(conv.cyclesToSubsecondTime(t_start));
  return conv.subsecondTimeToCycles(result);
}

SubsecondTime ContentionModel::getStartTime(SubsecondTime t_start) {
  /* Peek start time for a new request */

  if (m_num_outstanding == 0) return t_start;
