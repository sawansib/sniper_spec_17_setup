...
QueueModelWindowedMG1::QueueModelWindowedMG1(String name, UInt32 id)
    : m_window_size(SubsecondTime::NS(
          Sim()->getCfg()->getInt("queue_model/windowed_mg1/window_size"))),
      m_total_requests(0),
...
SubsecondTime QueueModelWindowedMG1::computeQueueDelay(
  ...
  // Advance the window based on the global (barrier) time, as this guarantees
  // the earliest time any thread may be at. Use a backup value of 10 window
  // sizes before the current request to avoid excessive memory usage in case
  // something fishy is going on.
  removeItems(std::max(
      Sim()->getClockSkewMinimizationServer()->getGlobalTime() - m_window_size,
      pkt_time - 10 * m_window_size));
  ...
  if (m_num_arrivals > 1) {
    ...
    // If requesters do not throttle based on returned latency, it's their
    // problem, not ours
    if (utilization > .99) utilization = .99;

    t_queue = SubsecondTime::PS(arrival_rate * service_time_Es2 /
                                (2 * (1. - utilization)));

    // Our memory is limited in time to m_window_size. It would be strange to
    // return more latency than that.
    if (t_queue > m_window_size) t_queue = m_window_size;
  }
...
void QueueModelWindowedMG1::addItem(SubsecondTime pkt_time,
                                    SubsecondTime service_time) {
  m_window.insert(
      std::pair<SubsecondTime, SubsecondTime>(pkt_time, service_time));
  m_num_arrivals++;
...
void QueueModelWindowedMG1::removeItems(SubsecondTime earliest_time) {
  while (!m_window.empty() && m_window.begin()->first < earliest_time) {
    std::multimap<SubsecondTime, SubsecondTime>::iterator entry =
        m_window.begin();
    m_num_arrivals--;
