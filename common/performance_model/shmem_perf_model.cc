...
void ShmemPerfModel::setElapsedTime(Thread_t thread_num, SubsecondTime time) {
  LOG_PRINT("setElapsedTime: thread(%u), time(%s)", thread_num,
            itostr(time).c_str());
...
SubsecondTime ShmemPerfModel::getElapsedTime(Thread_t thread_num) {
  // ScopedReadLock sl(m_shmem_perf_model_lock);

  return m_elapsed_time[thread_num];
}

void ShmemPerfModel::updateElapsedTime(SubsecondTime time,
                                       Thread_t thread_num) {
  LOG_PRINT("updateElapsedTime: time(%s)", itostr(time).c_str());
  // ScopedLock sl(m_shmem_perf_model_lock);

  if (m_elapsed_time[thread_num] < time) m_elapsed_time[thread_num] = time;
}

void ShmemPerfModel::incrElapsedTime(SubsecondTime time, Thread_t thread_num) {
...
void ShmemPerfModel::incrTotalMemoryAccessLatency(SubsecondTime shmem_time) {
  if (m_enabled) {
    // ScopedLock sl(m_shmem_perf_model_lock);
