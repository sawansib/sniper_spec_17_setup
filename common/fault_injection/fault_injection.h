...
class FaultinjectionManager {
 ...
 public:
  ...
  FaultInjector *getFaultInjector(UInt32 core_id,
                                  MemComponent::component_t mem_component);

  void applyFault(Core *core, IntPtr read_address, UInt32 data_size,
                  MemoryResult &memres, Byte *data, const Byte *fault);
};
...
class FaultInjector {
 ...
 public:
  ...
  virtual void preRead(IntPtr addr, IntPtr location, UInt32 data_size,
                       Byte *fault, SubsecondTime time);
  virtual void postWrite(IntPtr addr, IntPtr location, UInt32 data_size,
                         Byte *fault, SubsecondTime time);
};
