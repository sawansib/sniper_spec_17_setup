...
FaultinjectionManager *FaultinjectionManager::create() {
...
FaultinjectionManager::FaultinjectionManager(fault_type_t type,
                                             fault_injector_t injector)
    : m_type(type), m_injector(injector) {}

FaultInjector *FaultinjectionManager::getFaultInjector(
    UInt32 core_id, MemComponent::component_t mem_component) {
  switch (m_injector) {
...
void FaultinjectionManager::applyFault(Core *core, IntPtr read_address,
                                       UInt32 data_size, MemoryResult &memres,
                                       Byte *data, const Byte *fault) {
  switch (m_type) {
    case FAULT_TYPE_TOGGLE:
      for (UInt32 i = 0; i < data_size; ++i) data[i] ^= fault[i];
      break;
...
FaultInjector::FaultInjector(UInt32 core_id,
                             MemComponent::component_t mem_component)
    : m_core_id(core_id), m_mem_component(mem_component) {}

void FaultInjector::preRead(IntPtr addr, IntPtr location, UInt32 data_size,
                            Byte *fault, SubsecondTime time) {
  // Data at virtual address <addr> is about to be read with size <data_size>.
  // <location> corresponds to the physical location (cache line) where the data
  // lives. Update <fault> here according to errors that have accumulated in
  // this memory location.
}

void FaultInjector::postWrite(IntPtr addr, IntPtr location, UInt32 data_size,
                              Byte *fault, SubsecondTime time) {
  // Data at virtual address <addr> has just been written to.
  // Update <fault> here according to errors that occured during the writing of
  // this memory location.
}
