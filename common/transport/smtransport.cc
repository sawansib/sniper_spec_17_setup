...
Transport::Node *SmTransport::getGlobalNode() { return m_global_node; }
...
SmTransport::SmNode::SmNode(core_id_t core_id, SmTransport *smt)
    : Node(core_id), m_smt(smt) {}

SmTransport::SmNode::~SmNode() {
  LOG_ASSERT_WARNING(m_queue.empty(), "Unread messages in queue for core: %d",
                     getCoreId());
  m_smt->clearNodeForId(getCoreId());
}

void SmTransport::SmNode::globalSend(SInt32 dest_proc, const void *buffer,
                                     UInt32 length) {
  LOG_ASSERT_ERROR(dest_proc == 0, "Destination other than zero: %d",
                   dest_proc);
  send((SmNode *)m_smt->getGlobalNode(), buffer, length);
}

void SmTransport::SmNode::send(SInt32 dest_id, const void *buffer,
                               UInt32 length) {
  SmNode *dest_node = m_smt->getNodeFromId(dest_id);
  LOG_ASSERT_ERROR(dest_node != NULL,
                   "Attempt to send to non-existent node: %d", dest_id);
  send(dest_node, buffer, length);
}

void SmTransport::SmNode::send(SmNode *dest_node, const void *buffer,
                               UInt32 length) {
  Byte *data = new Byte[length];
  memcpy(data, buffer, length);

  LOG_PRINT("sending msg -- size: %i, data: %p, dest: %p", length, data,
            dest_node);
