...
DvfsManager::DvfsManager() {
  ...
  m_transition_latency =
      SubsecondTime::NS() * Sim()->getCfg()->getInt("dvfs/transition_latency");

  LOG_ASSERT_ERROR("simple" == Sim()->getCfg()->getString("dvfs/type"),
                   "Currently, only this simple dvfs scheme is defined");

  // Initial configuration provides for socket-wide frequency control, with
  // [dvfs/simple/cores_per_socket] cores per socket
  ...
  // Create a domain, converting from GHz frequencies specified in the
  // configuration to Hz
  ComponentPeriod core_period =
      ComponentPeriod::fromFreqHz(core_frequency * 1000000000);

  // Allocate m_num_proc_domains clock domains, each with core_period as the
  // initial frequency/period
  ...
  for (unsigned int i = 0; i < m_num_app_cores; ++i) {
    float _core_frequency =
        Sim()->getCfg()->getFloatArray("perf_model/core/frequency", i);
    if (_core_frequency != core_frequency) {
      app_proc_domains[getCoreDomainId(i)] =
          ComponentPeriod::fromFreqHz(_core_frequency * 1000000000);
...
UInt32 DvfsManager::getCoreDomainId(UInt32 core_id) {
  LOG_ASSERT_ERROR(
      core_id < m_num_app_cores,
      "Core domain ids are only supported for application process domains");
...
const ComponentPeriod* DvfsManager::getGlobalDomain(
    DvfsGlobalDomain domain_id) {
  LOG_ASSERT_ERROR(UInt32(domain_id) < global_domains.size(),
                   "Global domain %d requested, only %d exist", domain_id,
                   global_domains.size());
...
void DvfsManager::setCoreDomain(UInt32 core_id, ComponentPeriod new_freq) {
  if (core_id < m_num_app_cores) {
    if (new_freq.getPeriod() !=
        app_proc_domains[getCoreDomainId(core_id)].getPeriod()) {
      /* queue a fake instruction that will account for the transition latency
       */
      PseudoInstruction* i = new DelayInstruction(
          m_transition_latency, DelayInstruction::DVFS_TRANSITION);
      Sim()
          ->getCoreManager()
          ->getCoreFromID(core_id)
          ->getPerformanceModel()
          ->queuePseudoInstruction(i);
    }
