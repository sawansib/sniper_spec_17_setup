...
class MemoryTracker {
 public:
  // A light routine tracer that will activate the RoutineTracer infrastructure
  // and allow us to get call stacks
  ...
  class RoutineTracer : public ::RoutineTracer {
   public:
    ...
    virtual RoutineTracerThread *getThreadHandler(Thread *thread) {
      return new RoutineTracerThread(thread);
    }
  ...
  void logMalloc(thread_id_t thread_id, UInt64 eip, UInt64 address,
                 UInt64 size);
 ...
 private:
  ...
  void ce_notify_access(UInt64 owner, Core::mem_op_t mem_op_type,
  ...
  static void __ce_notify_access(UInt64 user, UInt64 owner,
