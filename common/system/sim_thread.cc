...
SimThread::~SimThread() { delete m_thread; }

void SimThread::run() {
  core_id_t core_id =
      Sim()->getCoreManager()->registerSimThread(CoreManager::SIM_THREAD);
  ...
  net->registerCallback(SIM_THREAD_TERMINATE_THREADS, terminateFunc,
                        (void *)&cont);

  // Actual work gets done here
  while (cont) net->netPullFromTransport();
