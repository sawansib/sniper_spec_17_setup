...
class SimFutex {
 ...
 public:
  SimFutex();
  ~SimFutex();
  bool enqueueWaiter(thread_id_t thread_id, int mask, SubsecondTime time,
                     SubsecondTime timeout_time, SubsecondTime &time_end);
  thread_id_t dequeueWaiter(thread_id_t thread_by, int mask,
                            SubsecondTime time);
...
class SyscallServer {
 public:
  ...
  void handleSleepCall(thread_id_t thread_id, SubsecondTime wake_time,
                       SubsecondTime curr_time, SubsecondTime &end_time);
  IntPtr handleFutexCall(thread_id_t thread_id, futex_args_t &args,
                         SubsecondTime curr_time, SubsecondTime &end_time);
  SubsecondTime getNextTimeout(SubsecondTime time);

 private:
  // Handling Futexes
  IntPtr futexWait(thread_id_t thread_id, int *uaddr, int val, int act_val,
                   int val3, SubsecondTime curr_time,
                   SubsecondTime timeout_time, SubsecondTime &end_time);
  IntPtr futexWake(thread_id_t thread_id, int *uaddr, int nr_wake, int val3,
                   SubsecondTime curr_time, SubsecondTime &end_time);
  IntPtr futexWakeOp(thread_id_t thread_id, int *uaddr, int *uaddr2,
                     int nr_wake, int nr_wake2, int op, SubsecondTime curr_time,
                     SubsecondTime &end_time);
  IntPtr futexCmpRequeue(thread_id_t thread_id, int *uaddr, int val,
                         int *uaddr2, int val3, int act_val,
                         SubsecondTime curr_time, SubsecondTime &end_time);

  SimFutex *findFutexByUaddr(int *uaddr, thread_id_t thread_id);
  thread_id_t wakeFutexOne(SimFutex *sim_futex, thread_id_t thread_by, int mask,
                           SubsecondTime curr_time);
  int futexDoOp(Core *core, int op, int *uaddr);

  void futexPeriodic(SubsecondTime time);

  SubsecondTime applyRescheduleCost(thread_id_t thread_id,
