...
class RoutineTracerFunctionStats {
 public:
  typedef std::unordered_map<ThreadStatsManager::ThreadStatType, UInt64>
      RtnValues;
  class Routine : public RoutineTracer::Routine {
   public:
    ...
    Routine(IntPtr eip, const char *name, const char *imgname, IntPtr offset,
            int column, int line, const char *filename)
        : RoutineTracer::Routine(eip, name, imgname, offset, column, line,
                                 filename),
          m_provisional(false),
          m_calls(0),
          m_values(),
  ...
  class RtnMaster : public RoutineTracer {
   public:
    ...
    virtual void addRoutine(IntPtr eip, const char *name, const char *imgname,
                            IntPtr offset, int column, int line,
                            const char *filename);
    virtual bool hasRoutine(IntPtr eip);
    void updateRoutine(IntPtr eip, UInt64 calls, RtnValues values);
    void updateRoutineFull(const CallStack &stack, UInt64 calls,
                           RtnValues values);
    void updateRoutineFull(RoutineTracerFunctionStats::Routine *rtn,
                           UInt64 calls, RtnValues values);
    RoutineTracerFunctionStats::Routine *getRoutineFullPtr(
        const CallStack &stack);

   private:
    Lock m_lock;
    // Flat-profile per-thread statistics (excludes statistics from child
    // calls).
    typedef std::unordered_map<IntPtr, RoutineTracerFunctionStats::Routine *>
        RoutineMap;
    RoutineMap m_routines;
    // Call-stack-based statistics (includes statistics from child calls).
    typedef std::unordered_map<CallStack, RoutineTracerFunctionStats::Routine *>
        RoutineMapFull;
    RoutineMapFull m_callstack_routines;

    UInt64 ce_get_owner(core_id_t core_id, UInt64 address);
    void ce_notify_evict(bool on_roi_end, UInt64 owner, UInt64 evictor,
    ...
    static void __ce_notify_evict(UInt64 user, bool on_roi_end, UInt64 owner,
                                  UInt64 evictor,
                                  CacheBlockInfo::BitsUsedType bits_used,
                                  UInt32 bits_total) {
      ((RtnMaster *)user)
          ->ce_notify_evict(on_roi_end, owner, evictor, bits_used, bits_total);
    }
