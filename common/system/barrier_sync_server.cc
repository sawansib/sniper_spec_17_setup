...
BarrierSyncServer::BarrierSyncServer()
    : m_local_clock_list(Sim()->getConfig()->getApplicationCores(),
                         SubsecondTime::Zero()),
      ...
      m_core_thread(Sim()->getConfig()->getApplicationCores(),
                    INVALID_THREAD_ID),
  ...
  try {
    m_barrier_interval =
        SubsecondTime::NS() * (UInt64)Sim()->getCfg()->getInt(
                                  "clock_skew_minimization/barrier/quantum");
  } catch (...) {
    LOG_PRINT_ERROR(
        "Error Reading 'clock_skew_minimization/barrier/quantum' from the "
        "config file");
  }
  ...
  // Order our hooks to occur after possible reschedulings (which are done with
  // ORDER_ACTION)
  Sim()->getHooksManager()->registerHook(
      HookType::HOOK_THREAD_EXIT, BarrierSyncServer::hookThreadExit,
      (UInt64)this, HooksManager::ORDER_NOTIFY_POST);
  Sim()->getHooksManager()->registerHook(
      HookType::HOOK_THREAD_STALL, BarrierSyncServer::hookThreadStall,
      (UInt64)this, HooksManager::ORDER_NOTIFY_POST);
  Sim()->getHooksManager()->registerHook(
      HookType::HOOK_THREAD_MIGRATE, BarrierSyncServer::hookThreadMigrate,
      (UInt64)this, HooksManager::ORDER_NOTIFY_POST);
...
void BarrierSyncServer::synchronize(core_id_t core_id, SubsecondTime time) {
  ...
  if (m_fastforward)
    master_core_id =
        core_id;  // In fast-forward, the SMT performance model in not active so
                  // every core (HW context) calls into the barrier
  else
    master_core_id = m_core_group[core_id] == INVALID_CORE_ID
                         ? core_id
                         : m_core_group[core_id];
  Core *master_core = Sim()->getCoreManager()->getCoreFromID(core_id);
  thread_id_t thread_me = core->getThread()->getId();

  CLOG("barrier", "Core %d entry (master core %d, thread %d, ffwd %d)", core_id,
       master_core_id, thread_me, m_fastforward);
  LOG_PRINT("Received 'SIM_BARRIER_WAIT' from Core(%i), Time(%s)", core_id,
            itostr(time).c_str());

  LOG_ASSERT_ERROR(core->getState() == Core::RUNNING ||
                       core->getState() == Core::INITIALIZING,
                   "Core(%i) is not running or initializing at time(%s)",
                   core_id, itostr(time).c_str());
  LOG_ASSERT_ERROR(m_barrier_acquire_list[master_core_id] == false,
                   "Core(%i) or its sibling is already in the barrier (this is "
                   "thread %d, we have thread %d)",
                   master_core_id, thread_me, m_core_thread[master_core_id]);

  if (time < m_next_barrier_time && !m_fastforward) {
    LOG_PRINT(
        "Sent 'SIM_BARRIER_RELEASE' immediately time(%s), "
        "m_next_barrier_time(%s)",
        itostr(time).c_str(), itostr(m_next_barrier_time).c_str());
    // LOG_PRINT_WARNING("core_id(%i), local_clock(%llu),
    // m_next_barrier_time(%llu), m_barrier_interval(%llu)", core_id, time,
    // m_next_barrier_time, m_barrier_interval);
  ...
  bool mustWait = true;
  if (isBarrierReached()) mustWait = barrierRelease(thread_me);
  ...
  CLOG("barrier", "Core %d exit (master core %d, thread %d)", core_id,
       master_core_id, thread_me);
}
...
void BarrierSyncServer::threadMigrate(HooksManager::ThreadMigrate *argument) {
  ...
  releaseThread(argument->thread_id);
  // Migration due to thread stall/exit will generate another event later, we'll
  // do a signal() then Migration because of pre-emption is done only inside
  // periodic(), we'll return into barrierRelease()
}
...
void BarrierSyncServer::releaseThread(thread_id_t thread_id) {
  for (core_id_t core_id = 0;
    ...
    if (m_barrier_acquire_list[core_id] &&
        m_core_thread[core_id] == thread_id) {
...
void BarrierSyncServer::signal() {
  ...
  if (isBarrierReached()) barrierRelease(INVALID_THREAD_ID);
}

bool BarrierSyncServer::isCoreRunning(core_id_t core_id, bool siblings) {
  Core *core = Sim()->getCoreManager()->getCoreFromID(core_id);
  if (core->getState() == Core::RUNNING) {
    LOG_ASSERT_ERROR(core->getThread(),
                     "Core (%d) is running but has no thread", core_id);
    if (Sim()->getThreadManager()->isThreadRunning(core->getThread()->getId()))
      return true;
  }

  if (siblings && !m_fastforward) {
    for (core_id_t sibling_core_id = 0;
         sibling_core_id < (core_id_t)Sim()->getConfig()->getApplicationCores();
         sibling_core_id++) {
      if (m_core_group[sibling_core_id] == core_id) {
        if (isCoreRunning(sibling_core_id, false)) return true;
      }
    }
  }
...
void BarrierSyncServer::advance() { barrierRelease(INVALID_THREAD_ID, true); }
...
bool BarrierSyncServer::isBarrierReached() {
  ...
  for (core_id_t core_id = 0;
       core_id < (core_id_t)Sim()->getConfig()->getApplicationCores();
       core_id++) {
    // In fastforward mode, it's enough that a core is waiting. In detailed
    // mode, it needs to have advanced up to the predefined barrier time
    if (m_fastforward) {
      if (m_barrier_acquire_list[core_id]) {
...
bool BarrierSyncServer::barrierRelease(thread_id_t caller_id,
                                       bool continue_until_release) {
  ...
  LOG_ASSERT_ERROR(
      m_to_release.size() == 0,
      "Reached the barrier while some threads haven't even restarted?");
  ...
  while (!core_resumed) {
    ...
    Sim()->getHooksManager()->callHooks(
        ...
        static_cast<subsecond_time_t>(m_next_barrier_time).m_time);
    ...
    if (continue_until_release) {
      ...
      if (Sim()->getThreadManager()->anyThreadRunning())
        ...
        LOG_ASSERT_ERROR(
            Sim()->getSyscallServer()->getNextTimeout(m_global_time) <
                SubsecondTime::MaxTime(),
            "No threads running, no timeout. Application has deadlocked...");
    }

    // If the barrier was disabled from HOOK_PERIODIC (for instance, if roi-end
    // was triggered from a script), break
    if (m_disable) return false;

    m_next_barrier_time += m_barrier_interval;
    LOG_PRINT("m_next_barrier_time updated to (%s)",
              itostr(m_next_barrier_time).c_str());
    ...
    for (core_id_t core_id = 0;
      ...
      if (m_local_clock_list[core_id] < m_next_barrier_time) {
        ...
        if (m_barrier_acquire_list[core_id] == true) {
          // Core *core = Sim()->getCoreManager()->getCoreFromID(core_id);
          // LOG_ASSERT_ERROR(core->getState() == Core::RUNNING ||
          // core->getState() == Core::INITIALIZING, "(%i) has acquired barrier,
          // local_clock(%s), m_next_barrier_time(%s), but not initializing or
          // running", core_id, itostr(m_local_clock_list[core_id]).c_str(),
          // itostr(m_next_barrier_time).c_str());
  ...
  // To avoid overwhelming the OS scheduler, we only release N threads at a time
  // (N ~= host cores). Once a thread is done (stops executing because it
  // completed the next barrier quantum, or due to thread stall), one more
  // thread is released so we always have at most N running threads.
...
void BarrierSyncServer::setGroup(core_id_t core_id, core_id_t master_core_id) {
  if (master_core_id != INVALID_CORE_ID)
    LOG_ASSERT_ERROR(
        m_barrier_acquire_list[core_id] == false,
        "Core(%d) is in the barrier, cannot set participate to false", core_id);

  m_core_group[core_id] = master_core_id;
}

void BarrierSyncServer::setFastForward(bool fastforward,
                                       SubsecondTime next_barrier_time) {
