...
RoutineTracerFunctionStats::RtnThread::RtnThread(
    RoutineTracerFunctionStats::RtnMaster *master, Thread *thread)
    : RoutineTracerThread(thread), m_master(master), m_current_eip(0) {}
...
void RoutineTracerFunctionStats::RtnThread::functionBeginHelper(
    IntPtr eip, RtnValues &values_start) {
  m_current_eip = eip;
  const ThreadStatsManager::ThreadStatTypeList &types =
      Sim()->getThreadStatsManager()->getThreadStatTypes();
  for (ThreadStatsManager::ThreadStatTypeList::const_iterator it =
           types.begin();
       it != types.end(); ++it) {
...
void RoutineTracerFunctionStats::RtnThread::functionEndHelper(IntPtr eip,
                                                              UInt64 count) {
  RtnValues values;
  const ThreadStatsManager::ThreadStatTypeList &types =
      Sim()->getThreadStatsManager()->getThreadStatTypes();
...
void RoutineTracerFunctionStats::RtnThread::functionEndFullHelper(
    const CallStack &stack, UInt64 count) {
  RtnValues values;
  const ThreadStatsManager::ThreadStatTypeList &types =
      Sim()->getThreadStatsManager()->getThreadStatTypes();
...
void RoutineTracerFunctionStats::RtnThread::functionBegin(IntPtr eip) {
  ...
  if (m_stack.size()) functionBeginHelper(eip, m_values_start_full[m_stack]);
}

void RoutineTracerFunctionStats::RtnThread::functionEnd(
    IntPtr eip, bool is_function_start) {
  Sim()->getThreadStatsManager()->update(m_thread->getId());

  functionEndHelper(eip, is_function_start ? 1 : 0);
  if (m_stack.size()) functionEndFullHelper(m_stack, is_function_start ? 1 : 0);
}

UInt64 RoutineTracerFunctionStats::RtnThread::getThreadStat(
    ThreadStatsManager::ThreadStatType type) {
  return Sim()->getThreadStatsManager()->getThreadStatistic(m_thread->getId(),
                                                            type);
}
...
RoutineTracerFunctionStats::RtnMaster::RtnMaster() {
  ThreadStatNamedStat::registerStat("fp_addsub", "interval_timer",
                                    "uop_fp_addsub");
  ThreadStatNamedStat::registerStat("fp_muldiv", "interval_timer",
                                    "uop_fp_muldiv");
  ...
  if (ThreadStatNamedStat::registerStat("cpiBase", "interval_timer",
                                        "cpiBase") ==
      ThreadStatsManager::INVALID)
    ThreadStatNamedStat::registerStat("cpiBase", "rob_timer", "cpiBase");
  if (ThreadStatNamedStat::registerStat("cpiBranchPredictor", "interval_timer",
                                        "cpiBranchPredictor") ==
      ThreadStatsManager::INVALID)
    ThreadStatNamedStat::registerStat("cpiBranchPredictor", "rob_timer",
                                      "cpiBranchPredictor");
  ThreadStatCpiMem::registerStat();
  Sim()->getConfig()->setCacheEfficiencyCallbacks(
      __ce_get_owner, NULL, __ce_notify_evict, (UInt64)this);
}

RoutineTracerFunctionStats::RtnMaster::~RtnMaster() {
  writeResults(
      Sim()->getConfig()->formatOutputFileName("sim.rtntrace").c_str());
  writeResultsFull(
      Sim()->getConfig()->formatOutputFileName("sim.rtntracefull").c_str());
}
...
void RoutineTracerFunctionStats::RtnMaster::ce_notify_evict(
    bool on_roi_end, UInt64 owner, UInt64 evictor,
  ...
  RoutineTracerFunctionStats::Routine *rtn =
      (RoutineTracerFunctionStats::Routine *)owner;

  LOG_ASSERT_ERROR(m_routines.count(rtn->m_eip) != 0,
                   "Routine not found (rtn %lx, eip %lx)", rtn, rtn->m_eip);
...
void RoutineTracerFunctionStats::RtnMaster::addRoutine(
    IntPtr eip, const char *name, const char *imgname, IntPtr offset,
    int column, int line, const char *filename) {
  ScopedLock sl(m_lock);

  if (m_routines.count(eip) == 0) {
    m_routines[eip] = new RoutineTracerFunctionStats::Routine(
        eip, name, imgname, offset, column, line, filename);
  } else if (m_routines[eip]->isProvisional()) {
    m_routines[eip]->updateLocation(name, imgname, offset, column, line,
                                    filename);
...
void RoutineTracerFunctionStats::RtnMaster::updateRoutine(IntPtr eip,
                                                          UInt64 calls,
                                                          RtnValues values) {
  ScopedLock sl(m_lock);

  if (m_routines.count(eip) == 0) {
    // Another thread must have done the instrumentation and set the function
    // information, but it's still going through the (SIFT) pipe. Create a
    // provisional record now to hold the statistics, we will update the
    // name/location information once it arrives.
    m_routines[eip] = new RoutineTracerFunctionStats::Routine(
        eip, "(unknown)", "(unknown)", 0, 0, 0, "");
...
RoutineTracerFunctionStats::Routine *
RoutineTracerFunctionStats::RtnMaster::getRoutineFullPtr(
    const CallStack &stack) {
  ScopedLock sl(m_lock);

  if (m_routines.count(stack.back()) == 0) {
    m_routines[stack.back()] = new RoutineTracerFunctionStats::Routine(
        stack.back(), "(unknown)", "(unknown)", 0, 0, 0, "");
    m_routines[stack.back()]->setProvisional(true);
  }

  if (m_callstack_routines.count(stack) == 0) {
    m_callstack_routines[stack] =
        new RoutineTracerFunctionStats::Routine(*m_routines[stack.back()]);
  }
...
void RoutineTracerFunctionStats::RtnMaster::updateRoutineFull(
    const CallStack &stack, UInt64 calls, RtnValues values) {
  updateRoutineFull(getRoutineFullPtr(stack), calls, values);
}

void RoutineTracerFunctionStats::RtnMaster::updateRoutineFull(
    RoutineTracerFunctionStats::Routine *rtn, UInt64 calls, RtnValues values) {
  ScopedLock sl(m_lock);
...
void RoutineTracerFunctionStats::RtnMaster::writeResults(const char *filename) {
  ...
  for (RoutineMap::iterator it = m_routines.begin(); it != m_routines.end();
       ++it) {
    fprintf(fp, "%" PRIxPTR "\t%s\t%s\t%" PRId64 "\t%" PRId64 "\t%" PRId64,
            it->second->m_eip, it->second->m_name, it->second->m_location,
            it->second->m_calls, it->second->m_bits_used,
            it->second->m_bits_total);
    for (ThreadStatsManager::ThreadStatTypeList::const_iterator jt =
             types.begin();
         jt != types.end(); ++jt)
  ...
  for (RoutineMap::iterator it = m_routines.begin(); it != m_routines.end();
    ...
    if (it->second->m_calls)
      fprintf(fp, ":%" PRIxPTR "\t%s\t%s\n", it->second->m_eip,
              it->second->m_name, it->second->m_location);
  }

  // now print context-aware statistics
  for (auto it = m_callstack_routines.begin(); it != m_callstack_routines.end();
       ++it) {
    if (it->second->m_calls) {
      std::ostringstream s;
      ...
      fprintf(fp, "%s\t%" PRId64 "\t%" PRId64 "\t%" PRId64, s.str().c_str(),
              it->second->m_calls, it->second->m_bits_used,
              it->second->m_bits_total);
      for (ThreadStatsManager::ThreadStatTypeList::const_iterator jt =
               types.begin();
...
class ThreadStatAggregates {
 ...
 private:
  static UInt64 callback(ThreadStatsManager::ThreadStatType type,
                         thread_id_t thread_id, Core *core, UInt64 user);
};

void RoutineTracerFunctionStats::ThreadStatAggregates::registerStats() {
  Sim()->getThreadStatsManager()->registerThreadStatMetric(
      ThreadStatsManager::DYNAMIC, "global_instructions", callback,
      (UInt64)GLOBAL_INSTRUCTIONS);
  Sim()->getThreadStatsManager()->registerThreadStatMetric(
      ThreadStatsManager::DYNAMIC, "global_nonidle_elapsed_time", callback,
      (UInt64)GLOBAL_NONIDLE_ELAPSED_TIME);
}

UInt64 RoutineTracerFunctionStats::ThreadStatAggregates::callback(
    ThreadStatsManager::ThreadStatType type, thread_id_t thread_id, Core *core,
    UInt64 user) {
  UInt64 result = 0;
...
ThreadStatsManager::ThreadStatType
RoutineTracerFunctionStats::ThreadStatCpiMem::registerStat() {
  ThreadStatCpiMem *tsns = new ThreadStatCpiMem();
  return Sim()->getThreadStatsManager()->registerThreadStatMetric(
      ThreadStatsManager::DYNAMIC, "cpiMem", callback, (UInt64)tsns);
}
  ...
  for (core_id_t core_id = 0;
    ...
    for (int h = HitWhere::WHERE_FIRST; h < HitWhere::NUM_HITWHERES; h++) {
      if (HitWhereIsValid((HitWhere::where_t)h)) {
        ...
        StatsMetricBase *m = Sim()->getStatsManager()->getMetricObject(
            "interval_timer", core_id, metricName);
        if (!m)
          m = Sim()->getStatsManager()->getMetricObject("rob_timer", core_id,
                                                        metricName);
        LOG_ASSERT_ERROR(m != NULL, "Invalid statistic %s.%d.%s",
                         "interval_timer", core_id, metricName.c_str());
...
UInt64 RoutineTracerFunctionStats::ThreadStatCpiMem::callback(
  ...
  UInt64 result = 0;
  for (std::vector<StatsMetricBase *>::iterator it = stats.begin();
       it != stats.end(); ++it)
