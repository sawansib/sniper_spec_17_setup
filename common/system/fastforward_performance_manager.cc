...
FastForwardPerformanceManager *FastForwardPerformanceManager::create(void) {
...
FastForwardPerformanceManager::FastForwardPerformanceManager()
    : m_sync_interval(SubsecondTime::NS(
          Sim()->getCfg()->getInt("perf_model/fast_forward/oneipc/interval"))),
      m_enabled(false),
      m_target_sync_time(SubsecondTime::Zero()) {
  Sim()->getHooksManager()->registerHook(
      HookType::HOOK_INSTR_COUNT,
      FastForwardPerformanceManager::hook_instr_count, (UInt64)this);
  Sim()->getHooksManager()->registerHook(
      HookType::HOOK_PERIODIC, FastForwardPerformanceManager::hook_periodic,
      (UInt64)this);
}
...
void FastForwardPerformanceManager::enable() {
  ...
  // Make sure all threads are released, as the barrier has just changed from
  // per-core to per-HW-context mode
...
void FastForwardPerformanceManager::disable() {
  ...
  for (unsigned int core_id = 0;
    ...
    core->disablePerformanceModels();  // Probably someone else will turn them
                                       // on again soon, but let's do this
                                       // anyway
    core->getPerformanceModel()->setFastForward(false);
    core->disableInstructionsCallback();
    if (core->getThread() && Sim()->getThreadManager()->isThreadRunning(
  ...
  // If some threads were behind but still caught the barrier (everyone does in
  // fast-forward mode), make sure to release them so they have a chance to make
  // progress and hit the barrier again at the proper time.
  Sim()->getClockSkewMinimizationServer()->release();
}
...
void FastForwardPerformanceManager::recalibrateInstructionsCallback(
  ...
  if (now > m_target_sync_time) {
    ...
    // One IPC
    SubsecondTime cpi = Sim()
                            ->getCoreManager()
                            ->getCoreFromID(core_id)
                            ->getDvfsDomain()
                            ->getPeriod();
    UInt64 ninstrs =
        SubsecondTime::divideRounded(m_target_sync_time - now, cpi);
