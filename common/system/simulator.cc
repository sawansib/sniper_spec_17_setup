...
void Simulator::start() {
  ...
  if (m_trace_manager) m_trace_manager->init();

  m_sim_thread_manager->spawnSimThreads();

  Instruction::initializeStaticInstructionModel();

  InstMode::inst_mode_init =
      InstMode::fromString(getCfg()->getString("general/inst_mode_init"));
  InstMode::inst_mode_roi =
      InstMode::fromString(getCfg()->getString("general/inst_mode_roi"));
  InstMode::inst_mode_end =
      InstMode::fromString(getCfg()->getString("general/inst_mode_end"));
...
Simulator::~Simulator() {
  ...
  // In case we're still in ROI (ROI is the full application, or someone forgot
  // to turn it off), end ROI now
...
void Simulator::enablePerformanceModels() {
  if (Sim()->getFastForwardPerformanceManager() &&
      InstMode::inst_mode_roi == InstMode::DETAILED)
...
void Simulator::setInstrumentationMode(InstMode::inst_mode_t new_mode,
  ...
  if (new_mode != InstMode::inst_mode) {
    ...
    // If there is a fast-forward performance model, it needs to take care of
    // barrier synchronization. If we're called with update_barrier == false,
    // the caller (SamplingManager) manages the barrier. Else, disable the
    // barrier in fast-forward/cache-only
    if (update_barrier && !Sim()->getFastForwardPerformanceManager())
      getClockSkewMinimizationServer()->setDisable(new_mode !=
                                                   InstMode::DETAILED);

    Sim()->getHooksManager()->callHooks(HookType::HOOK_INSTRUMENT_MODE,
                                        (UInt64)new_mode);
  }
}
...
void Simulator::printInstModeSummary() {
  ...
  switch (getConfig()->getSimulationROI()) {
    ...
    case Config::ROI_SCRIPT:
      printf(
          "[SNIPER] Running in script-driven instrumenation mode "
          "(--roi-script)\n");
