 ...
 public:
  ...
  class ThreadStats {
   ...
   private:
    ...
    std::unordered_map<ThreadStatType, UInt64>
        m_counts;  // Running total of thread statistics
    std::unordered_map<ThreadStatType, UInt64>
        m_last;  // Snapshot of core's statistics when we last updated m_current

    friend class ThreadStatsManager;
  };
  typedef UInt64 (*ThreadStatCallback)(ThreadStatType type,
  ...
  // Thread statistics are updated lazily (on thread move and before statistics
  // writing), call this function to force an update before reading
  void update(thread_id_t thread_id = INVALID_THREAD_ID,
  ...
  ThreadStatType registerThreadStatMetric(ThreadStatType type, const char *name,
 ...
 private:
  struct StatCallback {
    ...
    StatCallback(const char *name, ThreadStatCallback func, UInt64 user)
        : m_name(name), m_func(func), m_user(user) {}
    UInt64 call(ThreadStatType type, thread_id_t thread_id, Core *core) {
      return m_func(type, thread_id, core, m_user);
    }
  };
  // Make sure m_threads_stats is statically allocated, as we may do inserts and
  // reads simultaneously which does not work on an unordered_map
  ...
  static UInt64 metricCallback(ThreadStatType type, thread_id_t thread_id,
                               Core *core, UInt64 user);
  UInt64 callThreadStatCallback(ThreadStatType type, thread_id_t thread_id,
                                Core *core);

  void pre_stat_write();
  void threadCreate(thread_id_t thread_id);
  void threadStart(thread_id_t thread_id, SubsecondTime time);
  void threadStall(thread_id_t thread_id, ThreadManager::stall_type_t reason,
                   SubsecondTime time);
  void threadResume(thread_id_t thread_id, thread_id_t thread_by,
                    SubsecondTime time);
  ...
  static SInt64 hook_thread_stall(UInt64 ptr, UInt64 _args) {
    ...
    ((ThreadStatsManager *)ptr)
        ->threadStall(args->thread_id, args->reason, args->time);
    return 0;
  }
  static SInt64 hook_thread_resume(UInt64 ptr, UInt64 _args) {
    HooksManager::ThreadResume *args = (HooksManager::ThreadResume *)_args;
    ((ThreadStatsManager *)ptr)
        ->threadResume(args->thread_id, args->thread_by, args->time);
    return 0;
  }
...
// Caches the StatsMetricBase objects to avoid the string lookups on every
// access.
class ThreadStatNamedStat {
 public:
  static ThreadStatsManager::ThreadStatType registerStat(const char *name,
                                                         String objectName,
                                                         String metricName);
