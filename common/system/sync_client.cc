...
SyncClient::SyncClient(Thread *thread)
    : m_thread(thread), m_server(Sim()->getSyncServer()) {}
...
std::pair<SubsecondTime, bool> SyncClient::__mutexLock(carbon_mutex_t *mux,
                                                       bool tryLock,
                                                       SubsecondTime delay) {
  Thread *thread = Sim()->getThreadManager()->getCurrentThread();
  Core *core = thread->getCore();
  SubsecondTime start_time =
      core->getPerformanceModel()->getElapsedTime() + delay;

  std::pair<SubsecondTime, bool> result =
      m_server->mutexLock(thread->getId(), mux, tryLock, start_time);

  if (thread->reschedule(result.first, core)) core = thread->getCore();

  core->getPerformanceModel()->queuePseudoInstruction(
      new SyncInstruction(result.first, SyncInstruction::PTHREAD_MUTEX));

  return std::pair<SubsecondTime, bool>(result.first > start_time
                                            ? result.first - start_time
                                            : SubsecondTime::Zero(),
                                        result.second);
}

SubsecondTime SyncClient::mutexUnlock(carbon_mutex_t *mux,
                                      SubsecondTime delay) {
  Thread *thread = Sim()->getThreadManager()->getCurrentThread();
  SubsecondTime start_time =
      thread->getCore()->getPerformanceModel()->getElapsedTime() + delay;
...
SubsecondTime SyncClient::condWait(carbon_cond_t *cond, carbon_mutex_t *mux) {
  ...
  SubsecondTime time =
      m_server->condWait(thread->getId(), cond, mux, start_time);

  if (thread->reschedule(time, core)) core = thread->getCore();

  core->getPerformanceModel()->queuePseudoInstruction(
      new SyncInstruction(time, SyncInstruction::PTHREAD_COND));
...
SubsecondTime SyncClient::condSignal(carbon_cond_t *cond) {
  Thread *thread = Sim()->getThreadManager()->getCurrentThread();
  SubsecondTime start_time =
      thread->getCore()->getPerformanceModel()->getElapsedTime();
...
SubsecondTime SyncClient::barrierWait(carbon_barrier_t *barrier) {
  ...
  SubsecondTime time =
      m_server->barrierWait(thread->getId(), barrier, start_time);

  if (thread->reschedule(time, core)) core = thread->getCore();

  core->getPerformanceModel()->queuePseudoInstruction(
      new SyncInstruction(time, SyncInstruction::PTHREAD_BARRIER));
