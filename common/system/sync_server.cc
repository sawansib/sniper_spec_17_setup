...
SubsecondTime SimMutex::lock(thread_id_t thread_id, SubsecondTime time) {
  if (m_owner == NO_OWNER) {
    ...
    return Sim()->getThreadManager()->stallThread(
        thread_id, ThreadManager::STALL_MUTEX, time);
  }
}

bool SimMutex::lock_async(thread_id_t thread_id, thread_id_t thread_by,
                          SubsecondTime time) {
  if (m_owner == NO_OWNER) {
    m_owner = thread_id;
...
SubsecondTime SimCond::wait(thread_id_t thread_id, SubsecondTime time,
                            SimMutex *simMux) {
  ...
  return Sim()->getThreadManager()->stallThread(
      thread_id, ThreadManager::STALL_COND, time);
}
...
SubsecondTime SimBarrier::wait(thread_id_t thread_id, SubsecondTime time) {
  ...
  if (m_waiting.size() == m_count - 1) {
    ...
    m_waiting.push(thread_id);
    return Sim()->getThreadManager()->stallThread(
        thread_id, ThreadManager::STALL_BARRIER, time);
  }
}
...
SimMutex *SyncServer::getMutex(carbon_mutex_t *mux, bool canCreate) {
  // if mux is the address of a pthread mutex (with default initialization, not
  // through pthread_mutex_init), look it up in m_mutexes or create a new one if
...
std::pair<SubsecondTime, bool> SyncServer::mutexLock(thread_id_t thread_id,
                                                     carbon_mutex_t *mux,
                                                     bool tryLock,
                                                     SubsecondTime time) {
  ScopedLock sl(Sim()->getThreadManager()->getLock());
...
SubsecondTime SyncServer::mutexUnlock(thread_id_t thread_id,
                                      carbon_mutex_t *mux, SubsecondTime time) {
  ScopedLock sl(Sim()->getThreadManager()->getLock());
  ...
  SubsecondTime new_time =
      time + (new_owner == SimMutex::NO_OWNER
                  ? SubsecondTime::Zero()
                  : m_reschedule_cost /* we had to call futex_wake */);
  return new_time;
}

// -- Condition Variable Stuffs -- //
SimCond *SyncServer::getCond(carbon_cond_t *cond, bool canCreate) {
  // if cond is the address of a pthread cond (with default initialization, not
  // through pthread_cond_init), look it up in m_conds or create a new one if
  // it's the first time we see it
...
SubsecondTime SyncServer::condWait(thread_id_t thread_id, carbon_cond_t *cond,
                                   carbon_mutex_t *mux, SubsecondTime time) {
  ScopedLock sl(Sim()->getThreadManager()->getLock());
  SimMutex *psimmux = getMutex(mux);
...
SubsecondTime SyncServer::condSignal(thread_id_t thread_id, carbon_cond_t *cond,
                                     SubsecondTime time) {
  ScopedLock sl(Sim()->getThreadManager()->getLock());
  SimCond *psimcond = getCond(cond);
...
SubsecondTime SyncServer::condBroadcast(thread_id_t thread_id,
                                        carbon_cond_t *cond,
                                        SubsecondTime time) {
  ScopedLock sl(Sim()->getThreadManager()->getLock());
  SimCond *psimcond = getCond(cond);
...
void SyncServer::barrierInit(thread_id_t thread_id, carbon_barrier_t *barrier,
                             UInt32 count) {
  ScopedLock sl(Sim()->getThreadManager()->getLock());
...
SubsecondTime SyncServer::barrierWait(thread_id_t thread_id,
