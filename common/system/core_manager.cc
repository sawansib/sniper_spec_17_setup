...
CoreManager::~CoreManager() {
  for (std::vector<Core *>::iterator i = m_cores.begin(); i != m_cores.end();
       i++)
    delete *i;
...
void CoreManager::initializeCommId(SInt32 comm_id) {
  LOG_PRINT("initializeCommId - current core (id) = %p (%d)", getCurrentCore(),
            getCurrentCoreID());

  core_id_t core_id = getCurrentCoreID();

  LOG_ASSERT_ERROR(core_id != INVALID_CORE_ID,
                   "Unexpected invalid core id : %d", core_id);
...
void CoreManager::initializeThread(core_id_t core_id) {
  ...
  LOG_PRINT("Initialize thread for core %p (%d)", m_cores.at(core_id),
            m_cores.at(core_id)->getId());
  LOG_ASSERT_ERROR(m_core_tls->get() == (void *)(m_cores.at(core_id)),
                   "TLS appears to be broken. %p != %p", m_core_tls->get(),
                   (void *)(m_cores.at(core_id)));
}

void CoreManager::terminateThread() {
  LOG_ASSERT_WARNING(m_core_tls->get() != NULL,
                     "Thread not initialized while terminating.");
  m_core_tls->set(NULL);
}

Core *CoreManager::getCoreFromID(core_id_t id) {
  LOG_ASSERT_ERROR(id < (core_id_t)Config::getSingleton()->getTotalCores(),
                   "Illegal index in getCoreFromID!");
...
core_id_t CoreManager::registerSimThread(ThreadType type) {
  ...
  LOG_ASSERT_ERROR(
      *num_registered_threads < Config::getSingleton()->getTotalCores(),
