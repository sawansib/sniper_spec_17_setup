...
class BarrierSyncServer : public ClockSkewMinimizationServer {
 private:
  ...
  bool barrierRelease(thread_id_t thread_id = INVALID_THREAD_ID,
                      bool continue_until_release = false);
  ...
  static SInt64 hookThreadExit(UInt64 object, UInt64 argument) {
    ((BarrierSyncServer *)object)
        ->threadExit((HooksManager::ThreadTime *)argument);
    return 0;
  }
  static SInt64 hookThreadStall(UInt64 object, UInt64 argument) {
    ((BarrierSyncServer *)object)
        ->threadStall((HooksManager::ThreadStall *)argument);
    return 0;
  }
 ...
 public:
  ...
  void advance();
  void setFastForward(bool fastforward, SubsecondTime next_barrier_time =
                                            SubsecondTime::MaxTime());
  SubsecondTime getGlobalTime(bool upper_bound = false) {
    return upper_bound ? m_next_barrier_time : m_global_time;
  }
  void setBarrierInterval(SubsecondTime barrier_interval) {
    m_barrier_interval = barrier_interval;
  }
