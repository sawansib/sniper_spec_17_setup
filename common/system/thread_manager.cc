...
const char *ThreadManager::stall_type_names[] = {
    "unscheduled", "broken", "join",  "mutex", "cond",
    "barrier",     "futex",  "pause", "sleep", "syscall"};
static_assert(ThreadManager::STALL_TYPES_MAX ==
                  sizeof(ThreadManager::stall_type_names) / sizeof(char *),
              "Not enough values in ThreadManager::stall_type_names");

ThreadManager::ThreadManager()
    : m_thread_tls(TLS::create()), m_scheduler(Scheduler::create(this)) {}
...
Thread *ThreadManager::getThreadFromID(thread_id_t thread_id) {
  LOG_ASSERT_ERROR((size_t)thread_id < m_threads.size(), "Invalid thread_id %d",
                   thread_id);
...
Thread *ThreadManager::createThread_unlocked(app_id_t app_id,
  ...
  Sim()->getStatsManager()->logEvent(StatsManager::EVENT_THREAD_CREATE,
                                     SubsecondTime::MaxTime(), core_id,
                                     thread_id, app_id, creator_thread_id, "");

  HooksManager::ThreadCreate args = {
    thread_id : thread_id,
    creator_thread_id : creator_thread_id
  };
  Sim()->getHooksManager()->callHooks(HookType::HOOK_THREAD_CREATE,
                                      (UInt64)&args);
...
void ThreadManager::onThreadStart(thread_id_t thread_id, SubsecondTime time) {
  ...
  // Set thread state to running for the duration of HOOK_THREAD_START, we'll
  // move it to stalled later on if it didn't have a core
  m_thread_state[thread_id].status = Core::RUNNING;

  HooksManager::ThreadTime args = {thread_id : thread_id, time : time};
  Sim()->getHooksManager()->callHooks(HookType::HOOK_THREAD_START,
                                      (UInt64)&args);
  // Note: we may have been rescheduled during HOOK_THREAD_START
  // (Happens if core was occupied during our createThread() but became free
  // since then)
  ...
  if (core) {
    ...
    HooksManager::ThreadMigrate args = {
      thread_id : thread_id,
      core_id : core->getId(),
      time : time
    };
    Sim()->getHooksManager()->callHooks(HookType::HOOK_THREAD_MIGRATE,
                                        (UInt64)&args);
  } else {
...
void ThreadManager::onThreadExit(thread_id_t thread_id) {
  ...
  LOG_ASSERT_ERROR((UInt32)thread_id < m_thread_state.size(),
                   "Thread id out of range: %d", thread_id);
  ...
  if (thread->m_os_info.clear_tid) {
    uint32_t zero = 0;
    core->accessMemory(Core::NONE, Core::WRITE, thread->m_os_info.tid_ptr,
                       (char *)&zero, sizeof(zero));

    SubsecondTime end_time;  // ignored
    Sim()->getSyscallServer()->futexWake(
        thread_id, (int *)thread->m_os_info.tid_ptr, 1, FUTEX_BITSET_MATCH_ANY,
        time, end_time);
  }
  ...
  Sim()->getStatsManager()->logEvent(StatsManager::EVENT_THREAD_EXIT,
                                     SubsecondTime::MaxTime(), core->getId(),
                                     thread_id, 0, 0, "");

  HooksManager::ThreadTime args = {thread_id : thread_id, time : time};
  Sim()->getHooksManager()->callHooks(HookType::HOOK_THREAD_EXIT,
                                      (UInt64)&args);
...
void ThreadManager::waitForThreadStart(thread_id_t thread_id,
...
Thread *ThreadManager::getThreadOnCore(core_id_t core_id) {
  ...
  for (UInt32 thread_id = 0; thread_id < m_threads.size(); ++thread_id) {
    ...
    Core *core = thread->getCore();
    if (!core) continue;
    if (core->getId() == core_id) return thread;
  }
  return NULL;
}

void ThreadManager::moveThread(thread_id_t thread_id, core_id_t core_id,
                               SubsecondTime time) {
  Thread *thread = getThreadFromID(thread_id);
  CLOG("thread", "Move %d from %d to %d", thread_id,
       thread->getCore() ? thread->getCore()->getId() : -1, core_id);

  if (Core *core = thread->getCore()) core->setState(Core::IDLE);
  ...
  if (core_id == INVALID_CORE_ID) {
    ...
    if (thread->getCore() == NULL) {
      ...
      if (m_thread_state[thread_id].status == Core::STALLED &&
          m_thread_state[thread_id].stalled_reason == STALL_UNSCHEDULED)
...
void ThreadManager::joinThread(thread_id_t thread_id,
  ...
  if (thread->reschedule(end_time, core)) core = thread->getCore();

  core->getPerformanceModel()->queuePseudoInstruction(
      new SyncInstruction(end_time, SyncInstruction::JOIN));
...
void ThreadManager::wakeUpWaiter(thread_id_t thread_id, SubsecondTime time) {
  if (m_thread_state[thread_id].waiter != INVALID_THREAD_ID) {
    LOG_PRINT("Waking up core: %d at time: %s",
              m_thread_state[thread_id].waiter, itostr(time).c_str());
...
void ThreadManager::stallThread_async(thread_id_t thread_id,
                                      stall_type_t reason, SubsecondTime time) {
  LOG_PRINT("Core(%i) -> STALLED", thread_id);
  m_thread_state[thread_id].status = Core::STALLED;
  m_thread_state[thread_id].stalled_reason = reason;

  HooksManager::ThreadStall args = {
    thread_id : thread_id,
    reason : reason,
    time : time
  };
  Sim()->getHooksManager()->callHooks(HookType::HOOK_THREAD_STALL,
                                      (UInt64)&args);
  CLOG("thread", "Stall %d (%s)", thread_id,
       ThreadManager::stall_type_names[reason]);
}

SubsecondTime ThreadManager::stallThread(thread_id_t thread_id,
                                         stall_type_t reason,
                                         SubsecondTime time) {
  stallThread_async(thread_id, reason, time);
  // When all threads are stalled, we have a deadlock -- unless we let the
  // barrier advance time which may wake up threads that are sleeping or waiting
  // on a futex with a timeout value.
  while (!anyThreadRunning()) {
    Sim()->getClockSkewMinimizationServer()->advance();
  }
  // It's possible that a HOOK_PERIODIC, called by SkewMinServer::signal(),
  // called by stallThread_async(), woke us up again. We will then have been
  // signal()d, but this signal was lost since we weren't in wait() If this is
  // the case, don't go to sleep but return our wakeup time immediately
...
void ThreadManager::resumeThread_async(thread_id_t thread_id,
                                       thread_id_t thread_by,
                                       SubsecondTime time, void *msg) {
  LOG_PRINT("Core(%i) -> RUNNING", thread_id);
  m_thread_state[thread_id].status = Core::RUNNING;

  HooksManager::ThreadResume args = {
    thread_id : thread_id,
    thread_by : thread_by,
    time : time
  };
  Sim()->getHooksManager()->callHooks(HookType::HOOK_THREAD_RESUME,
                                      (UInt64)&args);
  CLOG("thread", "Resume %d (by %d)", thread_id, thread_by);
}

void ThreadManager::resumeThread(thread_id_t thread_id, thread_id_t thread_by,
                                 SubsecondTime time, void *msg) {
  // We still have the m_thread_lock, so thread doesn't actually start running
  // again until caller releases this lock
...
bool ThreadManager::anyThreadRunning() {
  for (thread_id_t thread_id = 0; thread_id < (thread_id_t)getNumThreads();
       ++thread_id) {
