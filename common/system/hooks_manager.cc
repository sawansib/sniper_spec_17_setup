...
static_assert(HookType::HOOK_TYPES_MAX ==
                  sizeof(HookType::hook_type_names) /
                      sizeof(HookType::hook_type_names[0]),
              "Not enough values in HookType::hook_type_names");

HooksManager::HooksManager() {}

void HooksManager::registerHook(HookType::hook_type_t type,
                                HookCallbackFunc func, UInt64 argument,
                                HookCallbackOrder order) {
  m_registry[type].push_back(HookCallback(func, argument, order));
}

SInt64 HooksManager::callHooks(HookType::hook_type_t type, UInt64 arg,
                               bool expect_return) {
  for (unsigned int order = 0; order < NUM_HOOK_ORDER; ++order) {
    for (std::vector<HookCallback>::iterator it = m_registry[type].begin();
         it != m_registry[type].end(); ++it) {
      if (it->order == (HookCallbackOrder)order) {
        SInt64 result = it->func(it->arg, arg);
        if (expect_return && result != -1) return result;
      }
    }
  }
