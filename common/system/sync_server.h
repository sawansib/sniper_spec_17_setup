...
class SimMutex {
 public:
  ...
  // try to take the lock in name of another thread, either waking them or
  // adding them to the list
  bool lock_async(thread_id_t thread_id, thread_id_t thread_by,
                  SubsecondTime time);
...
class SyncServer {
 ...
 public:
  ...
  std::pair<SubsecondTime, bool> mutexLock(thread_id_t thread_id,
                                           carbon_mutex_t *mux, bool tryLock,
                                           SubsecondTime time);
  SubsecondTime mutexUnlock(thread_id_t thread_id, carbon_mutex_t *mux,
                            SubsecondTime time);

  void condInit(thread_id_t thread_id, carbon_cond_t *cond);
  SubsecondTime condWait(thread_id_t thread_id, carbon_cond_t *cond,
                         carbon_mutex_t *mux, SubsecondTime time);
  SubsecondTime condSignal(thread_id_t thread_id, carbon_cond_t *cond,
                           SubsecondTime time);
  SubsecondTime condBroadcast(thread_id_t thread_id, carbon_cond_t *cond,
                              SubsecondTime time);

  void barrierInit(thread_id_t thread_id, carbon_barrier_t *barrier,
                   UInt32 count);
  SubsecondTime barrierWait(thread_id_t thread_id, carbon_barrier_t *barrier,
                            SubsecondTime time);
