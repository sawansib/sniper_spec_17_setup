...
class ThreadManager {
 public:
  ...
  Core::State getThreadState(thread_id_t thread_id) const {
    return m_thread_state.at(thread_id).status;
  }
  stall_type_t getThreadStallReason(thread_id_t thread_id) const {
    return m_thread_state.at(thread_id).stalled_reason;
  }
  ...
  // misc
  SubsecondTime stallThread(thread_id_t thread_id, stall_type_t reason,
                            SubsecondTime time);
  void stallThread_async(thread_id_t thread_id, stall_type_t reason,
                         SubsecondTime time);
  void resumeThread(thread_id_t thread_id, thread_id_t thread_id_by,
                    SubsecondTime time, void *msg = NULL);
  void resumeThread_async(thread_id_t thread_id, thread_id_t thread_id_by,
                          SubsecondTime time, void *msg = NULL);
