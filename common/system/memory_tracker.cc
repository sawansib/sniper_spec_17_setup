...
MemoryTracker::MemoryTracker() {
  Sim()->getConfig()->setCacheEfficiencyCallbacks(
      __ce_get_owner, __ce_notify_access, __ce_notify_evict, (UInt64)this);
}

MemoryTracker::~MemoryTracker() {
  FILE *fp = fopen(
      Sim()->getConfig()->formatOutputFileName("sim.memorytracker").c_str(),
      "w");
  ...
  for (auto it = m_allocation_sites.begin(); it != m_allocation_sites.end();
       ++it) {
    ...
    if (site->total_loads + site->total_stores) {
      for (auto jt = stack.begin(); jt != stack.end(); ++jt) {
        if (sites_printed.count(*jt) == 0) {
          const RoutineTracer::Routine *rtn =
              Sim()->getRoutineTracer()->getRoutineInfo(*jt);
          if (rtn)
      ...
      for (int h = HitWhere::WHERE_FIRST; h < HitWhere::NUM_HITWHERES; h++) {
        if (HitWhereIsValid((HitWhere::where_t)h) &&
            site->hit_where_load[h] + site->hit_where_store[h] > 0) {
          fprintf(fp, "L%s:%" PRId64 ",", HitWhereString((HitWhere::where_t)h),
                  site->hit_where_load[h]);
          fprintf(fp, "S%s:%" PRId64 ",", HitWhereString((HitWhere::where_t)h),
                  site->hit_where_store[h]);
        }
      }
      if (site->evicted_by.size()) {
        fprintf(fp, "\tevicted-by=");
        for (auto it = site->evicted_by.begin(); it != site->evicted_by.end();
             ++it)
...
void MemoryTracker::logMalloc(thread_id_t thread_id, UInt64 eip, UInt64 address,
                              UInt64 size) {
  ScopedLock sl(m_lock);

  ::RoutineTracerThread *tracer =
      Sim()->getThreadManager()->getThreadFromID(thread_id)->getRoutineTracer();
  const CallStack stack =
      dynamic_cast<MemoryTracker::RoutineTracerThread *>(tracer)
          ->getCallsiteStack();
  ...
  // Store the first address of the first cache line that no longer belongs to
  // the allocation
  UInt64 lower = address & ~63, upper = (address + size + 63) & ~63;

  // printf("memtracker: site %p(%lx) malloc %lx + %10lx (%lx .. %lx)\n", site,
  // eip, address, size, lower, upper);

  auto previous = m_allocations.upper_bound(lower);
  if (previous != m_allocations.end() &&
      lower >= previous->first - previous->second.size) {
    ...
    if (previous->first - previous->second.size < lower) {
      ...
      // printf("\tremain %p %lx .. %lx\n", previous->second.site, start,
      // lower);
    }
    if (previous->first > upper) {
      m_allocations[previous->first] =
          Allocation(previous->first - upper, previous->second.site);
      // printf("\tremain %p %lx .. %lx\n", previous->second.site, upper,
      // previous->first);
    } else {
...
UInt64 MemoryTracker::ce_get_owner(core_id_t core_id, UInt64 address) {
  ...
  // Because the key in m_allocations is the first cache line that no longer
  // falls into the range, we will find the correct alloction *if* address falls
  // within it
  auto upper = m_allocations.upper_bound(address);
  if (upper != m_allocations.end() &&
      address >= upper->first - upper->second.size)
    owner = upper->second.site;

#ifdef ASSERT_FIND_OWNER
  AllocationSite *owner_slow = (m_allocations_slow.count(address & ~63) == 0)
                                   ? NULL
                                   : m_allocations_slow[address & ~63];
  LOG_ASSERT_WARNING(
      owner == owner_slow,
      "ASSERT_FIND_OWNER: owners for %lx don't match (fast %p != slow %p)",
      address, owner, owner_slow);
#endif

  return (UInt64)owner;
}

void MemoryTracker::ce_notify_access(UInt64 owner, Core::mem_op_t mem_op_type,
...
void MemoryTracker::ce_notify_evict(bool on_roi_end, UInt64 owner,
                                    UInt64 evictor,
...
void MemoryTracker::RoutineTracer::addRoutine(IntPtr eip, const char *name,
                                              const char *imgname,
                                              IntPtr offset, int column,
                                              int line, const char *filename) {
  ScopedLock sl(m_lock);

  if (m_routines.count(eip) == 0) {
    m_routines[eip] = new RoutineTracer::Routine(eip, name, imgname, offset,
...
void MemoryTracker::RoutineTracerThread::functionEnter(IntPtr eip,
                                                       IntPtr callEip) {
