...
RoutineTracerThread::RoutineTracerThread(Thread *thread) : m_thread(thread) {
  Sim()->getHooksManager()->registerHook(HookType::HOOK_ROI_BEGIN,
                                         __hook_roi_begin, (UInt64)this);
  Sim()->getHooksManager()->registerHook(HookType::HOOK_ROI_END, __hook_roi_end,
                                         (UInt64)this);
}
...
void RoutineTracerThread::routineEnter_unlocked(IntPtr eip, IntPtr esp,
  ...
  if (Sim()->getMagicServer()->inROI()) functionEnter(eip, callEip);
}
...
void RoutineTracerThread::routineExit(IntPtr eip, IntPtr esp) {
  ...
  bool found = true;
  if (m_stack.back() != eip) {
    // If we are returning from a function that's not at the top of the stack,
    // search for it further down
    found = unwindTo(eip);
  }
  ...
  if (!found) {
    ...
    if (Sim()->getMagicServer()->inROI()) functionExit(eip);
    m_stack.pop_back();
  }
...
void RoutineTracerThread::routineAssert(IntPtr eip, IntPtr esp) {
  ...
  if (m_stack.size() == 0) {
    ...
    // Stack grew (downwards), or stayed constant (tail call): we entered a new
    // function
    routineEnter_unlocked(eip, esp, 0);
  } else {
    ...
    if (!found) {
      // We now seem to be in a function we haven't been before, enter it (tail
      // call elimination?)
      routineEnter_unlocked(eip, esp, 0);
    } else {
    ...
    LOG_ASSERT_ERROR(m_stack.back() == eip,
                     "Expected to be in function %lx but now in %lx", eip,
                     m_stack.back());
  }
}

bool RoutineTracerThread::unwindTo(IntPtr eip) {
  for (CallStack::reverse_iterator it = m_stack.rbegin(); it != m_stack.rend();
       ++it) {
    if (*it == eip) {
      // We found this eip further down the stack: unwind
      while (m_stack.back() != eip) {
        if (Sim()->getMagicServer()->inROI()) functionExit(m_stack.back());
        m_stack.pop_back();
        if (Sim()->getMagicServer()->inROI())
          functionChildExit(m_stack.back(), eip);
      }
...
void RoutineTracerThread::hookRoiBegin() {
  ...
  for (CallStack::iterator it = m_stack.begin(); it != m_stack.end(); ++it) {
    if (eip_parent) functionChildEnter(eip_parent, *it);
    functionEnter(*it, 0);
    eip_parent = *it;
  }
}
...
void RoutineTracerThread::hookRoiEnd() {
  ...
  // Since functionExit might use m_stack we need to keep it up-to-date by
  // popping items off, we'll use stack_save to remember them and restore
  // m_stack to its original state on exit.
  CallStack stack_save;
  IntPtr eip_child = 0;

  while (m_stack.size()) {
    if (eip_child) functionChildExit(m_stack.back(), eip_child);
...
RoutineTracer::Routine::Routine(IntPtr eip, const char *name,
                                const char *imgname, IntPtr offset, int column,
...
void RoutineTracer::Routine::updateLocation(const char *name,
                                            const char *imgname, IntPtr offset,
                                            int column, int line,
                                            const char *filename) {
  if (m_name) free(m_name);
  if (m_imgname) free(m_imgname);
  ...
  char location[4096];
  snprintf(location, 4095, "%s:%" PRIdPTR ":%s:%d:%d", imgname, offset,
