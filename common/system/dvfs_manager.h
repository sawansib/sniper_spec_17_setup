...
// Each process has a copy of all global frequencies, and of the core
// frequencies local to that process In addition, process 0 has a copy of all
// core frequencies so as to quickly fulfill queries from the MCP/scripts

class DvfsManager {
 public:
  enum DvfsGlobalDomain {
    DOMAIN_GLOBAL_DEFAULT,
    // If we wanted separate domains for e.g. DRAM, add them here and initialize
    // them in DvfsManager::DvfsManager()
    DOMAIN_GLOBAL_MAX
  };
  DvfsManager();
  UInt32 getCoreDomainId(UInt32 core_id);
  const ComponentPeriod* getCoreDomain(UInt32 core_id);
  const ComponentPeriod* getGlobalDomain(
