...
SimThreadManager::SimThreadManager() : m_active_threads(0) {}
...
void SimThreadManager::quitSimThreads() {
  ...
  NetPacket pkt1(SubsecondTime::Zero(), SIM_THREAD_TERMINATE_THREADS, 0, 0, 0,
                 NULL);
  NetPacket pkt2(SubsecondTime::Zero(), CORE_THREAD_TERMINATE_THREADS, 0, 0, 0,
                 NULL);

  for (core_id_t core_id = 0;
       core_id < (core_id_t)Config::getSingleton()->getTotalCores();
       core_id++) {
#ifdef ENABLE_PERF_MODEL_OWN_THREAD
    // First kill core thread (needs network thread to be alive to deliver the
    // message)
  ...
  while (m_active_threads > 0) sched_yield();
