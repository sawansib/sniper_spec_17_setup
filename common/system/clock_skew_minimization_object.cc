...
ClockSkewMinimizationObject::Scheme ClockSkewMinimizationObject::parseScheme(
  ...
  if (scheme == "barrier")
    ...
    config::Error("Unrecognized clock skew minimization scheme: %s",
                  scheme.c_str());
  }
}

ClockSkewMinimizationClient* ClockSkewMinimizationClient::create(Core* core) {
...
ClockSkewMinimizationManager* ClockSkewMinimizationManager::create() {
  Scheme scheme = Sim()->getConfig()->getClockSkewMinimizationScheme();
...
ClockSkewMinimizationServer* ClockSkewMinimizationServer::create() {
  Scheme scheme = Sim()->getConfig()->getClockSkewMinimizationScheme();
...
SubsecondTime ClockSkewMinimizationServer::getGlobalTime(bool upper_bound) {
  LOG_PRINT_ERROR(
      "This clock skew minimization server does not support getGlobalTime");
}
