...
static UInt64 handleMagic(thread_id_t thread_id, UInt64 cmd, UInt64 arg0 = 0,
                          UInt64 arg1 = 0) {
  Thread *thread = (thread_id == INVALID_THREAD_ID)
                       ? NULL
                       : Sim()->getThreadManager()->getThreadFromID(thread_id);
  Core *core = thread == NULL ? NULL : thread->getCore();
  return Sim()->getMagicServer()->Magic(
      thread_id, core ? core->getId() : INVALID_CORE_ID, cmd, arg0, arg1);
}
...
UInt64 handleMagicInstruction(thread_id_t thread_id, UInt64 cmd, UInt64 arg0,
                              UInt64 arg1) {
  switch (cmd) {
    ...
    case SIM_CMD_IN_SIMULATOR:
      return 0;
    default:
      LOG_PRINT_WARNING_ONCE("Encountered unknown magic instruction cmd(%u)",
                             cmd);
      return 1;
  }
}
