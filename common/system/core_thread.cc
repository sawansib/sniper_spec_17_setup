...
CoreThread::~CoreThread() { delete m_thread; }

void CoreThread::run() {
  core_id_t core_id =
      Sim()->getCoreManager()->registerSimThread(CoreManager::CORE_THREAD);
  ...
  net->registerCallback(CORE_THREAD_TERMINATE_THREADS, terminateFunc,
                        (void *)&cont);

  PerformanceModel *prfmdl =
      Sim()->getCoreManager()->getCurrentCore()->getPerformanceModel();
  while (cont) {
    prfmdl->iterate();
    usleep(
        1000);  // Reduce system load while there's nothing to do (outside ROI)
  }
