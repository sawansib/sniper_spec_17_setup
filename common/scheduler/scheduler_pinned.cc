...
SchedulerPinned::SchedulerPinned(ThreadManager *thread_manager)
  ...
  for (core_id_t core_id = 0;
       ...
       core_id++) {
    m_core_mask[core_id] =
        Sim()->getCfg()->getBoolArray("scheduler/pinned/core_mask", core_id);
  }
}
...
core_id_t SchedulerPinned::getNextCore(core_id_t core_id) {
  while (true) {
    ...
    if (m_core_mask[core_id]) return core_id;
  }
}
...
core_id_t SchedulerPinned::getFreeCore(core_id_t core_first) {
  ...
  do {
    if (m_core_thread_running[core_next] == INVALID_THREAD_ID) return core_next;
