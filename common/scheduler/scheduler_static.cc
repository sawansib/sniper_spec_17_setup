...
SchedulerStatic::SchedulerStatic(ThreadManager *thread_manager)
  ...
  for (core_id_t core_id = 0;
       ...
       core_id++) {
    m_core_mask[core_id] =
        Sim()->getCfg()->getBoolArray("scheduler/static/core_mask", core_id);
  }
}
...
core_id_t SchedulerStatic::findFirstFreeMaskedCore() {
  for (core_id_t core_id = 0;
    ...
    if (m_core_mask[core_id] &&
        Sim()->getCoreManager()->getCoreFromID(core_id)->getState() ==
            Core::IDLE) {
      return core_id;
    }
  }
...
core_id_t SchedulerStatic::threadCreate(thread_id_t thread_id) {
  ...
  LOG_ASSERT_ERROR(core_id != INVALID_CORE_ID,
                   "No cores available for spawnThread request.");

  app_id_t app_id =
      Sim()->getThreadManager()->getThreadFromID(thread_id)->getAppId();
  LOG_PRINT("Scheduler: thread %d from application %d now scheduled to core %d",
            thread_id, app_id, core_id);
