...
SchedulerRoaming::SchedulerRoaming(ThreadManager *thread_manager)
  ...
  for (core_id_t core_id = 0;
       ...
       core_id++) {
    m_core_mask[core_id] =
        Sim()->getCfg()->getBoolArray("scheduler/roaming/core_mask", core_id);
  }
}
...
void SchedulerRoaming::threadSetInitialAffinity(thread_id_t thread_id) {
  ...
  for (core_id_t core_id = 0;
       core_id < (core_id_t)Sim()->getConfig()->getApplicationCores();
       ++core_id) {
    if (m_core_mask[core_id]) m_thread_info[thread_id].addAffinity(core_id);
  }
}
