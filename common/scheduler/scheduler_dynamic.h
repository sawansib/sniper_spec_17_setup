...
class SchedulerDynamic : public Scheduler {
 public:
  ...
  virtual void threadStall(thread_id_t thread_id,
                           ThreadManager::stall_type_t reason,
                           SubsecondTime time) {}
  virtual void threadResume(thread_id_t thread_id, thread_id_t thread_by,
                            SubsecondTime time) {}
 ...
 private:
  ...
  void __threadStall(thread_id_t thread_id, ThreadManager::stall_type_t reason,
                     SubsecondTime time);
  void __threadResume(thread_id_t thread_id, thread_id_t thread_by,
                      SubsecondTime time);
  void __threadExit(thread_id_t thread_id, SubsecondTime time);

  // Hook stubs
  static SInt64 hook_periodic(UInt64 ptr, UInt64 time) {
    ((SchedulerDynamic *)ptr)->__periodic(*(subsecond_time_t *)&time);
    return 0;
  }
  ...
  static SInt64 hook_thread_stall(UInt64 ptr, UInt64 _args) {
    ...
    ((SchedulerDynamic *)ptr)
        ->__threadStall(args->thread_id, args->reason, args->time);
    return 0;
  }
  static SInt64 hook_thread_resume(UInt64 ptr, UInt64 _args) {
    HooksManager::ThreadResume *args = (HooksManager::ThreadResume *)_args;
    ((SchedulerDynamic *)ptr)
        ->__threadResume(args->thread_id, args->thread_by, args->time);
    return 0;
  }
