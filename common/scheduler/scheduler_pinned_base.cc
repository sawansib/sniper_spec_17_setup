...
// If multiple threads share a core, they are time-shared with a configurable
// quantum

SchedulerPinnedBase::SchedulerPinnedBase(ThreadManager *thread_manager,
    ...
    : SchedulerDynamic(thread_manager),
      ...
      m_core_thread_running(Sim()->getConfig()->getApplicationCores(),
                            INVALID_THREAD_ID),
      m_quantum_left(Sim()->getConfig()->getApplicationCores(),
                     SubsecondTime::Zero()) {}
...
void SchedulerPinnedBase::threadYield(thread_id_t thread_id) {
  ...
  if (core_id != INVALID_CORE_ID) {
    ...
    if (!m_thread_info[thread_id].hasAffinity(core_id)) {
      core_id_t free_core_id = findFreeCoreForThread(thread_id);
      if (free_core_id != INVALID_CORE_ID) {
        // We have just been moved to a different core(s), and one of them is
        // free. Schedule us there now.
...
bool SchedulerPinnedBase::threadSetAffinity(thread_id_t calling_thread_id,
                        ...
                        .getCoreRunning()))  // but not where we want it to
  {
    ...
    m_quantum_left[m_thread_info[thread_id].getCoreRunning()] =
        SubsecondTime::Zero();
             ...
             && !m_thread_info[thread_id]
                     .isRunning())  // Thread is not running (we can't preempt
                                    // it outside of the barrier)
  {
    core_id_t free_core_id = findFreeCoreForThread(thread_id);
    if (free_core_id != INVALID_THREAD_ID)  // Thread's new core is free
    {
      // We have just been moved to a different core, and that core is free.
...
bool SchedulerPinnedBase::threadGetAffinity(thread_id_t thread_id,
                                            size_t cpusetsize,
                                            cpu_set_t *mask) {
  ...
  for (core_id_t core_id = 0;
       ...
       ++core_id) {
    if (m_thread_info[thread_id].hasAffinity(core_id)
        // When application has not yet done any sched_setaffinity calls, lie
        // and return a fully populated affinity bitset. This makes libiomp5 use
        // all available cores.
...
void SchedulerPinnedBase::threadStart(thread_id_t thread_id,
                                      SubsecondTime time) {
  // Thread transitioned out of INITIALIZING, if it did not get a core assigned
  // by threadCreate but there is a free one now, schedule it there
  core_id_t free_core_id = findFreeCoreForThread(thread_id);
  if (free_core_id != INVALID_THREAD_ID) reschedule(time, free_core_id, false);
}

void SchedulerPinnedBase::threadStall(thread_id_t thread_id,
...
void SchedulerPinnedBase::threadResume(thread_id_t thread_id,
...
void SchedulerPinnedBase::threadExit(thread_id_t thread_id,
                                     SubsecondTime time) {
  // If the running thread becomes unrunnable, schedule someone else
  if (m_thread_info[thread_id].isRunning())
    reschedule(time, m_thread_info[thread_id].getCoreRunning(), false);
...
void SchedulerPinnedBase::reschedule(SubsecondTime time, core_id_t core_id,
                                     bool is_periodic) {
  thread_id_t current_thread_id = m_core_thread_running[core_id];

  if (current_thread_id != INVALID_THREAD_ID &&
      Sim()->getThreadManager()->getThreadState(current_thread_id) ==
          Core::INITIALIZING) {
  ...
  for (thread_id_t thread_id = 0;
       thread_id < (thread_id_t)m_threads_runnable.size(); ++thread_id) {
    if (m_thread_info[thread_id].hasAffinity(
            core_id)  // Thread is allowed to run on this core
        && m_threads_runnable[thread_id] == true  // Thread is not stalled
        && (!m_thread_info[thread_id]
                 .isRunning()  // Thread is not already running somewhere else
            || m_thread_info[thread_id].getCoreRunning() == core_id)) {
      SInt64 score;
      if (m_thread_info[thread_id].isRunning())
        // Thread is currently running: negative score depending on how long
        // it's already running
        score = SInt64(m_thread_info[thread_id].getLastScheduledIn().getPS()) -
                time.getPS();
      else
        // Thread is not currently running: positive score depending on how long
        // we have been waiting
        score = time.getPS() -
                SInt64(m_thread_info[thread_id].getLastScheduledOut().getPS());
  ...
  if (current_thread_id != new_thread_id) {
    // If a thread was running on this core, and we'll schedule another one,
    // unschedule the current one
    if (current_thread_id != INVALID_THREAD_ID) {
      m_thread_info[current_thread_id].setCoreRunning(INVALID_CORE_ID);
      // Update last scheduled out time, with a small extra penalty to make sure
      // we don't reconsider this thread in the same periodic() call but for a
      // next core
      m_thread_info[current_thread_id].setLastScheduledOut(
          time + SubsecondTime::PS(core_id));
      moveThread(current_thread_id, INVALID_CORE_ID, time);
    }

    // Set core as running this thread *before* we call moveThread(), otherwise
    // the HOOK_THREAD_RESUME callback for this thread might see an empty core,
    // causing a recursive loop of reschedulings
    ...
    if (new_thread_id != INVALID_THREAD_ID) {
      ...
      if (m_thread_info[new_thread_id].isRunning())
        m_core_thread_running[m_thread_info[new_thread_id].getCoreRunning()] =
            INVALID_THREAD_ID;
...
void SchedulerPinnedBase::printState() {
  ...
  for (thread_id_t thread_id = 0;
    ...
    if (m_thread_info[thread_id].isRunning()) {
      ...
      printf(" %c%c%s", state, m_threads_runnable[thread_id] ? '+' : '_',
