...
SchedulerBigSmall::SchedulerBigSmall(ThreadManager *thread_manager)
    : SchedulerPinnedBase(thread_manager,
                          SubsecondTime::NS(Sim()->getCfg()->getInt(
                              "scheduler/big_small/quantum"))),
      m_debug_output(Sim()->getCfg()->getBool("scheduler/big_small/debug")),
      m_last_reshuffle(SubsecondTime::Zero()),
      m_rng(rng_seed(42)) {
  // Figure out big and small cores, and create affinity masks for the set of
  // big cores and the set of small cores, respectively

  m_num_big_cores = 0;
  CPU_ZERO(&m_mask_big);
  CPU_ZERO(&m_mask_small);

  for (core_id_t coreId = 0;
       coreId < (core_id_t)Sim()->getConfig()->getApplicationCores();
       coreId++) {
...
void SchedulerBigSmall::threadStall(thread_id_t thread_id,
                                    ThreadManager::stall_type_t reason,
                                    SubsecondTime time) {
  // When a thread on the big core stalls, promote another thread to the big
  // core(s)

  if (m_debug_output)
    std::cout << "[SchedulerBigSmall] thread " << thread_id << " stalled"
              << std::endl;
  ...
  if (m_debug_output) printState();
}

void SchedulerBigSmall::threadExit(thread_id_t thread_id, SubsecondTime time) {
  // When a thread on the big core ends, promote another thread to the big
  // core(s)

  if (m_debug_output)
    std::cout << "[SchedulerBigSmall] thread " << thread_id << " ended"
              << std::endl;
...
void SchedulerBigSmall::periodic(SubsecondTime time) {
  ...
  if (time > m_last_reshuffle + m_quantum) {
    ...
    for (thread_id_t thread_id = 0;
         ...
         ++thread_id) {
      if (m_thread_isbig[thread_id]) moveToSmall(thread_id);
    }
    // Now promote as many threads to the big core pool as there are big cores
    for (UInt64 i = 0; i < std::min(m_num_big_cores,
                                    Sim()->getThreadManager()->getNumThreads());
         ++i) {
      pickBigThread();
    }
  ...
  if (print_state && m_debug_output) printState();
}

void SchedulerBigSmall::moveToSmall(thread_id_t thread_id) {
  threadSetAffinity(INVALID_THREAD_ID, thread_id, sizeof(m_mask_small),
                    &m_mask_small);
  m_thread_isbig[thread_id] = false;
}

void SchedulerBigSmall::moveToBig(thread_id_t thread_id) {
  threadSetAffinity(INVALID_THREAD_ID, thread_id, sizeof(m_mask_big),
                    &m_mask_big);
...
void SchedulerBigSmall::pickBigThread() {
  ...
  if (eligible.size() > 0) {
    ...
    if (m_debug_output)
      std::cout << "[SchedulerBigSmall] thread " << thread_id
                << " promoted to big core" << std::endl;
  }
}
