...
TraceThread::TraceThread(Thread *thread, SubsecondTime time_start,
                         String tracefile, String responsefile, app_id_t app_id,
                         bool cleanup)
    : m__thread(NULL),
      m_thread(thread),
      ...
      m_address_randomization(
          Sim()->getCfg()->getBool("traceinput/address_randomization")),
      m_stop(false),
      m_bbv_base(0),
      m_bbv_count(0),
      m_bbv_last(0),
      ...
      m_started(false),
      m_stopped(false) {
  m_trace.setHandleInstructionCountFunc(
      TraceThread::__handleInstructionCountFunc, this);
  ...
  if (Sim()->getRoutineTracer())
    m_trace.setHandleRoutineFunc(TraceThread::__handleRoutineChangeFunc,
                                 TraceThread::__handleRoutineAnnounceFunc,
                                 this);

  if (m_address_randomization) {
    // Fisher-Yates shuffle, simultaneously initializing array to
    // m_address_randomization_table[i] = i See
    // http://en.wikipedia.org/wiki/Fisher%E2%80%93Yates_shuffle#The_.22inside-out.22_algorithm
    // By using the app_id as a random seed, we get an app_id-specific
    // pseudo-random permutation of 0..255
  ...
  if (syntax == "intel")
    ...
    LOG_PRINT_ERROR("Unknown assembly syntax %s, should be intel, att or xed.",
                    syntax.c_str());
}
...
UInt64 TraceThread::va2pa(UInt64 va, bool *noMapping) {
  if (m_trace_has_pa) {
    ...
    if (pa != 0) {
      ...
      // Fall through to construct an address with our thread id in the upper
      // bits (assume address is private)
    }
  }

  if (m_address_randomization) {
    // Set 16 bits to app_id | remap middle 36 bits using app_id-specific
    // mapping | keep lower 12 bits (page offset)
    return (UInt64(m_thread->getAppId()) << pa_core_shift) |
           (remapAddress(va >> va_page_shift) << va_page_shift) |
           (va & va_page_mask);
  } else {
...
UInt64 TraceThread::remapAddress(UInt64 va_page) {
  ...
  // By randomly remapping the lower 24 bits of va_page, addresses will be
  // distributed over a 1<<(16+3*8) = 64 GB range which should avoid artificial
  // set contention in all cache levels. Of course we want the remapping to be
  // invertible so we never map different incoming addresses onto the same
  // outgoing address. This is guaranteed since m_address_randomization_table
...
void TraceThread::handleOutputFunc(uint8_t fd, const uint8_t *data,
                                   uint32_t size) {
  FILE *fp;
  if (fd == 1)
    fp = stdout;
  ...
  while (size) {
    ...
    if (ptr == data + size) {
      if (size > sizeof(m_output_leftover)) size = sizeof(m_output_leftover);
...
uint64_t TraceThread::handleSyscallFunc(uint16_t syscall_number,
                                        const uint8_t *data, uint32_t size) {
  // We may have been blocked in a system call, if we start executing
  // instructions again that means we're continuing
  if (m_blocked) {
    unblock();
  }
  ...
  switch (syscall_number) {
    ...
    default: {
      LOG_ASSERT_ERROR(size == sizeof(SyscallMdl::syscall_args_t),
                       "Syscall arguments not the correct size");
...
void TraceThread::handleRoutineChangeFunc(Sift::RoutineOpType event,
                                          uint64_t eip, uint64_t esp,
                                          uint64_t callEip) {
  switch (event) {
...
bool TraceThread::handleEmuFunc(Sift::EmuType type, Sift::EmuRequest &req,
                                Sift::EmuReply &res) {
  // We may have been blocked in a system call, if we start executing
  // instructions again that means we're continuing
  if (m_blocked) {
  ...
  switch (type) {
    case Sift::EmuTypeRdtsc: {
      ...
      const ComponentPeriod *dom_global =
    ...
    case Sift::EmuTypeGetTime: {
      res.gettime.time_ns =
          Sim()->getConfig()->getOSEmuTimeStart() * 1000000000 +
...
void TraceThread::handleRoutineAnnounceFunc(uint64_t eip, const char *name,
                                            const char *imgname,
                                            uint64_t offset, uint32_t line,
                                            uint32_t column,
                                            const char *filename) {
  Sim()->getRoutineTracer()->addRoutine(eip, name, imgname, offset, column,
                                        line, filename);
}

SubsecondTime TraceThread::getCurrentTime() const {
  LOG_ASSERT_ERROR(m_thread->getCore() != NULL,
                   "Cannot get time while not on a core");
...
Instruction *TraceThread::decode(Sift::Instruction &inst) {
  ...
  if (!xed_decoded_inst_get_attribute(&xed_inst, XED_ATTRIBUTE_NOP)) {
    for (uint32_t mem_idx = 0;
         mem_idx < xed_decoded_inst_number_of_memory_operands(&xed_inst);
         ++mem_idx)
  ...
  instruction->setAtomic(xed_operand_values_get_atomic(
      xed_decoded_inst_operands_const(&xed_inst)));
  char disassembly[64];
#if PIN_REV >= 67254
  xed_format_context(m_syntax, &xed_inst, disassembly, sizeof(disassembly) - 1,
                     inst.sinst->addr, 0, 0);
#else
  xed_format(m_syntax, &xed_inst, disassembly, sizeof(disassembly) - 1,
             inst.sinst->addr);
#endif
  instruction->setDisassembly(disassembly);

  const std::vector<const MicroOp *> *uops =
      InstructionDecoder::decode(inst.sinst->addr, &xed_inst, instruction);
...
Sift::Mode TraceThread::handleInstructionCountFunc(uint32_t icount) {
  if (!m_started) {
    // Received first instruction, let TraceManager know our SIFT connection is
    // up and running
  ...
  if (Sim()->getInstrumentationMode() == InstMode::DETAILED && icount) {
    ...
    core->getPerformanceModel()->queuePseudoInstruction(
        new UnknownInstruction(icount * core->getDvfsDomain()->getPeriod()));
  ...
  if (m_thread->reschedule(time, core)) {
    ...
    core->getPerformanceModel()->queuePseudoInstruction(
        new SyncInstruction(time, SyncInstruction::UNSCHEDULED));
  }
...
void TraceThread::handleCacheOnlyFunc(uint8_t icount, Sift::CacheOnlyType type,
                                      uint64_t eip, uint64_t address) {
  Core *core = m_thread->getCore();
  if (!core) {
  ...
  if (icount) core->countInstructions(0, icount);
  ...
  switch (type) {
    ...
    case Sift::CacheOnlyBranchNotTaken: {
      ...
      bool mispredict =
          core->accessBranchPredictor(va2pa(eip), taken, va2pa(address));
      if (mispredict) core->getPerformanceModel()->handleBranchMispredict();
      break;
    }
    ...
    case Sift::CacheOnlyMemWrite:
      core->accessMemory(
          Core::NONE, type == Sift::CacheOnlyMemRead ? Core::READ : Core::WRITE,
          va2pa(address), NULL, 4, Core::MEM_MODELED_COUNT, va2pa(eip));
      break;
...
void TraceThread::handleInstructionWarmup(Sift::Instruction &inst,
                                          Sift::Instruction &next_inst,
                                          Core *core, bool do_icache_warmup,
                                          UInt64 icache_warmup_addr,
                                          UInt64 icache_warmup_size) {
  const xed_decoded_inst_t &xed_inst = inst.sinst->xed_inst;
  ...
  if (inst.is_branch) {
    bool mispredict = core->accessBranchPredictor(
        va2pa(inst.sinst->addr), inst.taken, va2pa(next_inst.sinst->addr));
    if (mispredict) core->getPerformanceModel()->handleBranchMispredict();
  }
  ...
  if (inst.executed) {
    const bool is_atomic_update = xed_operand_values_get_atomic(
        xed_decoded_inst_operands_const(&xed_inst));
    ...
    if (!xed_decoded_inst_get_attribute(&xed_inst, XED_ATTRIBUTE_NOP)) {
      for (uint32_t mem_idx = 0;
           ...
           ++mem_idx) {
        if (xed_decoded_inst_mem_read(&xed_inst, mem_idx)) {
          LOG_ASSERT_ERROR(mem_idx < inst.num_addresses,
                           "Did not receive enough data addresses");

          bool no_mapping = false;
          UInt64 pa =
              va2pa(inst.addresses[mem_idx], is_prefetch ? &no_mapping : NULL);
          if (no_mapping) continue;

          core->accessMemory(
              /*(is_atomic_update) ? Core::LOCK :*/ Core::NONE,
              (is_atomic_update) ? Core::READ_EX : Core::READ, pa, NULL,
              xed_decoded_inst_get_memory_operand_length(&xed_inst, mem_idx),
              Core::MEM_MODELED_COUNT, va2pa(inst.sinst->addr));
        }
      }
      ...
      for (uint32_t mem_idx = 0;
        ...
        if (xed_decoded_inst_mem_written(&xed_inst, mem_idx)) {
          ...
          if (is_atomic_update)
            core->logMemoryHit(false, Core::WRITE, pa, Core::MEM_MODELED_COUNT,
                               va2pa(inst.sinst->addr));
          else
            core->accessMemory(
                /*(is_atomic_update) ? Core::UNLOCK :*/ Core::NONE, Core::WRITE,
                pa, NULL,
                xed_decoded_inst_get_memory_operand_length(&xed_inst, mem_idx),
                Core::MEM_MODELED_COUNT, va2pa(inst.sinst->addr));
        }
...
void TraceThread::handleInstructionDetailed(Sift::Instruction &inst,
                                            Sift::Instruction &next_inst,
  ...
  DynamicInstruction *dynins =
      prfmdl->createDynamicInstruction(ins, va2pa(inst.sinst->addr));
  ...
  if (!xed_decoded_inst_get_attribute(&xed_inst, XED_ATTRIBUTE_NOP)) {
    ...
    for (uint32_t mem_idx = 0;
         ...
         ++mem_idx) {
      if (xed_decoded_inst_mem_read(&xed_inst, mem_idx)) {
        addDetailedMemoryInfo(dynins, inst, xed_inst, mem_idx, Operand::READ,
                              is_prefetch, prfmdl);
      }
    }
    ...
    for (uint32_t mem_idx = 0;
         ...
         ++mem_idx) {
      if (xed_decoded_inst_mem_written(&xed_inst, mem_idx)) {
        addDetailedMemoryInfo(dynins, inst, xed_inst, mem_idx, Operand::WRITE,
                              is_prefetch, prfmdl);
      }
    }
  }
...
void TraceThread::addDetailedMemoryInfo(
    DynamicInstruction *dynins, Sift::Instruction &inst,
    const xed_decoded_inst_t &xed_inst, uint32_t mem_idx,
    Operand::Direction op_type, bool is_prefetch, PerformanceModel *prfmdl) {
  ...
  if (no_mapping) {
    dynins->addMemory(
        inst.executed, SubsecondTime::Zero(), 0,
        xed_decoded_inst_get_memory_operand_length(&xed_inst, mem_idx), op_type,
        0, HitWhere::PREFETCH_NO_MAPPING);
  } else {
    dynins->addMemory(
        inst.executed, SubsecondTime::Zero(), pa,
        xed_decoded_inst_get_memory_operand_length(&xed_inst, mem_idx), op_type,
        0, HitWhere::UNKNOWN);
  }
}
...
void TraceThread::unblock() {
  ...
  m_thread->getSyscallMdl()->runExit(0);
  {
    ...
    // We were blocked on a system call, but started executing instructions
    // again. This means we woke up. Since there is no explicit wakeup nor an
    // associated time, use global time.
    Sim()->getThreadManager()->resumeThread_async(
        m_thread->getId(), INVALID_THREAD_ID, end_time, NULL);
  }

  if (m_thread->getCore()) {
    m_thread->getCore()->getPerformanceModel()->queuePseudoInstruction(
        new SyncInstruction(end_time, SyncInstruction::SYSCALL));
  } else {
    // We were scheduled out during the system call
    m_thread->reschedule(end_time, NULL);
    m_thread->getCore()->getPerformanceModel()->queuePseudoInstruction(
        new SyncInstruction(end_time, SyncInstruction::UNSCHEDULED));
  }
...
void TraceThread::run() {
  ...
  // Open the trace (be sure to do this before potentially blocking on
  // reschedule() as this causes deadlock)
  m_trace.initStream();
  ...
  // Received first instruction, let TraceManager know our SIFT connection is up
  // and running
  Sim()->getTraceManager()->signalStarted();
  m_started = true;
  ...
  while (have_first && m_trace.Read(next_inst)) {
    ...
    switch (Sim()->getInstrumentationMode()) {
      ...
      case InstMode::CACHE_ONLY:
        handleInstructionWarmup(inst, next_inst, core, do_icache_warmup,
                                icache_warmup_addr, icache_warmup_size);
        break;
...
UInt64 TraceThread::getProgressExpect() { return m_trace.getLength(); }

UInt64 TraceThread::getProgressValue() { return m_trace.getPosition(); }

void TraceThread::handleAccessMemory(Core::lock_signal_t lock_signal,
                                     Core::mem_op_t mem_op_type, IntPtr d_addr,
  ...
  m_trace.AccessMemory(sift_lock_signal, sift_mem_op, d_addr,
