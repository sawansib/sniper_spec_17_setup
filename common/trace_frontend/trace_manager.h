...
class TraceManager {
 private:
  ...
  struct app_info_t {
    app_info_t() : thread_count(1), num_threads(1), num_runs(0) {}
    UInt32 thread_count;  //< Index counter for new thread's FIFO name
    UInt32 num_threads;   //< Number of active threads for this app (when zero,
                          //app is done)
  ...
  UInt32 m_num_apps;
  UInt32 m_num_apps_nonfinish;  //< Number of applications that have yet to
                                //complete their first run
  ...
  String getFifoName(app_id_t app_id, UInt64 thread_num, bool response,
                     bool create);
  thread_id_t newThread(app_id_t app_id, bool first, bool init_fifo, bool spawn,
                        SubsecondTime time, thread_id_t creator_thread_id);
 ...
 public:
  ...
  void accessMemory(int core_id, Core::lock_signal_t lock_signal,
