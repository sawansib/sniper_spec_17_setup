...
BbvCount::BbvCount(core_id_t core_id)
    : m_core_id(core_id),
      ...
      // Define N to skip X samples with X uniformely distributed between
      // 0..2*N, so on average 1/N samples
      ,
      m_sample_period(2 * Sim()->getCfg()->getInt("bbv/sampling")),
      m_sample_seed(rng_seed(m_sample_period)) {
  sampleReset();
  for (int i = 0; i < NUM_BBV; ++i)
    registerStatsMetric("core", core_id, "bbv-" + itostr(i),
                        &m_bbv_counts_abs[i]);
}
...
bool BbvCount::sample() { return m_sample-- == 0; }

void BbvCount::count(UInt64 address, UInt64 count) {
  sampleReset();
  ...
  UInt64 s0 = rng_seed(address), s1 = rng_seed(address + 1),
...
UInt64 BbvCount::getDiff() {
  ...
  for (int i = 0; i < NUM_BBV; ++i) {
    ...
    long int bbv_count_sub = bbv_count - m_bbv_previous[i];
    if (bbv_count_sub < 0) bbv_count_sub *= -1;
    diff += bbv_count_sub;
  }

  // Normalize by the number of BBVs to keep the results consistent as the count
  // changes
  return diff / NUM_BBV;
}
