...
class Core {
 public:
  ...
  enum MemModeled {
    ...
    MEM_MODELED_COUNT_TLBTIME, /* Count in #accesses/#misses, queue
                                  TLBMissInstruction on TLB miss */
    MEM_MODELED_TIME,          /* Count + account for access latency (using
                                  MemAccessInstruction) */
    MEM_MODELED_FENCED, /* Count + account for access latency as memory fence
                           (using MemAccessInstruction) */
    MEM_MODELED_RETURN, /* Count + time + return data to construct
                           DynamicInstruction */
  };
  ...
  MemoryResult accessMemory(lock_signal_t lock_signal, mem_op_t mem_op_type,
  ...
  MemoryResult nativeMemOp(lock_signal_t lock_signal, mem_op_t mem_op_type,
                           IntPtr d_addr, char *data_buffer, UInt32 data_size);

  void accessMemoryFast(bool icache, mem_op_t mem_op_type, IntPtr address);

  void logMemoryHit(bool icache, mem_op_t mem_op_type, IntPtr address,
                    MemModeled modeled = MEM_MODELED_NONE, IntPtr eip = 0);
  ...
  void setInstructionsCallback(UInt64 instructions) {
    m_instructions_callback = m_instructions + instructions;
  }
 ...
 private:
  ...
  MemoryResult initiateMemoryAccess(MemComponent::component_t mem_component,
 ...
 protected:
  // Optimized version of countInstruction has direct access to m_instructions
  // and m_instructions_callback
  friend class InstructionModeling;

  // In contrast to core->m_performance_model->m_instructions, this one always
  // increments, also when performance modeling is disabled or when
  // instrumenation mode is CACHE_ONLY or FAST_FORWARD
