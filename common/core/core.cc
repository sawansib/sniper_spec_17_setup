...
#define MYLOG(...)                                                          \
  {                                                                         \
    ...
    fflush(stderr);                                                         \
    fprintf(                                                                \
        stderr, "[%8lu] %dcor %-25s@%03u: ",                                \
        getPerformanceModel()->getCycleCount(ShmemPerfModel::_USER_THREAD), \
        m_core_id, __FUNCTION__, __LINE__);                                 \
...
const char *core_state_names[] = {
    "running",   "initializing", "stalled", "sleeping",
    "waking_up", "idle",         "broken",
};
static_assert(Core::NUM_STATES ==
                  sizeof(core_state_names) / sizeof(core_state_names[0]),
...
Core::Core(SInt32 id)
    : m_core_id(id),
      ...
      m_cheetah_manager(Sim()->getCfg()->getBool("core/cheetah/enabled")
                            ? new CheetahManager(id)
                            : NULL),
  ...
  m_memory_manager = MemoryManagerBase::createMMU(
      Sim()->getCfg()->getString("caching_protocol/type"), this, m_network,
      m_shmem_perf_model);
...
Core::~Core() {
  if (m_cheetah_manager) delete m_cheetah_manager;
  delete m_topology_info;
  delete m_memory_manager;
  delete m_shmem_perf_model;
  delete m_performance_model;
  if (m_clock_skew_minimization_client) delete m_clock_skew_minimization_client;
  delete m_network;
  for (UInt32 i = 0; i < Sim()->getConfig()->getLightCacheNum(); ++i)
    if (m_light_caches[i]) delete m_light_caches[i];
}
...
bool Core::countInstructions(IntPtr address, UInt32 count) {
  ...
  if (m_bbv.sample()) m_bbv.count(address, count);
  ...
  if (isEnabledInstructionsCallback()) {
    if (m_instructions >= m_instructions_callback) {
      ...
      Sim()->getHooksManager()->callHooks(HookType::HOOK_INSTR_COUNT,
                                          m_core_id);
      // When using the fast-forward performance model, HOOK_INSTR_COUNT may
      // cause a rescheduling of the current thread so let it know that it
      // should make the appropriate checks
...
void Core::hookPeriodicInsCheck() {
  if (m_instructions > m_instructions_hpi_callback) {
    __sync_fetch_and_add(&g_instructions_hpi_global,
                         m_instructions - m_instructions_hpi_last);
    m_instructions_hpi_callback +=
        Sim()->getConfig()->getHPIInstructionsPerCore();
...
void Core::hookPeriodicInsCall() {
  ...
  if (g_instructions_hpi_global > g_instructions_hpi_global_callback) {
    Sim()->getHooksManager()->callHooks(HookType::HOOK_PERIODIC_INS,
                                        g_instructions_hpi_global);
    g_instructions_hpi_global_callback +=
        Sim()->getConfig()->getHPIInstructionsGlobal();
  }
}

bool Core::accessBranchPredictor(IntPtr eip, bool taken, IntPtr target) {
...
MemoryResult makeMemoryResult(HitWhere::where_t _hit_where,
                              SubsecondTime _latency) {
  LOG_ASSERT_ERROR(_hit_where < HitWhere::NUM_HITWHERES, "Invalid HitWhere %u",
                   (long)_hit_where);
  MemoryResult res;
...
void Core::logMemoryHit(bool icache, mem_op_t mem_op_type, IntPtr address,
                        MemModeled modeled, IntPtr eip) {
  getMemoryManager()->addL1Hits(icache, mem_op_type, 1);
}

MemoryResult Core::readInstructionMemory(IntPtr address,
                                         UInt32 instruction_size) {
  LOG_PRINT("Instruction: Address(0x%x), Size(%u), Start READ", address,
            instruction_size);

  UInt64 blockmask = ~(getMemoryManager()->getCacheBlockSize() - 1);
  bool single_cache_line =
      ((address & blockmask) == ((address + instruction_size - 1) & blockmask));

  // Assume the core reads full instruction cache lines and caches them
  // internally for subsequent instructions. This reduces L1-I accesses and
  // power to more realistic levels. For Nehalem, it's in fact only 16 bytes,
  // other architectures (Sandy Bridge) have a micro-op cache, so this is just
  // an approximation.
  ...
  if ((address & blockmask) == m_icache_last_block) {
    if (single_cache_line) {
      return makeMemoryResult(HitWhere::L1I,
                              getMemoryManager()->getL1HitLatency());
    } else {
      // Instruction spanning cache lines: drop the first line, do access the
      // second one
  ...
  // Cases with multiple cache lines or when we are not sure that it will be a
  // hit call into the caches
  return initiateMemoryAccess(
      MemComponent::L1_ICACHE, Core::NONE, Core::READ, address & blockmask,
      NULL, getMemoryManager()->getCacheBlockSize(), MEM_MODELED_COUNT_TLBTIME,
      0, SubsecondTime::MaxTime());
}
...
void Core::accessMemoryFast(bool icache, mem_op_t mem_op_type, IntPtr address) {
  ...
  SubsecondTime latency = getMemoryManager()->coreInitiateMemoryAccessFast(
      icache, mem_op_type, address);

  if (latency > SubsecondTime::Zero())
    m_performance_model->handleMemoryLatency(latency, HitWhere::MISS);
}

MemoryResult Core::initiateMemoryAccess(MemComponent::component_t mem_component,
                                        lock_signal_t lock_signal,
                                        mem_op_t mem_op_type, IntPtr address,
                                        Byte *data_buf, UInt32 data_size,
                                        MemModeled modeled, IntPtr eip,
                                        SubsecondTime now) {
  MYLOG("access %lx+%u %c%c modeled(%s)", address, data_size,
        mem_op_type == Core::WRITE ? 'W' : 'R',
        mem_op_type == Core::READ_EX ? 'X' : ' ', ModeledString(modeled));

  if (data_size <= 0) {
    return makeMemoryResult((HitWhere::where_t)mem_component,
                            SubsecondTime::Zero());
  }

  // Setting the initial time
  SubsecondTime initial_time = (now == SubsecondTime::MaxTime())
                                   ? getPerformanceModel()->getElapsedTime()
                                   : now;

  // Protect from concurrent access by user thread (doing rewritten memops) and
  // core thread (doing icache lookups)
  if (lock_signal != Core::UNLOCK) m_mem_lock.acquire();
  ...
  LOG_PRINT("Time(%s), %s - ADDR(0x%x), data_size(%u), START",
            ...
            ((mem_op_type == READ) ? "READ" : "WRITE"), address, data_size);
  ...
  for (IntPtr curr_addr_aligned = begin_addr_aligned;
       curr_addr_aligned <= end_addr_aligned;
       curr_addr_aligned += cache_block_size) {
    ...
    LOG_PRINT(
        "Start InitiateSharedMemReq: ADDR(0x%x), offset(%u), curr_size(%u)",
    ...
    HitWhere::where_t this_hit_where =
        getMemoryManager()->coreInitiateMemoryAccess(
            mem_component, lock_signal, mem_op_type, curr_addr_aligned,
            curr_offset, data_buf ? curr_data_buffer_head : NULL, curr_size,
            modeled);
    ...
    if (hit_where == HitWhere::UNKNOWN ||
        (this_hit_where != HitWhere::UNKNOWN && this_hit_where > hit_where))
      hit_where = this_hit_where;

    LOG_PRINT("End InitiateSharedMemReq: ADDR(0x%x), offset(%u), curr_size(%u)",
              curr_addr_aligned, curr_offset, curr_size);
  ...
  LOG_ASSERT_ERROR(final_time >= initial_time,
                   ...
                   itostr(final_time).c_str(), itostr(initial_time).c_str());

  LOG_PRINT("Time(%s), %s - ADDR(0x%x), data_size(%u), END\n",
            itostr(final_time).c_str(),
            ((mem_op_type == READ) ? "READ" : "WRITE"), address, data_size);

  if (lock_signal != Core::LOCK) m_mem_lock.release();
  ...
  switch (modeled) {
    ...
    case MEM_MODELED_FENCED:
      if (m_performance_model->isEnabled()) {
        ...
        PseudoInstruction *i = new MemAccessInstruction(
            shmem_time, address, data_size, modeled == MEM_MODELED_FENCED);
...
/*
 * accessMemory (lock_signal_t lock_signal, mem_op_t mem_op_type, IntPtr d_addr,
 ...
 *   data_buffer :: buffer holding data for WRITE or buffer which must be
 * written on a READ data_size :: size of data we must read/write
 *
 * Return Value:
 *   number of misses :: State the number of cache misses
 */
MemoryResult Core::accessMemory(lock_signal_t lock_signal, mem_op_t mem_op_type,
                                IntPtr d_addr, char *data_buffer,
                                UInt32 data_size, MemModeled modeled,
                                IntPtr eip, SubsecondTime now,
                                bool is_fault_mask) {
  ...
  if (data_buffer && !is_fault_mask) {
    if (Sim()->getConfig()->getSimulationMode() == Config::PINTOOL) {
      ...
      Sim()->getTraceManager()->accessMemory(
          m_core_id, lock_signal, mem_op_type, d_addr, data_buffer, data_size);
    }
  ...
  if (modeled == MEM_MODELED_NONE)
    ...
    return initiateMemoryAccess(MemComponent::L1_DCACHE, lock_signal,
                                mem_op_type, d_addr, (Byte *)data_buffer,
                                data_size, modeled, eip, now);
}

MemoryResult Core::nativeMemOp(lock_signal_t lock_signal, mem_op_t mem_op_type,
                               IntPtr d_addr, char *data_buffer,
                               UInt32 data_size) {
  if (data_size <= 0) {
    return makeMemoryResult(HitWhere::UNKNOWN, SubsecondTime::Zero());
...
__attribute__((weak)) void applicationMemCopy(void *dest, const void *src,
                                              size_t n) {
  memcpy(dest, src, n);
}
...
void Core::emulateCpuid(UInt32 eax, UInt32 ecx, cpuid_result_t &res) const {
  switch (eax) {
    case 0x0: {
      ...
      res.eax =
          std::max(UInt32(0xb),
                   res.eax);  // Maximum input eax: make sure 0xb is included
      break;
    }
    case 0x1: {
      // Return native results, except for CPU id
      cpuid(eax, ecx, res);
      res.ebx = (m_core_id << 24) |
                (Sim()->getConfig()->getApplicationCores() << 16) |
                (res.ebx & 0xffff);
      break;
    }
    case 0xb: {
      ...
      switch (ecx) {
        ...
        case 1:
          ...
          res.ebx = m_topology_info->smt_count *
                    m_topology_info->core_count;  // HW contexts / package
