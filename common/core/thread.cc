...
Thread::~Thread() {
  ...
  if (m_rtn_tracer) delete m_rtn_tracer;
}

void Thread::setCore(Core *core) {
  if (m_core) m_core->setThread(NULL);
  ...
  if (m_core) {
    LOG_ASSERT_ERROR(
        core->getThread() == NULL,
        "Cannot move thread %d to core %d as it is already running thread %d",
        getId(), core->getId(), core->getThread()->getId());
...
bool Thread::reschedule(SubsecondTime &time, Core *current_core) {
  if (m_core == NULL || current_core != m_core) {
    ...
    // Do a definitive check for m_core, while holding the lock, unoptimized
    // because another thread will be changing it
    while ((volatile Core *)m_core == NULL)
      time = Sim()->getThreadManager()->stallThread(
          getId(), ThreadManager::STALL_UNSCHEDULED, time);

    m_core->getPerformanceModel()->queuePseudoInstruction(
...
bool Thread::updateCoreTLS(int threadIndex) {
  if (Sim()->getCoreManager()->getCurrentCore(threadIndex) != m_core) {
    ...
    if (m_core) Sim()->getCoreManager()->initializeThread(m_core->getId());
    return true;
  } else
    return false;
}
