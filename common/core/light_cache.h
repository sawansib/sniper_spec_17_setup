...
class LightCache {
 private:
  ...
  void reset() {
    ...
    for (std::vector<std::pair<IntPtr, bool> >::iterator i = m_state.begin();
         i != m_state.end(); ++i)
      *i = empty;
  }
 ...
 public:
  ...
  void access(IntPtr address, bool write) {
    if (!checkHit(address, write)) updateState(address, write);
  }
  ...
  UInt64 getDiff() {
    return (m_misses - m_misses_saved) * 100000 /
           (m_accesses - m_accesses_saved);
  }
};
