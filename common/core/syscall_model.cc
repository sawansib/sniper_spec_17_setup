...
const char *SyscallMdl::futex_names[] = {
    "FUTEX_WAIT",          "FUTEX_WAKE",        "FUTEX_FD",
...
SyscallMdl::SyscallMdl(Thread *thread)
  ...
  __attribute__((unused)) int rc =
      posix_memalign((void **)&futex_counters, 64,
                     futex_counters_size);  // Align by cache line size to
  ...
  for (unsigned int e = 0; e < sizeof(futex_names) / sizeof(futex_names[0]);
       e++) {
    registerStatsMetric("futex", thread->getId(),
                        boost::to_lower_copy(String(futex_names[e]) + "_count"),
                        &(futex_counters->count[e]));
    registerStatsMetric("futex", thread->getId(),
                        boost::to_lower_copy(String(futex_names[e]) + "_delay"),
                        &(futex_counters->delay[e]));
  }

  registerStatsMetric("syscall", thread->getId(), "stdout-bytes",
                      &m_stdout_bytes);
  registerStatsMetric("syscall", thread->getId(), "stderr-bytes",
                      &m_stderr_bytes);
}

SyscallMdl::~SyscallMdl() { free(futex_counters); }

bool SyscallMdl::runEnter(IntPtr syscall_number, syscall_args_t &args) {
  Core *core = m_thread->getCore();
  LOG_ASSERT_ERROR(core != NULL,
                   "Syscall by thread %d: core should not be null",
                   m_thread->getId());

  LOG_PRINT("Got Syscall: %i", syscall_number);
  CLOG("syscall", "Enter thread %d core %d syscall %" PRIdPTR,
       m_thread->getId(), core->getId(), syscall_number);
  ...
  hook_args.args = args;
  {
    ...
    Sim()->getHooksManager()->callHooks(HookType::HOOK_SYSCALL_ENTER,
                                        (UInt64)&hook_args);
  }
  ...
  switch (syscall_number) {
    ...
    case SYS_clock_gettime: {
      if (Sim()->getConfig()->getOSEmuClockReplace()) {
        ...
        switch (clock) {
          ...
          default:
            LOG_ASSERT_ERROR(
                false, "SYS_clock_gettime does not currently support clock(%u)",
                clock);
        }
        m_emulated = true;
      }
      // else: don't set m_emulated and the system call will be executed
      // natively
      break;
    }
    ...
    case SYS_nanosleep: {
      ...
      core->accessMemory(Core::NONE, Core::READ, (IntPtr)req,
                         (char *)&local_req, sizeof(local_req));

      SubsecondTime time_wake = start_time +
                                SubsecondTime::SEC(local_req.tv_sec) +
                                SubsecondTime::NS(local_req.tv_nsec);
      SubsecondTime end_time;
      Sim()->getSyscallServer()->handleSleepCall(m_thread->getId(), time_wake,
                                                 start_time, end_time);

      if (m_thread->reschedule(end_time, core)) core = m_thread->getCore();

      core->getPerformanceModel()->queuePseudoInstruction(
          new SyncInstruction(end_time, SyncInstruction::SLEEP));
      ...
      if (rem) {
        ...
        local_rem.tv_sec = 0;
        local_rem.tv_nsec = 0;
        core->accessMemory(Core::NONE, Core::WRITE, (IntPtr)rem,
                           (char *)&local_rem, sizeof(local_rem));
      }
    ...
    case SYS_wait4: {
      ...
      Sim()->getThreadManager()->stallThread_async(
          m_thread->getId(),
          syscall_number == SYS_pause ? ThreadManager::STALL_PAUSE
    ...
    case SYS_sched_yield: {
      {
        ScopedLock sl(Sim()->getThreadManager()->getLock());
        Sim()->getThreadManager()->getScheduler()->threadYield(
            m_thread->getId());
      }

      // We may have been rescheduled
      SubsecondTime time = core->getPerformanceModel()->getElapsedTime();
      if (m_thread->reschedule(time, core)) core = m_thread->getCore();
      core->getPerformanceModel()->queuePseudoInstruction(
    ...
    case SYS_sched_setaffinity: {
      ...
      if (thread) {
        ...
        core->accessMemory(Core::NONE, Core::READ, (IntPtr)cpuset, local_cpuset,
                           cpusetsize);

        ScopedLock sl(Sim()->getThreadManager()->getLock());
        success = Sim()->getThreadManager()->getScheduler()->threadSetAffinity(
            m_thread->getId(), thread->getId(), cpusetsize,
    ...
    case SYS_sched_getaffinity: {
      ...
      if (thread) {
        ...
        success = Sim()->getThreadManager()->getScheduler()->threadGetAffinity(
            m_thread->getId(), cpusetsize, (cpu_set_t *)local_cpuset);

        if (success && cpuset)
          core->accessMemory(Core::NONE, Core::WRITE, (IntPtr)cpuset,
                             local_cpuset, cpusetsize);
        if (local_cpuset) delete[] local_cpuset;
      }
      // else: success is already false, return EINVAL for invalid pid

      // On success: return size of affinity mask (in bytes) needed to represent
      // however many cores we're modeling (returning a value that is too large
      // can cause a segfault in the application's libc)
      m_ret_val = success ? (Sim()->getConfig()->getApplicationCores() + 7) / 8
                          : -EINVAL;
      m_emulated = true;
    ...
    case SYS_write: {
      ...
      if ((fd == STDOUT_FILENO && Sim()->getConfig()->suppressStdout()) ||
          (fd == STDERR_FILENO && Sim()->getConfig()->suppressStderr())) {
        m_ret_val = count;
        m_emulated = true;
      }

      if (fd == STDOUT_FILENO) m_stdout_bytes += count;
      if (fd == STDERR_FILENO) m_stderr_bytes += count;
...
IntPtr SyscallMdl::runExit(IntPtr old_return) {
  ...
  if (m_stalled) {
...

    {
      ...
      Sim()->getThreadManager()->resumeThread_async(
          m_thread->getId(), INVALID_THREAD_ID, time_wake, NULL);
    }
    ...
    core->getPerformanceModel()->queuePseudoInstruction(new SyncInstruction(
        time_wake, m_syscall_number == SYS_pause ? SyncInstruction::PAUSE
                                                 : SyncInstruction::SYSCALL));
  ...
  {
    ScopedLock sl(Sim()->getThreadManager()->getLock());
    Sim()->getHooksManager()->callHooks(HookType::HOOK_SYSCALL_EXIT,
                                        (UInt64)&hook_args);
  }
...
IntPtr SyscallMdl::handleFutexCall(syscall_args_t &args) {
  ...
  IntPtr ret_val = Sim()->getSyscallServer()->handleFutexCall(
      m_thread->getId(), fargs, start_time, end_time);

  if (m_thread->reschedule(end_time, core)) core = m_thread->getCore();

  core->getPerformanceModel()->queuePseudoInstruction(
      new SyncInstruction(end_time, SyncInstruction::FUTEX));
...
String SyscallMdl::formatSyscall() const {
  return String(syscall_string(m_syscall_number)) + "[" +
         itostr(m_syscall_number) + "] (" + itostr(m_syscall_args.arg0) + ", " +
         itostr(m_syscall_args.arg1) + ", " + itostr(m_syscall_args.arg2) +
         ", " + itostr(m_syscall_args.arg3) + ", " +
         itostr(m_syscall_args.arg4) + ", " + itostr(m_syscall_args.arg5) + ")";
}
