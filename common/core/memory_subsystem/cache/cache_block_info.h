...
class CacheBlockInfo {
 public:
  ...
  static const UInt8 BitsUsedOffset =
      3;  // Track usage on 1<<BitsUsedOffset granularity (per 64-bit / 8-byte)
  typedef UInt8 BitsUsedType;  // Enough to store one bit per 1<<BitsUsedOffset
                               // byte element per cache line (8 8-byte elements
 ...
 private:
  ...
  UInt64 m_owner;
  BitsUsedType m_used;
  UInt8 m_options;  // large enough to hold a bitfield for all available
                    // option_t's
