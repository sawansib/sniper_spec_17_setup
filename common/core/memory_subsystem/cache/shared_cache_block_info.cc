...
PrevCacheIndex SharedCacheBlockInfo::getCachedLoc() {
  LOG_ASSERT_ERROR(m_cached_locs.count() == 1, "m_cached_locs.count() == %u",
                   m_cached_locs.count());

  for (PrevCacheIndex idx = 0; idx < m_cached_locs.size(); ++idx)
    if (m_cached_locs.test(idx)) return idx;
  assert(false);
}

void SharedCacheBlockInfo::setCachedLoc(PrevCacheIndex idx) {
  LOG_ASSERT_ERROR(m_cached_locs.test(idx) == false,
                   "location %u already in set", idx);

  m_cached_locs.set(idx);
}

void SharedCacheBlockInfo::clearCachedLoc(PrevCacheIndex idx) {
