...
CacheSetNRU::CacheSetNRU(CacheBase::cache_t cache_type, UInt32 associativity,
                         UInt32 blocksize)
    : CacheSet(cache_type, associativity, blocksize) {
  m_lru_bits = new UInt8[m_associativity];
  for (UInt32 i = 0; i < m_associativity; i++)
    m_lru_bits[i] = 0;  // initially, lru bits of each set are set to zero, they
                        // are not touched yet
...
CacheSetNRU::~CacheSetNRU() { delete[] m_lru_bits; }

UInt32 CacheSetNRU::getReplacementIndex(CacheCntlr *cntlr) {
  ...
  for (UInt32 i = 0; i < m_associativity; i++) {
    if (!m_cache_block_info_array[i]->isValid()) {
      ...
      // of other lines, we choose the first invalid line to replace Mark our
      // newly-inserted line as recently used
      updateReplacementIndex(i);
      return i;
    } else if (m_lru_bits[i] == 0 && isValidReplacement(i)) {
      // Check if there are any valid replacement candidates with their LRU bit
      // set to zero If there are none, we'll ignore the LRU bit below and
      // select the first entry with isValidReplacement() as the victim
      have_zero_bit = true;
    }
  }
  ...
  for (UInt32 i = 0; i < m_associativity; i++) {
    if ((m_lru_bits[m_replacement_pointer] == 0 || have_zero_bit == false) &&
      ...
      // We choose the first non-touched line as the victim (note that we start
      // searching from the replacement pointer position)
