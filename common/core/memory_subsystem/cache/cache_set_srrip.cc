...
CacheSetSRRIP::CacheSetSRRIP(String cfgname, core_id_t core_id,
                             CacheBase::cache_t cache_type,
                             UInt32 associativity, UInt32 blocksize,
                             CacheSetInfoLRU* set_info, UInt8 num_attempts)
    : CacheSet(cache_type, associativity, blocksize),
      m_rrip_numbits(
          Sim()->getCfg()->getIntArray(cfgname + "/srrip/bits", core_id)),
  ...
  for (UInt32 i = 0; i < m_associativity; i++) m_rrip_bits[i] = m_rrip_insert;
}

CacheSetSRRIP::~CacheSetSRRIP() { delete[] m_rrip_bits; }

UInt32 CacheSetSRRIP::getReplacementIndex(CacheCntlr* cntlr) {
  for (UInt32 i = 0; i < m_associativity; i++) {
    if (!m_cache_block_info_array[i]->isValid()) {
      // If there is an invalid line(s) in the set, regardless of the LRU bits
      // of other lines, we choose the first invalid line to replace Prepare way
      // for a new line: set prediction to 'long'
    ...
    for (UInt32 i = 0; i < m_associativity; i++) {
      if (m_rrip_bits[m_replacement_pointer] >= m_rrip_max) {
        ...
        if (attempt < m_num_attempts - 1) {
          LOG_ASSERT_ERROR(cntlr != NULL,
                           "CacheCntlr == NULL, QBS can only be used when "
                           "cntlr is passed in");
          qbs_reject =
              cntlr->isInLowerLevelCache(m_cache_block_info_array[index]);
        }
        ...
        LOG_ASSERT_ERROR(isValidReplacement(index),
                         "SRRIP selected an invalid replacement candidate");
        return index;
      }
