...
// Define to enable tracking of which previous-level caches share each cache
// line Currently this is only used for asserts (makeing sure no non-sharers
// send use evictions) but it takes up space, especially when sharing caches
// with a lot of cores
...
typedef UInt8
    PrevCacheIndex;  // Should hold an integer up to MAX_NUM_PREVCACHES
#endif
