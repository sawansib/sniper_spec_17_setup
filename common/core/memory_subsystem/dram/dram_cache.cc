...
DramCache::DramCache(MemoryManagerBase* memory_manager,
  ...
  m_cache =
      new Cache("dram-cache", "perf_model/dram/cache", m_core_id, num_sets,
                associativity, m_cache_block_size,
...
boost::tuple<SubsecondTime, HitWhere::where_t> DramCache::getDataFromDram(
    ...
    ShmemPerf* perf) {
  std::pair<bool, SubsecondTime> res =
      doAccess(Cache::LOAD, address, requester, data_buf, now, perf);

  if (!res.first) ++m_read_misses;
  ++m_reads;

  return boost::tuple<SubsecondTime, HitWhere::where_t>(
      res.second, res.first ? HitWhere::DRAM_CACHE : HitWhere::DRAM);
}

boost::tuple<SubsecondTime, HitWhere::where_t> DramCache::putDataToDram(
    IntPtr address, core_id_t requester, Byte* data_buf, SubsecondTime now) {
  std::pair<bool, SubsecondTime> res =
      doAccess(Cache::STORE, address, requester, data_buf, now, NULL);

  if (!res.first) ++m_write_misses;
  ++m_writes;
...
std::pair<bool, SubsecondTime> DramCache::doAccess(
    Cache::access_t access, IntPtr address, core_id_t requester, Byte* data_buf,
  ...
  if (block_info) {
    ...
    m_cache->accessSingleLine(address, access, data_buf, m_cache_block_size,
                              now + latency, true);
    ...
    if (access == Cache::LOAD) {
      ...
      boost::tie(dram_latency, hit_where) = m_dram_cntlr->getDataFromDram(
          address, requester, data_buf, now + latency, perf);
      latency += dram_latency;
    }
    // For STOREs, we only do complete cache lines so we don't need to read from
    // DRAM
...
void DramCache::insertLine(Cache::access_t access, IntPtr address,
                           core_id_t requester, Byte* data_buf,
                           SubsecondTime now) {
  bool eviction;
  ...
  m_cache->insertSingleLine(address, data_buf, &eviction, &evict_address,
                            &evict_block_info, evict_buf, now);
  m_cache->peekSingleLine(address)->setCState(
      access == Cache::STORE ? CacheState::MODIFIED : CacheState::SHARED);
...
SubsecondTime DramCache::accessDataArray(Cache::access_t access,
                                         core_id_t requester,
...
void DramCache::callPrefetcher(IntPtr train_address, bool cache_hit,
                               bool prefetch_hit, SubsecondTime t_issue) {
  // Always train the prefetcher
  std::vector<IntPtr> prefetchList =
      m_prefetcher->getNextAddress(train_address, INVALID_CORE_ID);
  ...
  if (!cache_hit || (m_prefetch_on_prefetch_hit && prefetch_hit)) {
    for (std::vector<IntPtr>::iterator it = prefetchList.begin();
      ...
      if (!m_cache->peekSingleLine(prefetch_address)) {
        ...
        boost::tie(dram_latency, hit_where) = m_dram_cntlr->getDataFromDram(
            prefetch_address, m_core_id, data_buf, t_issue, NULL);
        // Insert into data array
        insertLine(Cache::LOAD, prefetch_address, m_core_id, data_buf,
                   t_issue + dram_latency);
        // Set prefetched bit
        PrL1CacheBlockInfo* block_info =
            (PrL1CacheBlockInfo*)m_cache->peekSingleLine(prefetch_address);
        block_info->setOption(CacheBlockInfo::PREFETCH);
        // Update completion time
        m_prefetch_mshr.getCompletionTime(t_issue, dram_latency,
                                          prefetch_address);
