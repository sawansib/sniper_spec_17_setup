...
std::vector<std::vector<CheetahModel*> > CheetahManager::s_cheetah_models(
    NUM_CHEETAH_TYPES);
const char* CheetahManager::cheetah_names[] = {"local", "by-2", "by-4", "by-8",
                                               "global"};

CheetahManager::CheetahManager(core_id_t core_id)
    : m_min_bits(Sim()->getCfg()->getInt("core/cheetah/min_size_bits")),
      m_max_bits_local(
          Sim()->getCfg()->getInt("core/cheetah/max_size_bits_local")),
      m_max_bits_global(
          Sim()->getCfg()->getInt("core/cheetah/max_size_bits_global")),
      m_address_buffer_size(0) {
  LOG_ASSERT_ERROR(m_min_bits >= CheetahModel::getMinSize(),
                   "cheetah/min_size_bits (%d) must be >= %d", m_min_bits,
                   CheetahModel::getMinSize());
  ...
  if (!s_cheetah_stats)
    s_cheetah_stats =
        new CheetahStats(m_min_bits, m_max_bits_local, m_max_bits_global);

  s_cheetah_models[CHEETAH_LOCAL].push_back(
      new CheetahModel(false, m_min_bits, m_max_bits_local));
  if ((core_id & 1) == 0)
    s_cheetah_models[CHEETAH_BY2].push_back(
        new CheetahModel(true, m_min_bits, m_max_bits_local));
  if ((core_id & 3) == 0)
    s_cheetah_models[CHEETAH_BY4].push_back(
        new CheetahModel(true, m_min_bits, m_max_bits_local));
  if ((core_id & 7) == 0)
    s_cheetah_models[CHEETAH_BY8].push_back(
        new CheetahModel(true, m_min_bits, m_max_bits_local));
  if (core_id == 0)
    s_cheetah_models[CHEETAH_GLOBAL].push_back(
        new CheetahModel(true, m_min_bits, m_max_bits_global));
...
CheetahManager::CheetahStats::CheetahStats(UInt32 min_bits,
...
void CheetahManager::CheetahStats::update() {
  for (unsigned int idx = 0; idx < NUM_CHEETAH_TYPES; ++idx) {
    ...
    for (auto it = s_cheetah_models[idx].begin();
         it != s_cheetah_models[idx].end(); ++it)
