...
CheetahSACLRU::CheetahSACLRU(int _N, int _B, int _A, int _L)
    : N(_N),
      ...
      L(_L),
      SAVE_INTERVAL(0),
      P_INTERVAL(0) /* unsupported for now */
      ,
      t_entries(0),
      depths(NULL),
      hitarr0(0),
...
void CheetahSACLRU::flush(void) {
  for (unsigned i = 0; i <= MAX_DEPTH; i++) sac_hits[0][i] += hitarr0;
  hitarr0 = 0;
}
...
Side effects: None
****************************************************************/
void CheetahSACLRU::outpr_saclru(FILE *fd) {
  unsigned i, j;
  uint64_t sum;
  ...
  fprintf(fd, "\t\t");
  for (i = 0; i < TWO_PWR_N; i++) fprintf(fd, "%d\t\t", (i + 1));
  fprintf(fd, "\n");
...
uint64_t CheetahSACLRU::hits(unsigned sets_log2, unsigned assoc) {
  assert(sets_log2 >= A);
  assert(sets_log2 <= B);
  ...
  uint64_t hits = 0;
  for (unsigned int j = 0; j < assoc; ++j) hits += sac_hits[j][sets_log2 - A];
...
Side effects: Allocates space for the arrays and initializes the array
         locations.
**********************************************************************/
void CheetahSACLRU::init_saclru(void) {
  uint64_t i, j, k, l;
  uint64_t init_value;
  ...
  assert(((uint64_t)(ONE << A) * TWO_POWER_MAX_DEPTH * TWO * BASE) <
         (uint64_t(1) << 32));
  arr = (uint64_t *)calloc(
      (uint64_t(ONE << A) * TWO_POWER_MAX_DEPTH * TWO * BASE),
      sizeof(uint64_t));
  if (!arr) fatal("out of virtual memory");
  ...
  if ((BASE_PWR_MAX_DEPTH_PLUS_ONE * sizeof(uint64_t)) < MEM_AVAIL_HITARR) {
    ...
    if (!hitarr) fatal("out of virtual memory");
  }

  base_pwr_array = (uint64_t *)calloc((MAX_DEPTH + 1), sizeof(uint64_t));
  if (!base_pwr_array) fatal("out of virtual memory");

  rm_arr = (uint64_t *)calloc((TWO_POWER_MAX_DEPTH), sizeof(uint64_t));
  if (!rm_arr) fatal("out of virtual memory");
  ...
  for (i = 0; i < ONE << A; i++) {
    ...
    slot_ptr = arr_ptr;
    *slot_ptr = 1;
    for (l = 1; l <= TWO_PWR_N; l++) *(slot_ptr + l) = init_value;
    ++init_value;
    for (j = 1; j <= MAX_DEPTH; j++) {
      for (k = ((ONE << j) - 1); k < (((ONE << j) - 1) + (ONE << (j - 1)));
           k++) {
  ...
  if (!depths) fatal("out of virtual memory");

  for (i = 0; i <= MAX_DEPTH; i++) depths[i] = 0;
}
...
*****************************************************************/
void CheetahSACLRU::sacnmul_woarr(intptr_t addr) {
  uint64_t t, t1;
  int i, fst;
  ...
  if (*(arr_ptr + 1) == orig_tag)
    ...
    for (i = 0; i <= (int)MAX_DEPTH; i++) depths[i] = 0;
  } /* else */
}
