...
Directory::Directory(core_id_t core_id, String directory_type_str,
                     UInt32 num_entries, UInt32 max_hw_sharers,
                     UInt32 max_num_sharers)
    : m_num_entries(num_entries),
      m_num_entries_allocated(0),
      m_max_hw_sharers(max_hw_sharers),
      m_use_max_hw_sharers(max_hw_sharers),  // Value to pass through to
                                             // DirectoryEntry::addSharer
  ...
  if (m_directory_type == LIMITLESS) {
    try {
      m_limitless_software_trap_penalty =
          SubsecondTime::NS() *
          Sim()->getCfg()->getInt(
              "perf_model/dram_directory/limitless/software_trap_penalty");
    } catch (...) {
      LOG_PRINT_ERROR(
          "Could not read 'cache_coherence/limitless/software_trap_penalty' "
          "from the config file");
    }
  }

  registerStatsMetric("directory", core_id, "entries-allocated",
                      &m_num_entries_allocated);
}

Directory::~Directory() {
  for (UInt32 i = 0; i < m_num_entries; i++) {
    if (m_directory_entry_list[i]) delete m_directory_entry_list[i];
  }
  delete[] m_directory_entry_list;
}

DirectoryEntry* Directory::getDirectoryEntry(UInt32 entry_num) {
  LOG_ASSERT_ERROR(entry_num < m_num_entries,
                   "Invalid entry_num(%d) >= num_entries(%d)", entry_num,
                   m_num_entries);
...
void Directory::setDirectoryEntry(UInt32 entry_num,
...
Directory::DirectoryType Directory::parseDirectoryType(
  ...
  if (directory_type_str == "full_map")
    ...
    LOG_PRINT_ERROR("Unsupported Directory Type: %s",
                    directory_type_str.c_str());
...
DirectoryEntry* Directory::createDirectoryEntrySized() {
  switch (m_directory_type) {
    case FULL_MAP:
      ...
      return new DirectoryEntryLimitedNoBroadcast<DirectorySharers>(
          m_max_num_sharers, m_max_num_sharers);

    case LIMITED_NO_BROADCAST:
      return new DirectoryEntryLimitedNoBroadcast<DirectorySharers>(
          m_max_hw_sharers, m_max_num_sharers);

    case LIMITLESS:
      return new DirectoryEntryLimitless<DirectorySharers>(
          m_max_hw_sharers, m_max_num_sharers,
          m_limitless_software_trap_penalty);
