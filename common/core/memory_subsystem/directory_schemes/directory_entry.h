...
class DirectorySharersVector : public std::vector<bool> {
 public:
  DirectorySharersVector(UInt32 max_num_sharers)
      : std::vector<bool>(max_num_sharers, false) {}
...
class DirectoryEntry {
 ...
 public:
  ...
  virtual void removeSharer(core_id_t sharer_id,
                            bool reply_expected = false) = 0;
  virtual UInt32 getNumSharers() = 0;

  virtual core_id_t getOwner() = 0;
  virtual void setOwner(core_id_t owner_id) = 0;

  virtual core_id_t getForwarder() { return m_forwarder_id; }
...
class DirectoryEntrySized : public DirectoryEntry {
 ...
 public:
  DirectoryEntrySized(UInt32 max_hw_sharers, UInt32 max_num_sharers)
      : DirectoryEntry(), m_sharers(max_num_sharers) {}
