...
class Dram : public CacheBase {
 ...
 public:
  Dram(Core* core, String name, UInt64 latency)
    ...
    registerStatsMetric(name, core->getId(), "total-access-latency",
                        &m_total_latency);
  }
...
class Cache : public CacheBase {
 ...
 public:
  Cache(Core* core, String name, MemComponent::component_t mem_component,
        UInt64 latency, CacheBase* next_level)
      : m_mem_component(mem_component),
        ...
        m_sets(m_num_sets) {
    LOG_ASSERT_ERROR(UInt64(1) << floorLog2(m_num_sets) == m_num_sets,
...
class CacheLocked : public Cache<assoc, size_kb> {
 ...
 public:
  CacheLocked(Core* core, String name, MemComponent::component_t mem_component,
              UInt64 latency, CacheBase* next_level)
