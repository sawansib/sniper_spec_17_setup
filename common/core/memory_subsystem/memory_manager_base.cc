...
MemoryManagerBase* MemoryManagerBase::createMMU(
    String protocol_type, Core* core, Network* network,
  ...
  switch (caching_protocol) {
    case PARAMETRIC_DRAM_DIRECTORY_MSI:
      return new ParametricDramDirectoryMSI::MemoryManager(core, network,
                                                           shmem_perf_model);
...
std::vector<core_id_t> MemoryManagerBase::getCoreListWithMemoryControllers() {
  ...
  try {
    ...
    memory_controllers_interleaving =
        Sim()->getCfg()->getInt("perf_model/dram/controllers_interleaving") *
        smt_cores;
    memory_controller_positions_from_cfg_file =
        Sim()->getCfg()->getString("perf_model/dram/controller_positions");
  } catch (...) {
    LOG_PRINT_ERROR(
        "Error reading number of memory controllers or controller positions");
  }

  LOG_ASSERT_ERROR(num_memory_controllers <= core_count,
                   "Num Memory Controllers(%i), Num Cores(%i)",
                   num_memory_controllers, core_count);

  if (num_memory_controllers != -1) {
    std::vector<core_id_t> core_list_from_cfg_file;
    parseMemoryControllerList(memory_controller_positions_from_cfg_file,
                              core_list_from_cfg_file, core_count);

    LOG_ASSERT_ERROR(
        (core_list_from_cfg_file.size() == 0) ||
    ...
    if (core_list_from_cfg_file.size() > 0) {
      ...
      try {
        ...
        l_models_memory_1 = NetworkModel::parseNetworkType(
      ...
      std::pair<bool, std::vector<core_id_t> >
          core_list_with_memory_controllers_1 =
              NetworkModel::computeMemoryControllerPositions(
                  l_models_memory_1, num_memory_controllers, core_count);
    ...
    if (memory_controllers_interleaving) {
      num_memory_controllers =
          (core_count + memory_controllers_interleaving - 1) /
          memory_controllers_interleaving;  // Round up
      for (core_id_t i = 0; i < num_memory_controllers; i++) {
        assert((i * memory_controllers_interleaving) < core_count);
        core_list_with_memory_controllers.push_back(
...
void MemoryManagerBase::parseMemoryControllerList(
    ...
    std::vector<core_id_t>& core_list_from_cfg_file, SInt32 core_count) {
  if (memory_controller_positions == "") return;
  ...
  while (!end_reached) {
    ...
    LOG_ASSERT_ERROR(core_num < core_count, "core_num(%i), num_cores(%i)",
                     core_num, core_count);
...
void MemoryManagerBase::printCoreListWithMemoryControllers(
  ...
  for (std::vector<core_id_t>::iterator it =
           core_list_with_memory_controllers.begin();
       it != core_list_with_memory_controllers.end(); it++) {
    core_list << *it << " ";
  }
