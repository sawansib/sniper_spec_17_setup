...
SimplePrefetcher::SimplePrefetcher(String configName, core_id_t _core_id,
                                   UInt32 _shared_cores)
    : core_id(_core_id),
      shared_cores(_shared_cores),
      n_flows(Sim()->getCfg()->getIntArray(
          "perf_model/" + configName + "/prefetcher/simple/flows", core_id)),
      flows_per_core(Sim()->getCfg()->getBoolArray(
          "perf_model/" + configName + "/prefetcher/simple/flows_per_core",
          core_id)),
      num_prefetches(Sim()->getCfg()->getIntArray(
          "perf_model/" + configName + "/prefetcher/simple/num_prefetches",
          core_id)),
      stop_at_page(Sim()->getCfg()->getBoolArray(
          "perf_model/" + configName +
              "/prefetcher/simple/stop_at_page_boundary",
          core_id)),
      n_flow_next(0),
...
std::vector<IntPtr> SimplePrefetcher::getNextAddress(IntPtr current_address,
                                                     core_id_t _core_id) {
  std::vector<IntPtr> &prev_address =
      m_prev_address.at(flows_per_core ? _core_id - core_id : 0);
  ...
  if (stride != 0) {
    for (unsigned int i = 0; i < num_prefetches; ++i) {
      ...
      if (!stop_at_page ||
          ((prefetch_address & PAGE_MASK) == (current_address & PAGE_MASK)))
