...
ATD::ATD(String name, String configName, core_id_t core_id, UInt32 num_sets,
         UInt32 associativity, UInt32 cache_block_size,
         String replacement_policy, CacheBase::hash_t hash_function)
    : m_cache_base(name, num_sets, associativity, cache_block_size,
                   hash_function),
      m_sets(),
      loads(0),
      stores(0),
  ...
  m_set_info = CacheSet::createCacheSetInfo(name, configName, core_id,
                                            replacement_policy, associativity);
  ...
  registerStatsMetric(name, core_id, "stores-constructive",
                      &stores_constructive);
  registerStatsMetric(name, core_id, "stores-destructive", &stores_destructive);

  String sampling =
      Sim()->getCfg()->getStringArray(configName + "/atd/sampling", core_id);
  if (sampling == "full") {
    for (UInt64 set_index = 0; set_index < num_sets; ++set_index) {
      m_sets[set_index] = CacheSet::createCacheSet(
          name, core_id, replacement_policy, CacheBase::PR_L1_CACHE,
          associativity, 0, m_set_info);
    }
  } else if (sampling == "2^n+1") {
    // Sample sets at indexes 2^N+1
    for (UInt64 set_index = 1; set_index < num_sets - 1; set_index <<= 1) {
      m_sets[set_index + 1] = CacheSet::createCacheSet(
          name, core_id, replacement_policy, CacheBase::PR_L1_CACHE,
          associativity, 0, m_set_info);
    }
  } else if (sampling == "random") {
    // Sample sets at random positions
    UInt64 state = rng_seed(Sim()->getCfg()->getIntArray(
        configName + "/atd/sampling/random/seed", core_id));
    UInt64 num_atds = Sim()->getCfg()->getIntArray(
        configName + "/atd/sampling/random/count", core_id);
    UInt64 num_attempts = 0;

    LOG_ASSERT_ERROR(
        num_atds <= num_sets,
        "Cannot sample more sets (%d) than the total number of sets (%d)",
        num_atds, num_sets);
    ...
    while (num_atds) {
      ...
      LOG_ASSERT_ERROR(
          ++num_attempts < 10 * num_sets,
          "Cound not find unique ATD sets even after many attempts");
    }
  } else {
...
ATD::~ATD() {
  if (m_set_info) delete m_set_info;
}

bool ATD::isSampledSet(UInt32 set_index) { return m_sets.count(set_index); }
...
void ATD::access(Core::mem_op_t mem_op_type, bool cache_hit, IntPtr address) {
  ...
  if (isSampledSet(set_index)) {
    ...
    if (atd_hit) {
      ...
      PrL1CacheBlockInfo* cache_block_info =
          new PrL1CacheBlockInfo(tag, CacheState::MODIFIED);
      bool eviction;
      PrL1CacheBlockInfo evict_block_info;
      m_sets[set_index]->insert(cache_block_info, NULL, &eviction,
                                &evict_block_info, NULL);
    }

    if (mem_op_type == Core::WRITE) {
      ++stores;
      if (!atd_hit) ++store_misses;
    } else {
      ++loads;
      if (!atd_hit) ++load_misses;
    }
