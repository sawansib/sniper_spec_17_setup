...
class CacheParameters {
 public:
  ...
  CacheParameters(String _configName, UInt32 _size, UInt32 _associativity,
                  UInt32 block_size, String _hash_function,
...
class CacheMasterCntlr {
 private:
  ...
  void createATDs(String name, String configName, core_id_t core_id,
                  UInt32 shared_cores, UInt32 size, UInt32 associativity,
                  UInt32 block_size, String replacement_policy,
                  CacheBase::hash_t hash_function);
  void accessATDs(Core::mem_op_t mem_op_type, bool hit, IntPtr address,
                  UInt32 core_num);
...
class CacheCntlr : ::CacheCntlr {
 private:
  ...
  struct {
    ...
    // for all prefetched lines, some may still be in the cache, or could have
    // been removed for some other reason. Also, in a shared cache, the prefetch
    // may have been triggered by another core than the one accessing/evicting
    // the line so *_prefetch statistics should be summed across the shared
    // cache
  ...
  boost::tuple<HitWhere::where_t, SubsecondTime> accessDRAM(
