...
NucaCache::NucaCache(MemoryManagerBase* memory_manager,
  ...
  m_cache = new Cache("nuca-cache", "perf_model/nuca/cache", m_core_id,
                      ...
                      m_cache_block_size, parameters.replacement_policy,
...
boost::tuple<SubsecondTime, HitWhere::where_t> NucaCache::write(
    IntPtr address, Byte* data_buf, bool& eviction, IntPtr& evict_address,
    Byte* evict_buf, SubsecondTime now, bool count) {
  HitWhere::where_t hit_where = HitWhere::MISS;
  ...
  if (block_info) {
    ...
    m_cache->insertSingleLine(address, data_buf, &eviction, &evict_address,
                              &evict_block_info, evict_buf, now + latency);
...
SubsecondTime NucaCache::accessDataArray(Cache::access_t access,
  ...
  if (m_queue_model) {
    ...
    queue_delay = processing_time + m_queue_model->computeQueueDelay(
                                        t_start, processing_time, m_core_id);
