...
GhbPrefetcher::GhbPrefetcher(String configName, core_id_t core_id)
    : m_prefetchWidth(Sim()->getCfg()->getIntArray(
          "perf_model/" + configName + "/prefetcher/ghb/width", core_id)),
      m_prefetchDepth(Sim()->getCfg()->getIntArray(
          "perf_model/" + configName + "/prefetcher/ghb/depth", core_id)),
      m_lastAddress(INVALID_ADDRESS),
      m_ghbSize(Sim()->getCfg()->getIntArray(
          "perf_model/" + configName + "/prefetcher/ghb/ghb_size", core_id)),
      m_ghbHead(0),
      m_generation(0),
      m_ghb(m_ghbSize),
      m_tableSize(Sim()->getCfg()->getIntArray(
          "perf_model/" + configName + "/prefetcher/ghb/ghb_table_size",
          core_id)),
      m_tableHead(0),
...
std::vector<IntPtr> GhbPrefetcher::getNextAddress(IntPtr currentAddress,
                                                  core_id_t core_id) {
  ...
  UInt32 i = 0;
  while (i < m_tableSize && m_ghbTable[i].delta != INVALID_DELTA &&
         m_ghbTable[i].delta != delta)
    ++i;

  if (i != m_tableSize && m_ghbTable[i].delta == delta &&
      m_ghbTable[i].generation ==
          m_ghb[m_ghbTable[i].ghbIndex]
              .generation)  // check if the table still points to the current
                            // GHB 'generation'
  {
    UInt32 width = 0;
    ...
    while (width < m_prefetchWidth && ghbIndex != INVALID_INDEX) {
      ...
      while (depth < m_prefetchDepth &&
        ...
        if (find(prefetchList.begin(), prefetchList.end(), newAddress) ==
            prefetchList.end())
      ...
      if ((nextIndex > ghbIndex ||  // if we circle the GHB
           ghbIndex >
               m_ghbHead) &&  // OR we were already larger than the head pointer
          nextIndex < m_ghbHead)  // AND the nextIndex has been overwritten by
                                  // new entries
  ...
  if (prevDelta != INVALID_DELTA) {
    ...
    if (i != m_tableSize &&
        m_ghbTable[i].delta == prevDelta) {  // update existing entry
    ...
    } else {  // prevDelta not found ==> add entry to table
