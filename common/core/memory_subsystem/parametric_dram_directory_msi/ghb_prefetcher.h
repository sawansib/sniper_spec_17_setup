...
class GhbPrefetcher : public Prefetcher {
 ...
 private:
  ...
  struct GHBEntry {
    ...
    UInt32 generation;
    GHBEntry()
        : nextIndex(INVALID_INDEX), delta(INVALID_DELTA), generation(0) {}
  };
  ...
  struct TableEntry {
    ...
    UInt32 generation;
    TableEntry()
        : ghbIndex(INVALID_INDEX), delta(INVALID_DELTA), generation(0) {}
  };
  ...
  UInt32 m_tableSize;
  UInt32 m_tableHead;  // next table position to be overwritten (in lack of a
                       // better replacement policy at the moment)
