...
ShmemMsg::ShmemMsg(msg_t msg_type,
                   ...
                   core_id_t requester, IntPtr address, Byte* data_buf,
                   UInt32 data_length, ShmemPerf* perf)
    : m_msg_type(msg_type),
...
ShmemMsg* ShmemMsg::getShmemMsg(Byte* msg_buf) {
  ...
  if (shmem_msg->getDataLength() > 0) {
    ...
    memcpy((void*)shmem_msg->getDataBuf(), msg_buf + sizeof(*shmem_msg),
           shmem_msg->getDataLength());
  }
  return shmem_msg;
}

Byte* ShmemMsg::makeMsgBuf() {
...
UInt32 ShmemMsg::getMsgLen() { return (sizeof(*this) + m_data_length); }

UInt32 ShmemMsg::getModeledLength() {
  switch (m_msg_type) {
    case EX_REQ:
    case SH_REQ:
    case INV_REQ:
