...
const char* shmem_reason_names[] = {
    "noc-base",        "noc-queue",        "td-access",
    "inv-imbalance",   "remote-cache-inv", "remote-cache-fwd",
    "remote-cache-wb", "pending-hit",      "nuca-tags",
    "nuca-bus",        "nuca-queue",       "nuca-data",
    "dram-cache",      "dram-cache-tags",  "dram-cache-queue",
    "dram-cache-bus",  "dram-cache-data",  "dram",
    "dram-queue",      "dram-bus",         "dram-device",
    "unknown",
};

static_assert(ShmemPerf::NUM_SHMEM_TIMES ==
                  sizeof(shmem_reason_names) / sizeof(shmem_reason_names[0]),
              "Not enough values in shmem_reason_names");

const char* ShmemReasonString(ShmemPerf::shmem_times_type_t reason) {
  LOG_ASSERT_ERROR(reason < ShmemPerf::NUM_SHMEM_TIMES,
                   "Invalid ShmemPerf reason %d", reason);
...
void ShmemPerf::updateTime(SubsecondTime time, shmem_times_type_t reason) {
  ...
  if (this) {
    LOG_ASSERT_ERROR(reason < NUM_SHMEM_TIMES, "Invalid ShmemPerf reason %d",
                     reason);
