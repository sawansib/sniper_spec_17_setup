...
DramDirectoryCache::DramDirectoryCache(
    core_id_t core_id, String directory_type_str, UInt32 total_entries,
    UInt32 associativity, UInt32 cache_block_size, UInt32 max_hw_sharers,
    UInt32 max_num_sharers, ComponentLatency dram_directory_cache_access_time,
  ...
  m_directory = new Directory(core_id, directory_type_str, total_entries,
...
DramDirectoryCache::~DramDirectoryCache() { delete m_directory; }

DirectoryEntry* DramDirectoryCache::getDirectoryEntry(IntPtr address,
                                                      bool modeled) {
  if (m_shmem_perf_model && modeled)
    getShmemPerfModel()->incrElapsedTime(
        m_dram_directory_cache_access_time.getLatency(),
  ...
  for (it = m_replaced_directory_entry_list.begin();
       it != m_replaced_directory_entry_list.end(); it++) {
...
void DramDirectoryCache::getReplacementCandidates(
    IntPtr address, std::vector<DirectoryEntry*>& replacement_candidate_list) {
  ...
  for (UInt32 i = 0; i < m_associativity; i++) {
    replacement_candidate_list.push_back(
        m_directory->getDirectoryEntry(set_index * m_associativity + i));
  }
}

DirectoryEntry* DramDirectoryCache::replaceDirectoryEntry(
    IntPtr replaced_address, IntPtr address, bool modeled) {
  if (m_shmem_perf_model && modeled)
    getShmemPerfModel()->incrElapsedTime(
        m_dram_directory_cache_access_time.getLatency(),
        ShmemPerfModel::_SIM_THREAD);
  ...
  for (UInt32 i = 0; i < m_associativity; i++) {
    ...
    if (replaced_directory_entry->getAddress() == replaced_address) {
      ...
      directory_entry->setAddress(address);
      m_directory->setDirectoryEntry(set_index * m_associativity + i,
                                     directory_entry);
...
void DramDirectoryCache::invalidateDirectoryEntry(IntPtr address) {
  std::vector<DirectoryEntry*>::iterator it;
  for (it = m_replaced_directory_entry_list.begin();
       it != m_replaced_directory_entry_list.end(); it++) {
    if ((*it)->getAddress() == address) {
...
void DramDirectoryCache::splitAddress(IntPtr address, IntPtr& tag,
                                      UInt32& set_index) {
