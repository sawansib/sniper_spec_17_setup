...
DramDirectoryCntlr::DramDirectoryCntlr(
    ...
    AddressHomeLookup* dram_controller_home_lookup, NucaCache* nuca_cache,
...
void DramDirectoryCntlr::handleMsgFromL2Cache(core_id_t sender,
  ...
  // we call getDirectoryEntry with modeled==true), elsewhere we assume
  // outstanding requests are stored in a fast MSHR-like structure
...
void DramDirectoryCntlr::retrieveDataAndSendToL2Cache(
  ...
  if (cached_data_buf != NULL) {
    ...
    getMemoryManager()->sendMsg(
        ...
        HitWhere::CACHE_REMOTE /* cached_data_buf was filled by a WB_REQ or
                                  FLUSH_REQ */
        ,
    ...
    if (m_nuca_cache) {
      ...
      boost::tie(nuca_latency, hit_where) = m_nuca_cache->read(
          address, nuca_data_buf,
          getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_SIM_THREAD),
      ...
      if (hit_where != HitWhere::MISS) {
        getMemoryManager()->sendMsg(
            reply_msg_type, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
            receiver /* requester */, receiver /* receiver */, address,
            nuca_data_buf, getCacheBlockSize(), HitWhere::NUCA_CACHE,
            orig_shmem_msg->getPerf(), ShmemPerfModel::_SIM_THREAD);
...
void DramDirectoryCntlr::processDRAMReply(core_id_t sender,
  ...
  // Keep a copy in NUCA
  sendDataToNUCA(
      address, shmem_req->getShmemMsg()->getRequester(),
...
void DramDirectoryCntlr::processInvRepFromL2Cache(core_id_t sender,
  ...
  if (m_dram_directory_req_queue_list->size(address) > 0) {
    ...
    if (shmem_req->getShmemMsg()->getMsgType() == ShmemMsg::EX_REQ) {
      ...
      // only required when all SHARED copies have been invalided
      ...
      if (shmem_req->getWaitForData()) {
        ...
        // eviction), the next request should only be woken up once its data
        // arrives from DRAM.
...
void DramDirectoryCntlr::processFlushRepFromL2Cache(core_id_t sender,
  ...
  // clean data to avoid a memory access
  if (directory_entry->getNumSharers() == 0) {
    directory_block_info->setDState(DirectoryState::UNCACHED);
  } else {
    assert(directory_block_info->getDState() == DirectoryState::SHARED);
  ...
  if (m_dram_directory_req_queue_list->size(address) != 0) {
    ...
    if (shmem_req->getShmemMsg()->getMsgType() == ShmemMsg::EX_REQ) {
      ...
      if (directory_entry->getNumSharers() == 0) {
        ...
        // anymore One of the sharers whas FLUSHed instead of INV to get the
        // data. This is it's reply
...
void DramDirectoryCntlr::sendDataToNUCA(IntPtr address, core_id_t requester,
  ...
  if (m_nuca_cache) {
    ...
    m_nuca_cache->write(
        address, data_buf, eviction, evict_address, evict_buf,
        getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_SIM_THREAD),
        count);
    ...
    if (eviction) {
      ...
      getMemoryManager()->sendMsg(
          ...
          MemComponent::TAG_DIR, MemComponent::DRAM, m_core_id /* requester */,
          dram_node /* receiver */, evict_address, evict_buf,
          getCacheBlockSize(), HitWhere::UNKNOWN, NULL,
          ShmemPerfModel::_SIM_THREAD);
    }
  }
  MYLOG("End");
...
void DramDirectoryCntlr::sendDataToDram(IntPtr address, core_id_t requester,
  ...
  if (m_nuca_cache) {
    // If we have a NUCA cache: write it there, it will be written to DRAM on
    // eviction
    ...
    getMemoryManager()->sendMsg(
        PrL1PrL2DramDirectoryMSI::ShmemMsg::DRAM_WRITE_REQ,
        MemComponent::TAG_DIR, MemComponent::DRAM, requester /* requester */,
        dram_node /* receiver */, address, data_buf, getCacheBlockSize(),
        HitWhere::UNKNOWN, NULL, ShmemPerfModel::_SIM_THREAD);
