...
Network::Network(Core *core) : _core(core) {
  LOG_ASSERT_ERROR(
      sizeof(g_type_to_static_network_map) / sizeof(EStaticNetwork) ==
          NUM_PACKET_TYPES,
  ...
  for (SInt32 i = 0; i < NUM_PACKET_TYPES; i++) _callbacks[i] = NULL;
  ...
  for (SInt32 i = 0; i < NUM_STATIC_NETWORKS; i++)
    _models[i] =
        NetworkModel::createModel(this, modelTypes[i], (EStaticNetwork)i);

  LOG_PRINT("Initialized.");
}

Network::~Network() {
  for (SInt32 i = 0; i < NUM_STATIC_NETWORKS; i++) delete _models[i];
...
void Network::registerCallback(PacketType type, NetworkCallback callback,
                               void *obj) {
...
void Network::netPullFromTransport() {
  do {
    ...
    LOG_PRINT("Pull packet : type %i, from %i, time %s", (SInt32)packet.type,
              packet.sender, itostr(packet.time).c_str());
    assert(0 <= packet.sender && packet.sender < _numMod);
    LOG_ASSERT_ERROR(0 <= packet.type && packet.type < NUM_PACKET_TYPES,
                     "Packet type: %d not between 0 and %d", packet.type,
                     NUM_PACKET_TYPES);
    ...
    if (packet.receiver != _core->getId()) {
      ...
      LOG_PRINT(
          "Forwarding packet : type %i, from %i, to %i, core_id %i, time %s.",
          (SInt32)packet.type, packet.sender, packet.receiver, _core->getId(),
          itostr(packet.time).c_str());
      forwardPacket(packet);

      // if this isn't a broadcast message, then we shouldn't process it further
      if (packet.receiver != NetPacket::BROADCAST) {
        if (packet.length > 0) delete[] (Byte *)packet.data;
        continue;
      }
    }
    ...
    if (callback != NULL) {
      LOG_PRINT(
          "Executing callback on packet : type %i, from %i, to %i, core_id %i, "
          "time %s",
          (SInt32)packet.type, packet.sender, packet.receiver, _core->getId(),
          itostr(packet.time).c_str());
      assert(0 <= packet.sender && packet.sender < _numMod);
    ...
    // synchronous I/O support
      ...
      LOG_PRINT(
          "Enqueuing packet : type %i, from %i, to %i, core_id %i, time %s.",
          (SInt32)packet.type, packet.sender, packet.receiver, _core->getId(),
          itostr(packet.time).c_str());
...
void Network::forwardPacket(NetPacket &packet) { netSend(packet); }
...
SInt32 Network::netSend(NetPacket &packet) {
  ...
  for (UInt32 i = 0; i < hopVec.size(); i++) {
    LOG_PRINT(
        "Send packet : type %i, from %i, to %i, next_hop %i, core_id %i, time "
        "%s",
        (SInt32)packet.type, packet.sender, hopVec[i].final_dest,
        hopVec[i].next_dest, _core->getId(), itostr(hopVec[i].time).c_str());
    // LOG_ASSERT_ERROR(hopVec[i].time >= packet.time, "hopVec[%d].time(%llu) <
    // packet.time(%llu)", i, hopVec[i].time, packet.time);
    ...
    if (hopVec[i].final_dest != NetPacket::BROADCAST) {
      ...
      while (hopVec[i].next_dest != hopVec[i].final_dest) {
        ...
        Core *remote_core =
            Sim()->getCoreManager()->getCoreFromID(hopVec[i].next_dest);
        NetworkModel *remote_network_model =
            remote_core->getNetwork()->getNetworkModelFromPacketType(
                packet.type);
    ...
    if (_core->getId() == buff_pkt->sender) buff_pkt->start_time = start_time;
...
class NetRecvIterator {
 public:
  NetRecvIterator(UInt32 i) : _mode(INT), _max(i), _i(0) {}
  NetRecvIterator(const std::vector<SInt32> &v)
      : _mode(SENDER_VECTOR), _senders(&v), _i(0) {}
  NetRecvIterator(const std::vector<PacketType> &v)
      : _mode(TYPE_VECTOR), _types(&v), _i(0) {}
  ...
  inline void reset() { _i = 0; }

 private:
  enum { INT, SENDER_VECTOR, TYPE_VECTOR } _mode;
...
NetPacket Network::netRecv(const NetMatch &match, UInt64 timeout_ns) {
  ...
  while (!found) {
    ...
    for (NetQueue::iterator i = _netQueue.begin(); i != _netQueue.end(); i++) {
      // only find packets that match
      for (sender.reset(); !sender.done(); sender.next()) {
        if (i->sender != (SInt32)sender.get()) continue;

        for (type.reset(); !type.done(); type.next()) {
          if (i->type != (PacketType)type.get()) continue;
    ...
    if (!found) {
      if (retry) {
        ...
        // After waking from either timeout or cond.signal, retry once but then
        // no more
        if (timeout_ns) retry = false;
      } else {
        // non-blocking: return a special packet with length == -1 to denote no
        // match
        _netQueueLock.release();
        NetPacket packet;
  ...
  assert((itr->receiver == _core->getId()) ||
         (itr->receiver == NetPacket::BROADCAST));
  ...
  LOG_PRINT("packet.time(%s), start_time(%s)", itostr(packet.time).c_str(),
            itostr(start_time).c_str());
  ...
  if (packet.time > start_time) {
    if (_core->getId() < (core_id_t)Sim()->getConfig()->getApplicationCores()) {
      if (packet.time - start_time < 100 * SubsecondTime::NS()) {
        ;  // Allow small timing differences, usually before performance models
           // are enabled
      } else {
        // We really should't do it this way. You're supposed to include the
        // time as a data item in the packet, and have the application layer
        // increment time (queueing a valid *Instruction which will attribute
        // the time increment to the correct CPI component)
        LOG_ASSERT_ERROR(false, "RecvInstruction(%s) being queued",
                         itostr(packet.time - start_time).c_str());
      }
    } else {
      // Not that non-application models even care about this (their core
      // performance models are never enabled). Why even bother?
      LOG_PRINT("Queueing RecvInstruction(%s)",
                itostr(packet.time - start_time).c_str());
  ...
  LOG_PRINT("Exiting netRecv : type %i, from %i", (SInt32)packet.type,
            packet.sender);
...
SInt32 Network::netSend(SInt32 dest, PacketType type, const void *buf,
                        UInt32 len) {
  NetPacket packet;
...
UInt32 Network::getModeledLength(const NetPacket &pkt) {
  if (pkt.type == SHARED_MEM_1) {
    ...
    UInt32 metadata_size =
        1 + 2 * Config::getSingleton()->getCoreIDLength() + 2;
    UInt32 data_size =
        getCore()->getMemoryManager()->getModeledLength(pkt.data);
...
NetPacket::NetPacket(SubsecondTime t, PacketType ty, SInt32 s, SInt32 r,
...
NetPacket::NetPacket(Byte *buffer) {
  ...
  // LOG_ASSERT_ERROR(length > 0, "type(%u), sender(%i), receiver(%i),
  // length(%u)", type, sender, receiver, length);
  if (length > 0) {
...
UInt32 NetPacket::bufferSize() const { return (sizeof(*this) + length); }
