...
NetworkModel::NetworkModel(Network *network, EStaticNetwork net_type)
    : _network(network),
      m_collect_traffic_matrix(
          Sim()->getCfg()->getBool("network/collect_traffic_matrix")) {
  ...
  if (m_collect_traffic_matrix) {
    ...
    for (UInt32 dst = 0; dst < ncores + 1; ++dst) {
      ...
      registerStatsMetric(netName, _network->getCore()->getId(),
                          "packets-to-" + dstName, &m_matrix_packets[dst]);
      registerStatsMetric(netName, _network->getCore()->getId(),
                          "bytes-to-" + dstName, &m_matrix_bytes[dst]);
    }
  }
}
...
void NetworkModel::countPacket(const NetPacket &packet) {
  if (m_collect_traffic_matrix) {
    SInt32 dst = packet.receiver == NetPacket::BROADCAST
                     ? m_matrix_bytes.size() - 1
                     : packet.receiver;
...
NetworkModel *NetworkModel::createModel(Network *net, UInt32 model_type,
...
UInt32 NetworkModel::parseNetworkType(String str) {
  if (str == "magic")
...
NetworkModel::computeMemoryControllerPositions(UInt32 network_type,
  ...
  switch (network_type) {
    ...
    case NETWORK_BUS: {
      SInt32 spacing_between_memory_controllers =
          core_count / num_memory_controllers;
      ...
      for (core_id_t i = 0; i < num_memory_controllers; i++) {
        ...
        core_list_with_memory_controllers.push_back(
            i * spacing_between_memory_controllers);
      }
    ...
    case NETWORK_EMESH_HOP_BY_HOP:
      return NetworkModelEMeshHopByHop::computeMemoryControllerPositions(
