...
NetworkModelEMeshHopCounter::NetworkModelEMeshHopCounter(
    Network *net, EStaticNetwork net_type)
  ...
  try {
    _linkBandwidth = ComponentBandwidthPerCycle(
        Sim()->getDvfsManager()->getCoreDomain(net->getCore()->getId()),
        Sim()->getCfg()->getInt("network/emesh_hop_counter/link_bandwidth"));
    _hopLatency = ComponentLatency(
        Sim()->getDvfsManager()->getCoreDomain(net->getCore()->getId()),
        Sim()->getCfg()->getInt("network/emesh_hop_counter/hop_latency"));
  } catch (...) {
    LOG_PRINT_ERROR(
        "Could not read emesh_hop_counter paramters from the cfg file");
  }
...
void NetworkModelEMeshHopCounter::computePosition(core_id_t core, SInt32 &x,
                                                  SInt32 &y) {
  x = core % _meshWidth;
  y = core / _meshWidth;
}

SInt32 NetworkModelEMeshHopCounter::computeDistance(SInt32 x1, SInt32 y1,
                                                    SInt32 x2, SInt32 y2) {
  ...
  if (pkt.receiver == NetPacket::BROADCAST) {
    ...
    for (SInt32 i = 0; i < (SInt32)total_cores; i++) {
      ...
      SubsecondTime latency =
          computeDistance(sx, sy, dx, dy) * _hopLatency.getLatency();
      if (i != pkt.sender) latency += serialization_latency;
    ...
    SubsecondTime latency =
        computeDistance(sx, sy, dx, dy) * _hopLatency.getLatency();
    if (pkt.receiver != pkt.sender) latency += serialization_latency;
...
void NetworkModelEMeshHopCounter::processReceivedPacket(NetPacket &pkt) {
  ScopedLock sl(_lock);
