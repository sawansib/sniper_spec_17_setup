...
NetworkModelBusGlobal *NetworkModelBus::_bus_global[NUM_STATIC_NETWORKS] = {
    NULL};

NetworkModelBusGlobal::NetworkModelBusGlobal(String name)
    : _bandwidth(8 *
                 Sim()->getCfg()->getFloat(
                     "network/bus/bandwidth")) /* = 8 * GB/s / Gcycles/s = bits
                                                  / cycle, round down (implicit:
  ...
  String model_type =
      Sim()->getCfg()->getString("network/bus/queue_model/type");
  // Emulate the original code, with 10 cycles of latency for the history_list,
  // and 0 outstanding transactions for the contention model
  SubsecondTime proc_period = ComponentPeriod::fromFreqHz(
      Sim()->getCfg()->getFloatArray("perf_model/core/frequency", 0) *
      1000000000);
  _queue_model =
      QueueModel::create("bus-queue", 0, model_type, 10 * proc_period);
  /* 8 * GB/s / Gcycles/s = bits / cycle, round down (implicit: float to int
   * conversion) */
...
NetworkModelBusGlobal::~NetworkModelBusGlobal() { delete _queue_model; }

/* Model bus utilization. In: packet start time and size. Out: packet out time
 */
SubsecondTime NetworkModelBusGlobal::useBus(
    SubsecondTime t_start, UInt32 length, subsecond_time_t *queue_delay_stats) {
  ...
  if (queue_delay_stats) *queue_delay_stats += t_queue;
  if (t_queue > SubsecondTime::Zero()) _num_packets_delayed++;
...
NetworkModelBus::NetworkModelBus(Network *net, EStaticNetwork net_type)
    : NetworkModel(net, net_type),
      _enabled(false),
      _ignore_local(
          Sim()->getCfg()->getBool("network/bus/ignore_local_traffic")) {
...
void NetworkModelBus::routePacket(const NetPacket &pkt,
...
void NetworkModelBus::processReceivedPacket(NetPacket &pkt) {}

bool NetworkModelBus::accountPacket(const NetPacket &pkt) {
  ...
  if (!_enabled ||
      ...
      (pkt.sender >=
       (core_id_t)Config::getSingleton()->getApplicationCores()) ||
      (pkt.receiver >=
       (core_id_t)Config::getSingleton()->getApplicationCores()))
    return false;
  else
