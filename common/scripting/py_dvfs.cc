  ...
  if (domain_id < 0) {
    if (allow_global) {
      ...
      PyErr_SetString(PyExc_ValueError,
                      "Can only set core frequency, not global domains");
      return NULL;
    }
  } else {
...
static PyObject *getFrequency(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "l", &domain_id)) return NULL;
...
static PyObject *setFrequency(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "ll", &core_id, &freq_mhz)) return NULL;

  const ComponentPeriod *domain = getDomain(core_id, false);
  if (!domain) return NULL;

  // We're running in a hook so we already have the thread lock, call
  // MagicServer directly
...
static PyMethodDef PyDvfsMethods[] = {
    {"get_frequency", getFrequency, METH_VARARGS,
     "Get core or global frequency, in MHz."},
    {"set_frequency", setFrequency, METH_VARARGS,
     "Set core frequency, in MHz."},
