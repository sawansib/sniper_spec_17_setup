...
static PyObject *getStatsValue(PyObject *self, PyObject *args) {
  ...
  StatsMetricBase *metric =
      Sim()->getStatsManager()->getMetricObject(objectName, index, metricName);
...
//////////
// getter(): return a statsGetterObject Python object which, when called,
...
static PyObject *getStatsGetter(PyObject *self, PyObject *args) {
  ...
  statsGetterObject *pGetter =
      PyObject_New(statsGetterObject, &statsGetterType);
...
static UInt64 statsCallback(String objectName, UInt32 index, String metricName,
                            UInt64 _pFunc) {
  PyObject *pFunc = (PyObject *)_pFunc;
  PyObject *pResult = HooksPy::callPythonFunction(
      pFunc,
      Py_BuildValue("(sls)", objectName.c_str(), index, metricName.c_str()));

  if (!pResult || !PyLong_Check(pResult)) {
    LOG_PRINT_WARNING(
        "Stats callback: return value must be (convertable into) 64-bit "
        "unsigned integer");
...
static PyObject *registerStats(PyObject *self, PyObject *args) {
  ...
  Sim()->getStatsManager()->registerMetric(
      new StatsMetricCallback(objectName, index, metricName,
                              (StatsCallback)statsCallback, (UInt64)pFunc));
...
static PyObject *writeMarker(PyObject *self, PyObject *args) {
  UInt64 core_id = INVALID_CORE_ID, thread_id = INVALID_THREAD_ID, arg0 = 0,
         arg1 = 0;
  const char *description = NULL;

  if (!PyArg_ParseTuple(args, "llll|z", &core_id, &thread_id, &arg0, &arg1,
                        &description))
    return NULL;

  Sim()->getStatsManager()->logMarker(
      Sim()->getClockSkewMinimizationServer()->getGlobalTime(), core_id,
      thread_id, arg0, arg1, description);
...
static PyMethodDef PyStatsMethods[] = {
    {"get", getStatsValue, METH_VARARGS,
     "Retrieve current value of statistic (objectName, index, metricName)."},
    {"getter", getStatsGetter, METH_VARARGS,
     "Return object to retrieve statistics value."},
    {"write", writeStats, METH_VARARGS,
     "Write statistics (<prefix>, [<filename>])."},
    {"register", registerStats, METH_VARARGS,
     "Register callback that defines statistics value for (objectName, index, "
     "metricName)."},
    {"register_per_thread", registerPerThread, METH_VARARGS,
     "Add a per-thread statistic (perthreadName) based on a named statistic "
     "(objectName, metricName)."},
    {"marker", writeMarker, METH_VARARGS,
     "Record a marker (coreid, threadid, arg0, arg1, [description])."},
    {"time", getTime, METH_VARARGS,
     "Retrieve the current global time in femtoseconds (approximate, last "
     "barrier)."},
    {"icount", getIcount, METH_VARARGS,
     "Retrieve current global instruction count."},
...
void HooksPy::PyStats::setup(void) {
  ...
  if (PyType_Ready(&statsGetterType) < 0) return;
