...
static PyObject *readMemory(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "lll", &core_id, &address, &size)) return NULL;

  Core *core = Sim()->getCoreManager()->getCoreFromID(core_id);
  LOG_ASSERT_ERROR(core != NULL, "Invalid core_id %d", core_id);

  char *buf = new char[size];
  core->accessMemory(Core::NONE, Core::READ, address, buf, size,
...
static PyObject *readCstr(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "il", &core_id, &address)) return NULL;
  ...
  char *ptr = buf;
  do {
    core->accessMemory(Core::NONE, Core::READ, address, ptr, chunksize,
                       Core::MEM_MODELED_NONE);
    address += chunksize;
    ptr += chunksize;
...
static PyMethodDef PyMemMethods[] = {
    ...
    {"read_cstr", readCstr, METH_VARARGS,
     "Read null-terminated string (core, address)"},
    {NULL, NULL, 0, NULL} /* Sentinel */
};

void HooksPy::PyMem::setup(void) { Py_InitModule("sim_mem", PyMemMethods); }
