...
void HooksPy::init() {
  ...
  for (UInt64 i = 0; i < numscripts; ++i) {
    String scriptname =
        Sim()->getCfg()->getString(String("hooks/script") + itostr(i) + "name");
    if (scriptname.substr(scriptname.length() - 3) == ".py") {
      if (!pyInit) {
        setup();
      }

      String args = Sim()->getCfg()->getString(String("hooks/script") +
                                               itostr(i) + "args");
      char *argv[] = {(char *)(scriptname.c_str()), (char *)(args.c_str())};
      PySys_SetArgvEx(2, argv, 0 /* updatepath */);

      printf("Executing Python script %s\n", scriptname.c_str());
      int s = PyRun_SimpleFileEx(fopen(scriptname.c_str(), "r"),
                                 scriptname.c_str(), 1 /* closeit */);
      if (s != 0) {
        PyErr_Print();
...
void HooksPy::setup() {
  ...
  LOG_ASSERT_ERROR(sim_root,
                   "Please make sure SNIPER_ROOT or GRAPHITE_ROOT is set");
