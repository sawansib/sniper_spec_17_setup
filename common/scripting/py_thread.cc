...
static PyObject *getThreadAppid(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "l", &thread_id)) return NULL;
...
static PyObject *getThreadAffinity(PyObject *self, PyObject *args) {
  ...
  cpu_set_t mask;
  bool success = Sim()->getThreadManager()->getScheduler()->threadGetAffinity(
      thread_id, sizeof(cpu_set_t), &mask);
  if (!success) {
  ...
  for (unsigned int cpu = 0; cpu < Sim()->getConfig()->getApplicationCores();
       ++cpu) {
...
static PyObject *setThreadAffinity(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "lO", &thread_id, &py_mask)) return NULL;
  ...
  if (PySequence_Size(py_mask) >
      (Py_ssize_t)Sim()->getConfig()->getApplicationCores()) {
    PyErr_SetString(PyExc_ValueError,
                    "Core mask is longer than number of available cores");
    return NULL;
  }

  cpu_set_t mask;
  CPU_ZERO(&mask);
  for (unsigned int cpu = 0; cpu < (unsigned int)PySequence_Size(py_mask) &&
                             cpu < 8 * sizeof(cpu_set_t);
       ++cpu) {
    PyObject *item = PySequence_ITEM(py_mask, cpu);
    if (PyObject_IsTrue(item)) CPU_SET(cpu, &mask);
    Py_DECREF(item);
  }

  bool result = Sim()->getThreadManager()->getScheduler()->threadSetAffinity(
      INVALID_THREAD_ID, thread_id, sizeof(cpu_set_t), &mask);
...
static PyMethodDef PyThreadMethods[] = {
    ...
    {"get_thread_appid", getThreadAppid, METH_VARARGS,
     "Get application ID for a thread"},
    {"get_thread_name", getThreadName, METH_VARARGS, "Get thread name"},
    {"get_thread_affinity", getThreadAffinity, METH_VARARGS,
     "Get thread affinity"},
    {"set_thread_affinity", setThreadAffinity, METH_VARARGS,
     "Set thread affinity"},
