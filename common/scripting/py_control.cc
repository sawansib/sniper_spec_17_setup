...
static PyObject *setROI(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "b", &inRoi)) return NULL;
...
static PyObject *setInstrumentationMode(PyObject *self, PyObject *args) {
  ...
  switch (mode) {
    ...
    case SIM_OPT_INSTRUMENT_FASTFORWARD:
      Sim()->getMagicServer()->Magic_unlocked(
          INVALID_CORE_ID, INVALID_THREAD_ID, SIM_CMD_INSTRUMENT_MODE, mode, 0);
      break;
    default:
      LOG_PRINT_ERROR("Unexpected instrumentation mode from python: %lx.",
                      mode);
      return NULL;
  }
...
static PyObject *setProgress(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "f", &progress)) return NULL;
...
static PyObject *simulatorAbort(PyObject *self, PyObject *args) {
  ...
  // For benchmarks where, after ROI, functionally simulating until the end
  // takes too long.
...
static PyMethodDef PyControlMethods[] = {
    ...
    {"set_instrumentation_mode", setInstrumentationMode, METH_VARARGS,
     "Set instrumentation mode"},
    {"set_progress", setProgress, METH_VARARGS,
     "Set simulation progress indicator (0..1)"},
