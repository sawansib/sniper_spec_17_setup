...
Sift::Reader::Reader(const char *filename, const char *response_filename,
                     uint32_t id)
    : input(NULL),
      response(NULL),
  ...
  // initing stream here could cause deadlock when using pipes, as this should
  // be able to be new()ed from a thread that should not block
}
...
Sift::Reader::~Reader() {
  ...
  for (std::unordered_map<uint64_t, const uint8_t *>::iterator i =
           icache.begin();
       i != icache.end(); ++i) {
    delete[] (*i).second;
  }
  for (std::unordered_map<uint64_t, const StaticInstruction *>::iterator i =
           scache.begin();
       i != scache.end(); ++i) {
    delete (*i).second;
  }
...
bool Sift::Reader::Read(Instruction &inst) {
  ...
  while (!m_seen_end) {
      ...
      switch (rec.Other.type) {
        ...
        case RecOtherIcacheVariable: {
          ...
          while (size_left > 0) {
            ...
            size_t read_amount =
                std::min(size_left, size_t(ICACHE_SIZE - offset));
            input->read(const_cast<char *>(reinterpret_cast<const char *>(
                            &(icache[base_addr][offset]))),
                        read_amount);

#if VERBOSE_ICACHE
            std::cerr << __FUNCTION__ << ": Wrote " << read_amount
                      << " bytes to 0x" << std::hex
                      << (void *)&(icache[base_addr][offset]) << std::dec
                      << std::endl;
...
#if VERBOSE > 0
          std::cerr << "[DEBUG:" << m_id << "] Read InstructionCount"
                    << std::endl;
#endif
        ...
        case RecOtherCacheOnly: {
...
#endif
          assert(rec.Other.size == sizeof(uint8_t) + sizeof(uint8_t) +
          ...
          if (handleCacheOnlyFunc)
            handleCacheOnlyFunc(handleCacheOnlyArg, icount,
                                (Sift::CacheOnlyType)type, eip, address);
          break;
        }
        case RecOtherOutput: {
#if VERBOSE > 0
        ...
        case RecOtherSyscallRequest: {
#if VERBOSE > 0
          std::cerr << "[DEBUG:" << m_id << "] Read SyscallRequest"
                    << std::endl;
#endif
          ...
          uint8_t *bytes = new uint8_t[size];
          input->read(reinterpret_cast<char *>(&syscall_number),
                      sizeof(uint16_t));
          input->read(reinterpret_cast<char *>(bytes), size);
#if VERBOSE_HEX > 0
          ...
          for (int i = 0; i < (size / 8); i++) {
            std::cerr << __FUNCTION__ << ": syscall args[" << i
                      << "] = " << ((uint64_t *)bytes)[i] << std::endl;
          }
#endif
...
#endif
            uint64_t ret = handleSyscallFunc(handleSyscallArg, syscall_number,
                                             bytes, size);
...
#if VERBOSE > 0
            std::cerr << "[DEBUG:" << m_id << "] HandleNewThread Done"
                      << std::endl;
#endif
          }
          break;
        ...
        case RecOtherMagicInstruction: {
          ...
          sendSimpleResponse(RecOtherMagicInstructionResponse, &result,
                             sizeof(result));
          break;
        }
        case RecOtherEmu: {
          ...
          input->read(reinterpret_cast<char *>(&req),
                      rec.Other.size - sizeof(uint16_t));
          bool result = false;
          EmuReply res = {};
          if (handleEmuFunc) {
        ...
        case RecOtherRoutineChange: {
          ...
          if (handleRoutineChangeFunc)
            handleRoutineChangeFunc(handleRoutineArg,
                                    Sift::RoutineOpType(event), eip, esp,
                                    callEip);
          break;
        }
        case RecOtherRoutineAnnounce: {
          ...
          input->read(reinterpret_cast<char *>(&len_filename),
                      sizeof(uint16_t));
          filename = (char *)malloc(len_filename);
          input->read(filename, len_filename);
          if (handleRoutineAnnounceFunc)
            handleRoutineAnnounceFunc(handleRoutineArg, eip, name, imgname,
                                      offset, line, column, filename);
          free(name);
          free(filename);
          break;
        }
    ...
    for (int i = 0; i < inst.num_addresses; ++i)
      input->read(reinterpret_cast<char *>(&inst.addresses[i]),
                  sizeof(uint64_t));
...
#endif
#if VERBOSE > 2
    printf("%016lx (%d) A%u %c%c %c%c\n", inst.sinst->addr, inst.sinst->size,
           inst.num_addresses, inst.is_branch ? 'B' : '.',
  ...
  // We should not return false (no more instructions) unless we get the End
  // packet. Return true in case we get to this point (which we shouldn't).
  return true;
}

void Sift::Reader::AccessMemory(MemoryLockType lock_signal, MemoryOpType mem_op,
                                uint64_t d_addr, uint8_t *data_buffer,
                                uint32_t data_size) {
#if VERBOSE > 0
  if (mem_op == MemWrite)
    std::cerr << "[DEBUG:" << m_id << "] Write MemoryRequest - Write"
              << std::endl;
  if (mem_op == MemWrite)
    std::cerr << "[DEBUG:" << m_id << "] Write MemoryRequest - Read"
              << std::endl;
#endif
  ...
  rec.Other.size =
      sizeof(d_addr) + sizeof(data_size) + sizeof(lock_signal) + sizeof(mem_op);
  if (mem_op == MemWrite) {
  ...
  if (mem_op == MemRead) {
#if VERBOSE > 0
    std::cerr << "[DEBUG:" << m_id << "] Read MemoryResponse - Read Return Data"
              << std::endl;
#endif
    ...
    std::cerr << "Sift::Reader::" << __FUNCTION__
              << ": invalid return memory op type" << std::endl;
    assert(false);
  }
}

const Sift::StaticInstruction *Sift::Reader::decodeInstruction(uint64_t addr,
                                                               uint8_t size) {
  ...
  xed_decoded_inst_zero_set_mode(
      const_cast<xed_decoded_inst_t *>(&sinst->xed_inst), &xed_state);
  xed_error_enum_t result =
      xed_decode(const_cast<xed_decoded_inst_t *>(&sinst->xed_inst),
                 sinst->data, sinst->size);
  assert(result == XED_ERROR_NONE);

  return sinst;
}

const Sift::StaticInstruction *Sift::Reader::getStaticInstruction(
    uint64_t addr, uint8_t size) {
  const StaticInstruction *sinst;

  // Lookup in a large unordered_map is quite expensive if we have to do this
  // for every dynamic instruction Therefore, keep a pointer to the probable
  // next instruction in each (static) instruction
...
void Sift::Reader::sendSimpleResponse(RecOtherType type, void *data,
                                      uint32_t size) {
#if VERBOSE > 0
  std::cerr << "[DEBUG:" << m_id << "] Write SimpleResponse type=" << type
            << std::endl;
#endif
...
uint64_t Sift::Reader::getLength() { return filesize; }
