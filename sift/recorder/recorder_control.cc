...
void beginROI(THREADID threadid) {
  if (app_id < 0) findMyAppId();

  if (in_roi) {
    std::cerr << "[SIFT_RECORDER:" << app_id
              << "] Error: ROI_START seen, but we have already started."
              << std::endl;
  } else {
  ...
  if (KnobEmulateSyscalls.Value()) {
    ...
    for (unsigned int i = 0; i < MAX_NUM_THREADS; i++) {
      if (thread_data[i].running && !thread_data[i].output) openFile(i);
    }
  }
}
...
void endROI(THREADID threadid) {
  if (KnobEmulateSyscalls.Value()) {
    ...
    thread_data[threadid].output->Syscall(SYS_exit_group, (char *)args,
                                          sizeof(args));
  ...
  sprintf(filename, "%s.app%" PRId32 ".appid", KnobOutputFile.Value().c_str(),
          app_id);
  unlink(filename);
  ...
  if (!KnobUseResponseFiles.Value()) {
    for (unsigned int i = 0; i < MAX_NUM_THREADS; i++) {
      if (thread_data[i].running && thread_data[i].output) closeFile(i);
    }
  }
}
...
void setInstrumentationMode(Sift::Mode mode) {
  if (current_mode != mode && mode != Sift::ModeUnknown) {
    ...
    switch (mode) {
      ...
      case Sift::ModeStop:
        for (unsigned int i = 0; i < MAX_NUM_THREADS; i++) {
          if (thread_data[i].output) closeFile(i);
        }
        exit(0);
...
ADDRINT handleMagic(THREADID threadid, ADDRINT gax, ADDRINT gbx, ADDRINT gcx) {
  ...
  if (KnobUseResponseFiles.Value() && thread_data[threadid].running &&
      thread_data[threadid].output) {
    res = thread_data[threadid].output->Magic(gax, gbx, gcx);
  }

  if (gax == SIM_CMD_ROI_START) {
    if (KnobUseROI.Value() && !in_roi) beginROI(threadid);
  } else if (gax == SIM_CMD_ROI_END) {
    if (KnobUseROI.Value() && in_roi) endROI(threadid);
  }
...
void findMyAppId() {
  ...
  // Try to lock from app_id 0 onwards, claiming the app_id if the lock
  // succeeds, and fail when there are no more files
  for (uint64_t id = 0; id < 1000; ++id) {
    char filename[1024] = {0};

    // First check whether this many appids are supported by stat()ing the
    // request FIFO for thread 0
    struct stat sts;
    sprintf(filename, "%s.app%" PRIu64 ".th%" PRIu64 ".sift",
            KnobOutputFile.Value().c_str(), id, (uint64_t)0);
    if (stat(filename, &sts) != 0) {
      break;
    }

    // Atomically create .appid file
    sprintf(filename, "%s.app%" PRIu64 ".appid", KnobOutputFile.Value().c_str(),
            id);
    int fd = open(filename, O_CREAT | O_EXCL, 0600);
    if (fd != -1) {
      // Success: use this app_id
      app_id = id;
      std::cerr << "[SIFT_RECORDER:" << app_id << "] Application started"
                << std::endl;
      return;
    }
  ...
  std::cerr
      << "[SIFT_RECORDER] Cannot find free application id, too many processes!"
      << std::endl;
  exit(1);
}
...
static void routineCallback(RTN rtn, void *v) {
  if (KnobMPIImplicitROI.Value()) {
    ...
    if (rtn_name.find("MPI_Init") != string::npos &&
        rtn_name.find("MPI_Initialized") ==
            string::npos)  // Actual name can be MPI_Init, MPI_Init_thread,
                           // PMPI_Init_thread, etc.
    {
      RTN_Open(rtn);
      RTN_InsertCall(rtn, IPOINT_AFTER, AFUNPTR(handleRoutineImplicitROI),
                     IARG_THREAD_ID, IARG_BOOL, true, IARG_END);
      RTN_Close(rtn);
    }
    if (rtn_name.find("MPI_Finalize") != string::npos) {
      RTN_Open(rtn);
      RTN_InsertCall(rtn, IPOINT_BEFORE, AFUNPTR(handleRoutineImplicitROI),
...
void openFile(THREADID threadid) {
  ...
  if (KnobUseResponseFiles.Value() == 0) {
    if (blocksize)
      sprintf(filename, "%s.%" PRIu64 ".sift", KnobOutputFile.Value().c_str(),
              thread_data[threadid].blocknum);
    else
      sprintf(filename, "%s.sift", KnobOutputFile.Value().c_str());
  } else {
    if (blocksize)
      sprintf(filename, "%s.%" PRIu64 ".app%" PRId32 ".th%" PRIu64 ".sift",
              KnobOutputFile.Value().c_str(), thread_data[threadid].blocknum,
              app_id, thread_data[threadid].thread_num);
    else
      sprintf(filename, "%s.app%" PRId32 ".th%" PRIu64 ".sift",
              KnobOutputFile.Value().c_str(), app_id,
              thread_data[threadid].thread_num);
  }

  if (KnobVerbose.Value())
    std::cerr << "[SIFT_RECORDER:" << app_id << ":"
              << thread_data[threadid].thread_num << "] Output = [" << filename
              << "]" << std::endl;

  if (KnobUseResponseFiles.Value()) {
    sprintf(response_filename, "%s_response.app%" PRId32 ".th%" PRIu64 ".sift",
            KnobOutputFile.Value().c_str(), app_id,
            thread_data[threadid].thread_num);
    if (KnobVerbose.Value())
      std::cerr << "[SIFT_RECORDER:" << app_id << ":"
                << thread_data[threadid].thread_num << "] Response = ["
                << response_filename << "]" << std::endl;
  }
...
#endif
    thread_data[threadid].output = new Sift::Writer(
        filename, getCode, KnobUseResponseFiles.Value() ? false : true,
        response_filename, threadid, arch32, false,
        KnobSendPhysicalAddresses.Value());
  } catch (...) {
    std::cerr << "[SIFT_RECORDER:" << app_id << ":"
              << thread_data[threadid].thread_num
              << "] Error: Unable to open the output file " << filename
              << std::endl;
    exit(1);
  }
...
void closeFile(THREADID threadid) {
  if (KnobVerbose.Value()) {
    std::cerr << "[SIFT_RECORDER:" << app_id << ":"
              << thread_data[threadid].thread_num << "] Recorded "
  ...
  if (blocksize) {
    if (thread_data[threadid].bbv_count) {
      ...
      thread_data[threadid].bbv_base =
          0;  // Next instruction starts a new basic block
      thread_data[threadid].bbv_count = 0;
    }

    char filename[1024];
    sprintf(filename, "%s.%" PRIu64 ".bbv", KnobOutputFile.Value().c_str(),
            thread_data[threadid].blocknum);
    ...
    for (int i = 0; i < Bbv::NUM_BBV; ++i)
      fprintf(fp, "%" PRIu64 "\n",
              thread_data[threadid].bbv->getDimension(i) /
                  thread_data[threadid].bbv->getInstructionCount());
    fclose(fp);
...
void initRecorderControl() { RTN_AddInstrumentFunction(routineCallback, 0); }
