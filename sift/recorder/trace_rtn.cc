...
static void routineEnter(THREADID threadid, ADDRINT eip, ADDRINT esp) {
  if ((any_thread_in_detail || KnobRoutineTracingOutsideDetailed.Value()) &&
    ...
    thread_data[threadid].output->RoutineChange(
        Sift::RoutineEnter, eip, esp, thread_data[threadid].last_call_site);
...
static void announceRoutine(INS ins) {
  if (!thread_data[PIN_ThreadId()].output) return;
  ...
  thread_data[PIN_ThreadId()].output->RoutineAnnounce(
      eip, RTN_Valid(rtn) ? RTN_Name(rtn).c_str() : "??",
      IMG_Valid(img) ? IMG_Name(img).c_str() : "??",
      IMG_Valid(img) ? IMG_LoadOffset(img) : 0, column, line, filename.c_str());
}

static void announceRoutine(RTN rtn) { announceRoutine(RTN_InsHeadOnly(rtn)); }
...
static void routineCallback(RTN rtn, VOID *v) {
  ...
  if (routines.count(RTN_Address(rtn)) == 0) announceRoutine(rtn);

  RTN_InsertCall(rtn, IPOINT_BEFORE, AFUNPTR(routineEnter), IARG_THREAD_ID,
                 IARG_ADDRINT, RTN_Address(rtn), IARG_REG_VALUE, REG_STACK_PTR,
                 IARG_END);
  RTN_InsertCall(rtn, IPOINT_AFTER, AFUNPTR(routineExit), IARG_THREAD_ID,
                 IARG_ADDRINT, RTN_Address(rtn), IARG_REG_VALUE, REG_STACK_PTR,
                 IARG_END);
...
static void traceCallback(TRACE trace, VOID *v) {
  // At the start of each trace, check to see if this part of the code belongs
  // to the function we think we're in. This will detect longjmps and tail call
  // elimination, and fix up the call stack appropriately.
  ...
  if (RTN_Valid(rtn)) {
    ...
    TRACE_InsertCall(trace, IPOINT_BEFORE, AFUNPTR(routineAssert),
                     IARG_THREAD_ID, IARG_ADDRINT, RTN_Address(rtn),
                     IARG_REG_VALUE, REG_STACK_PTR, IARG_END);
  } else {
    if (routines.count(0) == 0) announceInvalidRoutine();

    TRACE_InsertCall(trace, IPOINT_BEFORE, AFUNPTR(routineAssert),
                     IARG_THREAD_ID, IARG_ADDRINT, 0, IARG_REG_VALUE,
  ...
  for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl))
    for (INS ins = BBL_InsHead(bbl); INS_Valid(ins); ins = INS_Next(ins)) {
      if (INS_IsProcedureCall(ins)) {
        announceRoutine(ins);
        INS_InsertCall(ins, IPOINT_BEFORE, AFUNPTR(routineCallSite),
