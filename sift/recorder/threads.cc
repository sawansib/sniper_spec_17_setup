...
static_assert((sizeof(thread_data_t) % LINE_SIZE_BYTES) == 0,
              "Error: Thread data should be a multiple of the line size to "
              "prevent false sharing");

thread_data_t *thread_data;

// The thread that watched this new thread start is responsible for setting up
// the connection with the simulator
static VOID threadStart(THREADID threadid, CONTEXT *ctxt, INT32 flags,
  ...
  if (threadid > 0 && (any_thread_in_detail || KnobEmulateSyscalls.Value())) {
    ...
    // We should send a EmuTypeSetThreadInfo, but not now as we hold the Pin VM
    // lock: Sending EmuTypeSetThreadInfo requires the response channel to be
    // opened, which is done by TraceThread but not any time soon if we aren't
    // scheduled on a core.
...
static VOID threadFinishHelper(VOID *arg) {
  ...
  if (thread_data[threadid].tid_ptr) {
    ...
    *(int *)tid = 0;
    // Send the FUTEX_WAKE to the simulator to wake up a potential
    // pthread_join() caller
    syscall_args_t args = {0};
    args[0] = (intptr_t)tid;
    args[1] = FUTEX_WAKE;
    args[2] = 1;
    thread_data[threadid].output->Syscall(SYS_futex, (char *)args,
                                          sizeof(args));
  }
...
static VOID threadFinish(THREADID threadid, const CONTEXT *ctxt, INT32 flags,
                         VOID *v) {
#if DEBUG_OUTPUT
  std::cerr << "[SIFT_RECORDER:" << app_id << ":"
            << thread_data[threadid].thread_num << "] Finish Thread"
            << std::endl;
#endif

  if (thread_data[threadid].thread_num == 0 && thread_data[threadid].output &&
      KnobEmulateSyscalls.Value()) {
  ...
  // To prevent deadlocks during simulation, start a new thread to handle this
  // thread's cleanup.  This is needed because this function could be called in
  // the context of another thread, creating a deadlock scenario.
  PIN_SpawnInternalThread(threadFinishHelper, (VOID *)(unsigned long)threadid,
                          0, NULL);
}
