...
void handleAccessMemory(void *arg, Sift::MemoryLockType lock_signal,
                        Sift::MemoryOpType mem_op, uint64_t d_addr,
                        uint8_t *data_buffer, uint32_t data_size) {
  // Lock memory globally if requested
  // This operation does not occur very frequently, so this should not impact
  // performance
...
// Do this as a regular callback (versus syscall enter/exit functions) as those
// hold the global pin lock
...
#else
#error \
    "Unknown target architecture, require either TARGET_IA32 or TARGET_INTEL64"
#endif
  ...
  if (KnobEmulateSyscalls.Value() && thread_data[threadid].output) {
    switch (syscall_number) {
      ...
      // When the CLONE_CHILD_CLEARTID option is enabled, remember its
      // child_tidptr and then when the thread ends, write 0 to the tid mutex
      // and futex_wake it
      case SYS_clone: {
...
// Store the thread's tid ptr for later use
      ...
      case SYS_wait4:
        ...
        thread_data[threadid].last_syscall_emulated = false;
        thread_data[threadid].output->Syscall(syscall_number, (char *)args,
                                              sizeof(args));
        break;
...
static VOID syscallEntryCallback(THREADID threadid, CONTEXT *ctxt,
...
static VOID syscallExitCallback(THREADID threadid, CONTEXT *ctxt,
                                SYSCALL_STANDARD syscall_standard, VOID *v) {
  if (!thread_data[threadid].last_syscall_emulated) {
    return;
  }

  PIN_SetContextReg(ctxt, REG_GAX,
                    thread_data[threadid].last_syscall_returnval);
  thread_data[threadid].last_syscall_emulated = false;
}
