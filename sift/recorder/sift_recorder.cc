...
void __sift_assert_fail(__const char *__assertion, __const char *__file,
                        unsigned int __line, __const char *__function) __THROW {
  if (assert_ignore()) {
    // Timing model says it's done, ignore assert and pretend to have exited
    // cleanly
    exit(0);
  } else {
    std::cerr << "[SIFT_RECORDER] " << __file << ":" << __line << ": "
...
int main(int argc, char **argv) {
  ...
  if (posix_memalign((void **)&thread_data, LINE_SIZE_BYTES,
                     thread_data_size) != 0) {
  ...
  // When attaching with --pid, there could be a number of threads already
  // running. Manually call NewThread() because the normal method to start new
  // thread pipes (SYS_clone) will already have happened
  ...
  if (KnobReplayer.Value()) {
    if (KnobEmulateSyscalls.Value()) {
      std::cerr << "Error, emulating syscalls is not compatible with PinPlay "
                   "replaying."
                << std::endl;
      exit(1);
    }
    pinplay_engine.Activate(argc, argv, false /*logger*/,
                            KnobReplayer.Value() /*replayer*/);
  }
#else
  if (KnobReplayer.Value()) {
    std::cerr << "Error, PinPlay support not compiled in. Please use a "
                 "compatible pin kit when compiling."
              << std::endl;
    exit(1);
  }
#endif
  ...
  if (KnobEmulateSyscalls.Value()) {
    if (!KnobUseResponseFiles.Value()) {
      std::cerr
          << "Error, Response files are required when using syscall emulation."
          << std::endl;
      exit(1);
    }
  ...
  if (KnobRoutineTracing.Value()) initRoutineTracer();
