...
ozstream::ozstream(vostream* output) : output(output) {
...
void ozstream::doCompress(bool finish) {
  ...
  if (finish) assert(ret == Z_STREAM_END);
}

izstream::izstream(vistream* input)
    : input(input), m_eof(false), m_fail(false), peek_valid(false) {
  zstream.zalloc = Z_NULL;
  zstream.zfree = Z_NULL;
  zstream.opaque = Z_NULL;
  zstream.avail_in = 0;
...
void izstream::read(char* s, std::streamsize n) {
  ...
  do {
    if (zstream.avail_in == 0)  // If input data was left over from the previous
                                // call, use that up first
    {
...
int izstream::peek() {
  if (peek_valid == true) return peek_value;
