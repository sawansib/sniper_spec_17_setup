...
int main(int argc, char* argv[]) {
  if (argc > 1 && strcmp(argv[1], "-d") == 0) {
    ...
    while (reader.Read(inst)) {
      if ((icount++ & 0xffff) == 0)
        fprintf(stderr, "Reading SIFT trace: %" PRId64 "%%\r",
                100 * reader.getPosition() / reader.getLength());
    ...
    for (auto it = instructions.begin(); it != instructions.end(); ++it) {
      if (eip_last && (it->first != eip_last)) printf("\n");
      ...
      for (int i = it->second->size; i < 8; ++i) printf("   ");
      char buffer[64];
#if PIN_REV >= 67254
      xed_format_context(syntax, &it->second->xed_inst, buffer,
                         sizeof(buffer) - 1, it->first, 0, 0);
#else
      xed_format(syntax, &it->second->xed_inst, buffer, sizeof(buffer) - 1,
                 it->first);
#endif
      printf("  %-40s\n", buffer);
      if (it->second->size > 8) {
    ...
    while (reader.Read(inst)) {
      ...
      char buffer[64];
#if PIN_REV >= 67254
      xed_format_context(syntax, &inst.sinst->xed_inst, buffer,
                         sizeof(buffer) - 1, inst.sinst->addr, 0, 0);
#else
      xed_format(syntax, &inst.sinst->xed_inst, buffer, sizeof(buffer) - 1,
                 inst.sinst->addr);
#endif
      printf("%-40s  ", buffer);
