...
__attribute__((weak)) void __sift_assert_fail(
    __const char *__assertion, __const char *__file, unsigned int __line,
    __const char *__function) __THROW {
  __assert_fail(__assertion, __file, __line, __function);
}

Sift::Writer::Writer(const char *filename, GetCodeFunc getCodeFunc,
                     bool useCompression, const char *response_filename,
                     uint32_t id, bool arch32, bool requires_icache_per_insn,
                     bool send_va2pa_mapping)
    : response(NULL),
  ...
  uint64_t options = 0;
  if (useCompression) options |= CompressionZlib;
  if (arch32) options |= ArchIA32;
  if (requires_icache_per_insn) options |= IcacheVariable;
  if (m_send_va2pa_mapping) options |= PhysicalAddress;
  ...
  if (options & CompressionZlib) output = new ozstream(output);
}
...
void Sift::Writer::End() {
  ...
  if (response) {
    /*
          // Disable EndResponse because of lock-up issues with Pin and
       sift_recorder #if VERBOSE > 0 std::cerr << "[DEBUG:" << m_id << "] Write
       End - Response Wait" << std::endl; #endif

          Record respRec;
          response->read(reinterpret_cast<char*>(&respRec),
       sizeof(respRec.Other)); sift_assert(respRec.Other.zero == 0);
...
Sift::Writer::~Writer() {
  ...
  for (int i = 1; i < 16; ++i) printf(" %u", hsize[i]);
  printf(" haddr");
  for (int i = 1; i <= MAX_DYNAMIC_ADDRESSES; ++i) printf(" %u", haddr[i]);
  printf(" branch %u predicate %u\n", nbranch, npredicate);
  printf("instrsmall %u ext %u\n", ninstrsmall, ninstrext);
#endif
}

void Sift::Writer::Instruction(uint64_t addr, uint8_t size,
                               uint8_t num_addresses, uint64_t addresses[],
                               bool is_branch, bool taken, bool is_predicate,
                               bool executed) {
  sift_assert(size < 16);
  ...
  if (m_requires_icache_per_insn) {
    if (!icache[addr]) {
#if VERBOSE_ICACHE
      std::cerr << "[DEBUG:" << m_id << "] Write icache per instruction addr=0x"
    ...
    // Send ICACHE record?
    for (uint64_t base_addr = addr & ICACHE_PAGE_MASK;
         base_addr <= ((addr + size - 1) & ICACHE_PAGE_MASK);
...
#if VERBOSE > 2
  printf("%016lx (%d) A%u %c%c %c%c\n", addr, size, num_addresses,
         is_branch ? 'B' : '.', is_branch ? (taken ? 'T' : '.') : '.',
         is_predicate ? 'C' : '.', is_predicate ? (executed ? 'E' : 'n') : '.');
#endif

  send_va2pa(addr);
  for (int i = 0; i < num_addresses; ++i) send_va2pa(addresses[i]);
...
#if VERBOSE > 2
    std::cerr << "[DEBUG:" << m_id << "] Write Simple Full Instruction"
              << std::endl;
#endif
...
void Sift::Writer::CacheOnly(uint8_t icount, CacheOnlyType type, uint64_t eip,
                             uint64_t address) {
#if VERBOSE > 1
...
#if VERBOSE > 0
        std::cerr << "[DEBUG:" << m_id << "] Read NewThreadResponse"
                  << std::endl;
#endif
...
#if VERBOSE > 0
        std::cerr << "[DEBUG:" << m_id
                  << "] Got NewThreadResponse thread=" << retcode << std::endl;
#endif
        return retcode;
        break;
      default:
...
uint64_t Sift::Writer::Syscall(uint16_t syscall_number, const char *data,
                               uint32_t size) {
#if VERBOSE > 0
  std::cerr << "[DEBUG:" << m_id << "] Write Syscall" << std::endl;
#endif

  // Try to send some extra logical2physical address mappings for data
  // referenced by system call arguments. Also try to read from the address
  // first, if the mapping wasn't set up yet (never accessed before, or swapped
  // out), then this will cause a page fault that brings in the data.
...
int32_t Sift::Writer::Join(int32_t thread) {
#if VERBOSE > 0
  std::cerr << "[DEBUG:" << m_id << "] Write Join with thread=" << thread
            << std::endl;
#endif
  ...
  while (true) {
#if VERBOSE > 0
    std::cerr << "[DEBUG:" << m_id << "] Join Waiting for Response"
              << std::endl;
#endif
    Record respRec;
    response->read(reinterpret_cast<char *>(&respRec), sizeof(rec.Other));
...
bool Sift::Writer::Emulate(Sift::EmuType type, Sift::EmuRequest &req,
                           Sift::EmuReply &res) {
  // send magic
  Record rec;
  rec.Other.zero = 0;
  ...
  while (true) {
    ...
    switch (respRec.Other.type) {
      case RecOtherEmuResponse: {
        ...
        response->read(reinterpret_cast<char *>(&res),
                       respRec.Other.size - sizeof(uint8_t));
        return result;
      }
      case RecOtherMemoryRequest:
        handleMemoryRequest(respRec);
...
void Sift::Writer::RoutineChange(Sift::RoutineOpType event, uint64_t eip,
                                 uint64_t esp, uint64_t callEip) {
  Record rec;
  rec.Other.zero = 0;
...
void Sift::Writer::RoutineAnnounce(uint64_t eip, const char *name,
                                   const char *imgname, uint64_t offset,
                                   uint32_t line, uint32_t column,
                                   const char *filename) {
  uint16_t len_name = strlen(name) + 1, len_imgname = strlen(imgname) + 1,
           len_filename = strlen(filename) + 1;

  Record rec;
  rec.Other.zero = 0;
  rec.Other.type = RecOtherRoutineAnnounce;
  rec.Other.size = sizeof(uint64_t) + sizeof(uint16_t) + len_name +
                   sizeof(uint16_t) + len_imgname + sizeof(uint64_t) +
...
void Sift::Writer::handleMemoryRequest(Record &respRec) {
  ...
  MemoryLockType lock;
  MemoryOpType type;
  sift_assert(respRec.Other.size >=
              (sizeof(addr) + sizeof(size) + sizeof(lock) + sizeof(type)));
  ...
  uint32_t payload_size = respRec.Other.size - (sizeof(addr) + sizeof(size) +
                                                sizeof(lock) + sizeof(type));
  ...
  if (type == MemRead) {
    ...
    handleAccessMemoryFunc(handleAccessMemoryArg, lock, type, addr,
    ...
    // Do the write here via a callback to write the data to the appropriate
    // address
    handleAccessMemoryFunc(handleAccessMemoryArg, lock, type, addr,
...
uint64_t Sift::Writer::va2pa_lookup(uint64_t vp) {
  ...
  if (vp >= 0xffffffffff600ULL && vp < 0xfffffffffffffULL) return vp;
