...
class vofstream : public vostream {
 ...
 public:
  vofstream(const char *filename,
            std::ios_base::openmode mode = std::ios_base::out)
      : stream(new std::ofstream(filename, mode)) {}
  vofstream(std::ofstream *stream) : stream(stream) {}
  virtual ~vofstream() { delete stream; }
  virtual void write(const char *s, std::streamsize n) { stream->write(s, n); }
  virtual void flush() { stream->flush(); }
  virtual void fail() { stream->fail(); }
...
class ozstream : public vostream {
 ...
 public:
  ...
  virtual bool is_open() { return output->is_open(); }
};
...
class vifstream : public vistream {
 ...
 public:
  vifstream(const char *filename,
            std::ios_base::openmode mode = std::ios_base::in)
      : stream(new std::ifstream(filename, mode)) {}
  vifstream(std::ifstream *stream) : stream(stream) {}
  virtual ~vifstream() { delete stream; }
  virtual void read(char *s, std::streamsize n) { stream->read(s, n); }
